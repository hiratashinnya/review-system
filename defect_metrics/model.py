...
* **分子（主指標）**: 窓内に作成された Issue のうち、本文が参照する PR に
  ...
  **参照の記法は ``#N`` と同一リポジトリの完全 URL（``https://github.com/OWNER/REPO/pull/N``）の
  両方を同じ参照として数える**（Issue #493 オーナー確定・案 (a)）。人間にとって
  ``#487`` と ``.../pull/487`` は同じ意味であり、記法差はツールからは見えない。``#N`` だけを
  拾っていると、指標の正しさが「起票者が ``#N`` 記法で書き続ける」という**どこにも記録されて
  いない前提**に乗り、前提が崩れたときに主指標が静かに下振れする（そして「欠陥混入が減った」
  ように見える）。両記法を PR 番号へ正規化した集合で扱うので、同じ PR を両記法で書いた本文が
...
**Issue #493（URL 記法の同一視）による再計算の結果＝変化なし**（2026-09-08 実測）。
定義を広げると基線窓の派生 Issue 数が増えうるため再計算したが、基線窓に作成された Issue
（番号 302〜367。#301 は 2026-08-01T15:12Z で窓の手前、#368 は 2026-08-16T05:27Z で窓の外）の
本文に現れる自リポジトリの完全 URL は #323→323・#339→339 の**自己参照2件だけ**で、
どちらも Issue であって merged PR ではない。参照の追加は派生判定を増やす方向にしか働かない
ため、派生 Issue は 15 件のまま・分母と副指標は定義が変わっていないので、下記5定数はいずれも
据え置きである（「変わらなかった」ことを記録する＝Issue #493 Acceptance criteria）。
"""
...
#:
#: Issue #493（``#N`` と完全 URL を同一の参照として数える）でも **1 のまま据え置く**。
#: 本定数が表すのは**読取側のパース互換**であり、フィールドの追加・削除・型変更は無い
#: （変わったのは ``report_window.primary.definition`` の**文言**だけで、これは値として
#: レポートに載るので読み手は定義の変化をレポート自体から観測できる）。基線値も再計算の
#: 結果 変化しなかった（上記モジュール docstring）ため、``baseline_verification.reproduced``
#: が転ぶこともない。据え置く判断を黙って行わず、ここに理由を残す。
SCHEMA_VERSION = 1
...
#: Issue #493 で参照の記法を広げた後も再計算で同値であることを確認済み（上記 docstring）。
