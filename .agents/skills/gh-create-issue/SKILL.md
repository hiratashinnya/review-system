...
3. repository の `AGENTS.md` と、そこから参照される規約を読む。AI attribution、言語、権限、スコープ、先送りに関する規約を優先する。
4. GitHub connector で repository、Issue、label、Project を読み書きできるか確認する。Codex では GitHub connector を第一選択とし、必要な機能がない場合だけ `gh` を使う。権限不足、対象不明、規約矛盾なら fail-close で停止し、必要な判断または権限をユーザーへ具体的に求める。
...
期待する名前が live metadata にない、表記が異なる、同義候補が複数ある場合は勝手に label/option を作らず停止する。差分と推奨案を示してユーザー判断を得る。
...
実行環境に Project を読み書きする手段が無い場合がある（GitHub connector に Projects 系の操作が無く、
`gh` も使えない環境など）。この場合の既定はこれまでどおり **§7 の fail-close で停止する**こと。
...
GitHub connector が body を安全に渡せるならその入力を使う。shell 経由では本文をコマンド引数へ埋め込まず、展開されない方法で用意した一時ファイルと `gh issue create --body-file <path>` を使う。`--body`、backtick、`$()`、秘密値を含む shell interpolation を避ける。
