...
### ①-0 バックログ棚卸しの契機判定（1行・該当時のみ提案）
① に入る前に、**前回棚卸しから1暦月経過**または **open が純増 +15 / 総数 70 超**を判定する。
該当したら**棚卸しの実施をオーナーへ提案**し（実施可否も処分もオーナー判断）、該当しなければ黙って ① を続ける。
運用の実体＝[docs/tools/backlog-triage.md](../../../docs/tools/backlog-triage.md)（契機・処分表の形式・
1件あたりの情報量・AI の線引き・記録先）。**本スキルはその内容を再掲しない**（Issue #376／#368 のトークン方針）。
棚卸しは**呈示の仕方だけを変える**——判断主体はオーナーのままで、起票の抑制・WIP 上限ではない（オーナー却下済み・2026-08-16）。
...
> **未了（Issue #310）**：続行/停止の判定に使う**「実害」の定義**と**エスカレーション条件**は #310 で本節に追記する。
> ここは **#308 で新設した `issue-fixer` がどこからも呼ばれない状態を作らないための最小配線**に留める。
