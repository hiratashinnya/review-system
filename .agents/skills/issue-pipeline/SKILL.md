...
- supervisorはmain checkout/共通Gitをread-only、対象worktreeだけwriteable、private `/tmp`/`/dev`、Codex API control-plane通信を維持する。innerはdirect `codex exec -C <worktree> --sandbox workspace-write`、approval never、data-plane network deny、multi-agent/apps disabledとする。full MCP broker、feature catalog束縛、Landlock EXECUTE allowlist、command毎fresh-bwrap、空procfs、CAS束縛MCPは使わない。role contract digest、durable session、protected patch、host publish CASは維持する。
