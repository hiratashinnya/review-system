...
- supervisorは外側でmain checkout/共通Gitをread-only、対象worktreeだけwriteable、`/tmp` private、network unshareとし、内側Codexで`workspace-write`、approval never、web search/shell network/multi-agent/apps disabled、指定model/reasoningを固定する。`.git`/`.codex`/`.agents`はread-onlyで、変更提案はowner-approved exact pathとbase digestをhost検証するstructured patchに限定する。
- inner Codexは編集・test・handoffまで。commit/push/PRはexit後のhost側`gitgate`とrole別allowlistへ戻し、mergeは許可しない。JSONL/exit/handoffのいずれかが不正なら非終端entryとworktreeを保持する。rate limit後も同じrole/model/reasoning/thread/worktreeでresumeする。
