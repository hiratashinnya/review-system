...
description: 資産の横展方法を確立する。移植元のスキル/サブエージェント/原則を棚卸しし、ターゲットプラットフォーム向けに手作業で変換する。種別ごとの対応表に従い、skill/prompt/agent/instructions を取り違えない。
---
...
移植元の**スキル・サブエージェント・原則**を、GitHub Copilot など別プラットフォームで使える形式に変換する。
...
Copilot には移植元環境と対応する **4 種**のカスタマイズ実体がある。**起動方式（誰がいつ呼ぶか）で機械的に振り分ける**。

| Copilot 実体 | 何者か | 誰が起動 | 移植元の対応元 |
|---|---|---|---|
| **Skill**（`.github/skills/<name>/SKILL.md`） | モデルが description で**自動発見**して読み込む能力（progressive disclosure・バンドル資産可） | モデル（自動） | model-invocable な移植元 skill（資産バンドルあり優先） |
| **Prompt**（`.github/prompts/<name>.prompt.md`） | ユーザーが `/<name>` で**明示起動**する手順テンプレ | ユーザー（`/command`） | 自動起動を止めるメタデータを持つ移植元 skill（オーケストレータ／パイプライン） |
| **Agent**（`.github/agents/<name>.agent.md`） | ツール/MCP を絞った**専門エージェントプロファイル** | エージェント選択／委譲 | 移植元サブエージェント |
| **Instructions**（`.github/instructions/<name>.instructions.md` ＋ `.github/copilot-instructions.md`） | `applyTo` glob で**自動適用**される常時コンテキスト（起動概念なし） | 自動（パターン一致時） | 常時ロードの原則・規約（常時適用メタデータ、`spec-principles`、`AGENTS.md` 等） |

**振り分け決定木（移植元 skill の場合）**：
1. 常時適用メタデータを持つ原則（例 `spec-principles`） → **Instructions**（`copilot-instructions.md` に展開）。**skill/prompt にしない**。
2. 自動起動を止めるメタデータを持つユーザー起動オーケストレータ（例 `/asset-lateral-deploy`, `/impl-design-pipeline`） → **Prompt**。
3. それ以外（モデルが自動発見すべき能力。バンドル資産があればなお skill 向き） → **Skill**。

**移植元サブエージェントは必ず Agent へ**（instructions に流さない）。**`AGENTS.md` や同等の全体規約は `copilot-instructions.md` へ**。
...
### A. Skill → Skill（移植元 skill → `.github/skills/<name>/SKILL.md`）

| 移植元 | GitHub Copilot | 変換メモ |
|---|---|---|
| `name` | `name` | そのまま |
...
### B. Skill（オーケストレータ）→ Prompt（自動起動を止めるメタデータ → `.github/prompts/<name>.prompt.md`）
...
### C. Subagent → Agent（移植元 subagent → `.github/agents/<name>.agent.md`）
...
| `model` | `model` | **強推奨**。移植元モデル名はターゲット側で利用可能な対応モデル名へ読み替え |
...
### D. 常時原則 → Instructions（常時適用メタデータ / `AGENTS.md` → `.github/copilot-instructions.md`）

| 移植元 | GitHub Copilot | 変換メモ |
|---|---|---|
| `spec-principles`（PR1–PR10） | `.github/copilot-instructions.md` に**インライン展開** | 常時適用したい中核原則 |
| `AGENTS.md` などの作業規約 | `.github/copilot-instructions.md`（または同等の全体規約） | プロジェクト全体の常時ルール |
...
1. **棚卸し**：移植元 skill・subagent・全体規約を列挙。必要なら `asset-auditor` で重複/矛盾を点検。
...
- ✓ 各移植元資産が決定木どおり skill / prompt / agent / instructions に分類され、一覧がレビュー済み。
...
- ✓ 常時原則（spec-principles / AGENTS.md 等）のみ `.github/copilot-instructions.md` に展開。`.instructions.md` は真にパススコープなものに限定。
...
**移植元仕様**
- Skills: 移植元公式ドキュメントを確認する
- Subagents: 移植元公式ドキュメントを確認する
