...
<details><summary>⬡ FND-6 · v0.2</summary>
...
<details><summary>⬡ FND-92 · v0.1</summary>
...
<details><summary>⬡ FND-93 · v0.1</summary>
...
<details><summary>⬡ FND-94 · v0.1</summary>
...
<details><summary>⬡ FND-95 · v0.1</summary>
...
<details><summary>⬡ FND-102 · v0.2</summary>
...
<details><summary>⬡ FND-103 · v0.2</summary>
...
**対応状況**: resolved（2026-06-22・②案完了）。オーナー決定により案②（resolved 系2ルールを dedicated SPEC 化）を採用し、resolved 系2ルールを全 dedicated 化して被覆を均一化した:
...
<details><summary>⬡ FND-104 · v0.2</summary>
...
**指摘時 ref_version**: FND-103 "0.1"（doc-system/04-verification/02-findings.md の FND-103 ノードバッジ v0.1 時点）。なお被指摘対象の検出機構ギャップは、(1) `docs/doc-system/config.yaml` の `fnd_lifecycle.resolved.must_not_link_to`（DD-16・Q-4 から昇格・main commit 由来でファイル version を持たないため参照箇所＝当該キーを明記）と、(2) `docs/doc-system/05-verification.md` の RULE 表（段階②・RULE-006〔L78〕／段階①・RULE-001/002/022〔L35-37〕）に所在するが、いずれもノード化されない out-of-graph 資産のため forward 辺は張れない（FND-103 が config.yaml を本文参照に留めたのと同じ扱い）。forward 辺は同根コホート（fnd_lifecycle 被覆整備）の `to: FND-103` のみとし、被指摘の所在は本文で明記する。なお解消に伴い決定スパイン辺 `to: DD-17`（案B 決定ログ・指摘時 ref_version は DD-17 起票時 "0.1"）を追加した。
...
<details><summary>⬡ FND-105 · v0.1</summary>
...
> なお「将来 config の `resolved_field` 定義や省略時既定を変更した際、in-graph SPEC が追従漏れする」リスクは WARNING 寄りだが、これは現状の欠陥ではなく将来の運用機構の話であり、本 FND を SPEC-60 で充足（dedicated SPEC 化）することで吸収される（FND-102 の同種リスク評価と整合）。

**対応状況**: resolved（2026-06-22・SPEC-60 起票で充足）。オーナー決定により「FND（ギャップ起票）と SPEC（ギャップ充足）の両方を起票」する方針に従い、`resolved` フィールドの機械判定セマンティクス（`true`=resolved／未設定・`false`=unresolved／省略時 false 既定）を規定する dedicated behavior SPEC **SPEC-60** を FR-7 配下に新設して in-graph 被覆の空白を解消した。これにより、状態依存ルール SPEC（SPEC-18-1／SPEC-18-9／SPEC-59）が前提とする「resolved/unresolved の判定結果」が in-graph の dedicated SPEC で初めて規定され、config の `fnd_lifecycle.resolved_field` 定義と 1:1 で対応する台帳に組み込まれた。

処置成果の in-graph 代表である SPEC-60 から `→FND-105` のバックリファレンス辺を受ける（SPEC 側で付与）。forward 辺 `→SPEC-60`（充足 SPEC を指す処置対象辺・FND-103 が `→SPEC-18-1` を持つのと同型）は FND-101 辺逆転コホート（resolved FND の forward 削除・backward 付与）の一括是正対象として保持し、本ノード単体では削除しない（FND-102 baseline 慣行・`resolved: true` フィールドも付与しない＝machine 上は未 resolved 扱いだが dashboard 計数は resolved・prose で resolved とする）。
...
本処置で SPEC-60（FR-7 配下のトップレベル behavior SPEC）を新設し、`resolved` フィールドの解釈規約（`true`=resolved／未設定・`false`=unresolved／省略時 false 既定）を in-graph で規定した。これにより SPEC-18-1／SPEC-18-9／SPEC-59 が前提とする判定が SPEC-60 を上流に持つ形で根拠づけられ、config の `resolved_field` 定義との 1:1 トレーサビリティが成立した。
...
- **FND の `resolved` フィールドと DD/Q/PEND の非対称性**: SPEC-49 は DD/Q/PEND のライフサイクル状態が**ノード YAML に漏れていないこと**（本文バッジのみで保持）を検証する＝DD/Q/PEND は YAML に状態を持たない。一方 FND の `resolved` は **YAML フィールドとして持つ**＝両者は非対称である。この非対称（FND のみ機械判定フィールドを YAML に持つ）こそが、FND に固有の `resolved` 判定セマンティクス SPEC（SPEC-60）を必要とする理由であり、SPEC-60 はこの非対称性を in-graph で明示する位置づけを持つ。
...
- **① SPEC-60 で behavior SPEC 化（推奨・採択）**: `resolved` フィールドの解釈規約（`true`=resolved／未設定・`false`=unresolved／省略時 false 既定）を規定する dedicated behavior SPEC を FR-7 配下に新設し、config の `resolved_field` 定義と 1:1 対応させる。SPEC-18-1/18-9/59 が前提とする判定の根拠が in-graph で明示され、被覆が均一化する。FND-102/103 が進めた台帳化と整合する。SPEC が +1 増えるが、判定セマンティクスは fnd_lifecycle の中核であり dedicated 化の価値が高い。
- **② RULE-028 系で `resolved` フィールドの型検証も足す（①と排他ではなく別軸）**: SPEC-52-4（RULE-028）はフィールドスキーマ検証（共通必須フィールド＋型別必須拡張）を担うが、FND の任意フィールド `resolved` の**型検証（boolean 期待）**は対象外である。`resolved` に boolean 以外（文字列 `"true"` 等）が入った場合に検出する型検証を RULE-028 系に追加する案。ただしこれは**判定セマンティクスの規定（①）とは別軸の「フィールド型妥当性」論点**であり、①が解消する完結性ギャップ（セマンティクスの in-graph 不在）とは責務が異なる。

> **分析と推奨**: ① と ② は排他ではなく**別軸**である。本 FND-105 のギャップは「resolved 判定セマンティクスを in-graph で dedicated に規定する SPEC の不在」＝**①が直接充足する**。②（`resolved` の boolean 型検証を RULE-028 系へ追加）は「不正な型値が入った場合の検出」という別問題で、本 FND の完結性ギャップとは責務が異なるため、本 FND では**①を採択して SPEC-60 で充足**する。②は型検証被覆の論点として別途検討する価値があるが、本 FND のスコープ外（必要なら別 FND/Q として起票すべき独立論点）。SPEC-52-4 が `resolved` を型検証対象外としている事実は本 FND 本文で言及するに留め、その拡張要否はオーナー判断に委ねる（独断で「対応不要」「将来検討でよい」と結論づけない・PR7）。
>
> **決定: 案① 採用（SPEC-60 で behavior SPEC 化）**（オーナー決定・2026-06-22「FND と SPEC を両方起票」指示）。SPEC-60 を FR-7 配下に新設して `resolved` 判定セマンティクスを in-graph 規定し、本 FND を同セッションで resolved 化した。②（`resolved` の boolean 型検証）は別軸の独立論点として本 FND では扱わない。

**指摘時 ref_version**: SPEC-60 "0.1"（doc-system/02-what/03-spec.md の SPEC-60 ノードバッジ v0.1 時点・本セッションで新規著作）。

なお親 FR は **FR-7「検証・指摘の完結性検証」**（v0.2・fnd_lifecycle 系 SPEC-18-1/18-9/59 と同じく FR-7 を親とする）だが、FR-7 への言及は本文参照に留め、forward 辺は処置対象である SPEC-60 のみとした（FR-7 への辺は不要＝FND は処置対象＝充足 SPEC を指せば十分で、親 FR への辺は FND-103 が SPEC-18-1 のみを指したのと同型）。

被指摘対象の判定セマンティクス定義は `docs/doc-system/config.yaml` の `fnd_lifecycle.resolved_field`（DD-16・Q-4 から昇格・main commit 52093ed 由来でファイル version を持たないため参照キーを明記）に存在するが、config.yaml はノード化されない out-of-graph 資産のため forward 辺は張れない（FND-103 が config.yaml を本文参照に留めたのと同じ扱い）。forward 辺は充足 SPEC の `to: SPEC-60` のみとし、被指摘の所在（config の `resolved_field` 定義の in-graph 未被覆）は本文で明記する。

### config.yaml 規則変更チェック（FND-99 パターン）

本 FND は **`fnd_lifecycle.resolved_field` の判定セマンティクスを規定する in-graph SPEC の不在を指摘するもので、`must_link_to`/`must_be_linked_from`/`fnd_lifecycle` 等の config 接続規則の追加・変更・削除を含まない**（`resolved_field` 定義および省略時 false 既定の規約自体は main の Q-4→DD-16 で既にコミット済み・本 FND はその in-graph 被覆論点を起票するのみ）。処置（SPEC-60 の新設）も既存の config セマンティクスを in-graph で dedicated に規定する SPEC の追加に留まり、config.yaml の接続規則・`fnd_lifecycle` 定義は変更していない。

よって out-of-graph 著作資産（接続マトリクス `docs/doc-system/03-connection-matrix.md`・ドキュメント一覧 `docs/doc-system/01-document-items.md`・各 author エージェント／スキル）への規則伝播チェックは不要（RULE 台帳・接続マトリクスへの伝播対象なし）。SPEC-60 が config の `resolved_field` 定義を引く反映は SPEC-60 本文の入力/トリガで完了する（spec-author 所掌）。
