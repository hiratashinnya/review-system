...
<details><summary>⬡ FND-101 · v0.1.1</summary>
...
**改訂理由（z バンプ v0.1.0→v0.1.1・DD-16 辺逆転完了。辺逆転は downstream 無影響の provenance/lifecycle 操作のため z＝DD-8 §4「backref 追加＝z」準拠。当初 MINOR は誤りで Q-5/DD-21 により訂正）**:
DD-16（fnd_lifecycle 正式化・Q-4 選択肢A 採択）に伴い、本 FND が起票した「resolved FND の辺逆転未完了（double-edge）」を sprint-1 で一括是正し、本 FND 自身も resolved 化した。元 forward 辺（`→Q-4 "0.1"`）を削除し `edges: []`・`resolved: true` を付与。`→Q-4` は本 FND の是正方針が依存していた未決論点 Q-4 への provenance 辺であり、Q-4 は DD-16 へ昇格・closed 済みのため、provenance（FND→Q）として本文記録のみとし backref は張らない。指摘時 ref_version は本文に記録（DD-3）。

> **🔴 孤立エラーの意図的保持（FND-99 先例に倣う・恣意的抑制の禁止）**: 本 FND の処置対象は「辺逆転を一括是正した全 resolved FND（FND-1〜95 ほか）」というバッチであり、単一の in-graph 処置対象ノードを持たない（90 件超への backref 列挙は非現実的かつ各 resolved FND の処置対象は各自の対象ノードである）。よって `対象 → FND-101` のバックリファレンスを張る先が存在せず、`→Q-4` 削除後は incoming/outgoing をともに持たず**完全孤立**し、RULE-005（完全孤立・always_error・抑制不可）のエラーを発火する。これは欠陥ではなく、**「FND-101 は resolved だが、その処置（辺逆転の一括是正）はメタ作業でありバックリファレンス対象ノードを持たない」状態を正しく示す意図的なシグナル**である。`suppress` は付けず、エラー発火状態を意図的に保持する（FND-99 先例）。本是正の根拠は DD-16（fnd_lifecycle 正式化）であり、provenance として本文に記録する。
...
- 実処置対象が in-graph に存在する FND は対象ノード側に `対象 → FND-x` backward 辺を付与（reconciliation が他層ファイルへ反映）。実処置対象が out-of-graph（PR description・dashboard・.gitignore 等）で backref 対象が存在しない FND（FND-37/38/99/101 等）は、孤立エラーを意図的に保持（FND-99 先例・suppress なし）。
- 本 FND-101 自身も上記方針で resolved 化（孤立保持）。
