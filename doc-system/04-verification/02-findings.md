...
**対応状況**: resolved
**対応内容**: `docs/doc-system/04-notation.md` の summary バッジ説明箇所に「バッジは著作・最終更新時のファイル x.y スナップショットであり、ノード改訂カウントではない。現在のファイル x.y と一致しなくても RULE 違反にはならない」旨を追記。FR-1 に `→FND-32` バックリファレンス辺を付与（fr.md v0.2.6）。
