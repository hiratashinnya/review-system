...
## FND-96: MOD→P 強制がデータ/インフラ層モジュールに PR1 違反を生む

<details><summary>⬡ FND-96 · v0.1</summary>
...
**内容**:
`config.yaml` の必須辺 `{ node: MOD, target: P, activate_stage: design, severity: error, reason: "MODはプロセスを実装" }`（L44）は **「全モジュールはプロセスを実装する」と暗黙に仮定**している。しかし設計層には**プロセスを実装しないモジュール**が存在する。

- **MOD-1（domain）**：責務は「NodeRecord / EdgeRecord / ViolationRecord / ConfigSlice 等の値オブジェクトを定義する」＝**データ型の定義のみ**。プロセスを実装しない。にもかかわらず `MOD-1 → P-1`（ノード受付・パース）の辺を便宜的に張っており、本来は自分が定義する**データ D を realize すべき**ところを、発生源（プロセス P）の辺で表している。これは **PR1「もの＋発生源で分ける」違反**——データ（もの）をプロセス（発生源）の辺で表現する category error である。
...
追跡性は損なわれない：分析層に既に `D → P`（内部データは生成プロセスに依存・config L37）があるため、`MOD-1 → D → P` で設計→分析→プロセスの追跡経路は連続する（PR6 を満たす）。

**主たる指摘対象**: MOD-1（domain.py）

**同種の影響ノード（併記）**:
- **MOD-10（ports.py）**：FileSystemPort Protocol を定義（PORT-1 を実現・被参照あり）。プロセスでない。現状 `→P-1` は便宜的。
- **MOD-11（adapters/fs.py）**：FileSystemPort を実装するアダプタ。プロセスでない。現状 `→P-1`。
- **MOD-12（__main__.py）**：合成ルート＋CLI。ORC-1/E-1 を実現すべき結線役。現状 `→P-5` は恣意的。
...
**深刻度判定の根拠**:
機械 RULE（MOD→P＝RULE-006）は MOD-1/10/11/12 とも `→P` 辺を保持しており**現状は充足**しているため、検証ツール上の live な ERROR 違反は発生していない。一方で、これは「ルールが満たされているのに辺の意味が原則と食い違う」**設計層の構造的な原則違反（PR1 category error）**であり、放置すると設計→分析の追跡が誤った相手（P）を指し続ける。既存の構造的・原則違反 FND の判定（live RULE 失敗を伴わない原則違反は WARNING）に倣い **WARNING** と判定する。

**選択肢（オーナー判断）**:
- **選択肢A（推奨・最小かつ原則的）**：`config.yaml` の `must_link_to` を `MOD → [P, D]`（OR）へ拡張し、データ層モジュール（MOD-1）を D へ再ポイントする。インフラ系は別途扱う（MOD-10 は PORT-1 から被参照済み・MOD-11 はポート・MOD-12 は ORC/E 紐付け）。MOD 型は維持。category error を解消しつつ追跡性 D→P を温存でき、変更範囲は config.yaml ルールと MOD-1/10/11/12 の辺再ポイントに限定される。
- **選択肢B（PR1 最も忠実・大改修）**：型を分割する。ドメイン値オブジェクトは DM ノードへ（DM の必須辺も →P から →D/TERM へ要検討）、ports は PORT 型でカバー、合成ルートは ORC/E 紐付け。設計層の再採番を伴い影響大。
- **選択肢C（現状維持＋注記）**：`MOD→P` を維持し、データ/インフラモジュールは「主に関与するプロセスへ紐づける」と注記する。最安だが原則違反が残る。

**推奨**: **A**。category error を解消しつつ追跡性 D→P を温存でき、変更を config.yaml ルールと MOD-1/10/11/12 の辺再ポイントに限定できる。B は理想だが影響が大きく、C は違反据え置きとなる。
