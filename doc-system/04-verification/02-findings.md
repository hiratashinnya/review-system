...
**対応状況**: open
**対応内容（推奨）**: FR-1 配下に I-1 フォーマット完全スキーマ SPEC を追加（全フィールドの型・必須判定・値制約）。FR-14 配下（または新 FR）に O-2 出力フォーマット SPEC を追加。FR-11 配下に I-7 テンプレートフォーマット SPEC を追加。DD-5（NFR-SPEC 導出必須化）と合わせて対処を検討。
