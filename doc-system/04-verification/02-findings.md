...
<details><summary>⬡ FND-104 · v0.1.1</summary>
...
> **辺の扱い（resolved 段階）**: 本 FND は open 段階では `edges: []`（対象辺未確定）で起票しており、open 段階で付与した forward 辺は存在しない。案 A 承認・実施に伴い、指摘対象（FR-1・DD-8・DM-1）および z-bump 処置を施した SPEC-47 への forward 辺を**今回初めて付与**した。open 段階に forward 辺がなかったため辺逆転操作（FND→対象 を削除して 対象→FND に張替え）は不要（DD-16）。処置対象側のバックリファレンス辺は SPEC-47 に `→FND-104` を付与済み（コミット `7f55788`）。
...
- SPEC-47/47-1/47-2：x.y.z 形式に更新（z-bump）・SPEC-47 に →FND-104 backref
...
## FND-105: `_drift` が x.y.z フル比較で z バンプを誤ドリフト検出する（spec↔impl 乖離）

<details><summary>⬡ FND-105 · v0.1.2</summary>
...
FND-104（PR #38 処置中）がデータ側の x.y.z 化を完了させた一方、ドリフト判定ロジックが z-不問に未対応のため、z-bump 機構が実質まだ機能しない状態が残存する。
...
- `docidx/query.py`: `_xy()` ヘルパーを追加し x.y 部分のみ抽出して比較。docstring を x.y 比較に更新（FND-105・DD-8 §4 参照追加）。
...
- バックリファレンス付与先なし（指摘対象 `docidx/query.py` は out-of-graph）。仕様ノード SPEC-9 に `→FND-105` 辺を付与（仕様↔実装乖離のトレース）。

---

## FND-106: 03-spec.md 本文「例」記述 10 箇所に旧形式残存（バッジ 2 パート・ref_version 3 パート）

<details><summary>⬡ FND-106 · v0.1.0</summary>
...
**指摘対象**: `doc-system/02-what/03-spec.md` の SPEC-1-1・SPEC-26-1・SPEC-38-1・SPEC-40-2・SPEC-48-1・SPEC-48-2・SPEC-52-1〜52-4 の `**例**:` 記述（10 箇所）

**指摘時 ref_version**: 全10ノード v0.1.0 時点（ref_version: "0.1"）

**内容**:

PR #38 再レビュー（2026-06-24・Claude Code による自動点検）で指摘。`03-spec.md` の SPEC 本文「**例**:」記述 10 箇所に旧形式が残存し、確定方針①②と不整合。機械挙動・実 YAML edges への影響はなく WARNING。
...
なお `04-verification/02-findings.md` 内の :269/:804/:3093 は過去の系での凍結記録・FND-105 バグ再現手順（PR8 履歴保全）であり遡及修正不要。

### 処置記録（2026-06-24）

コミット（本 PR #38 内）。

- `doc-system/02-what/03-spec.md` 10 箇所の `**例**:` 記述を修正（バッジ x.y.z・ref_version x.y に統一）
- 処置対象 SPEC 10 ノード（SPEC-1-1・SPEC-26-1・SPEC-38-1・SPEC-40-2・SPEC-48-1・SPEC-48-2・SPEC-52-1〜52-4）に `→FND-106` バックリファレンス辺を付与・バッジ z バンプ（v0.1.0→v0.1.1）
