...
**対応状況**: resolved
**対応内容**: SPEC-41（FR-1 配下・I-1 フォーマット完全フィールドスキーマ）・SPEC-42（FR-3 配下・O-2 カバレッジテーブル出力フォーマット）・SPEC-43（FR-11 配下・I-7 テンプレートファイル必須構造）を著作し spec.md v0.3.2 に反映した（2026-06-13）。I-1 と FR-1 に `→FND-18` バックリファレンス辺を付与済み。なお O-2 出力フォーマット SPEC は元推奨の FR-14 ではなく FR-3（カバレッジ点検）配下に著作した（FR-14 は伝搬編集支援であり FR-3 が正当な親）。
