...
**対応状況**: open（初回処置を reconciliation 差し戻し・2026-06-13）

**差し戻し経緯（2026-06-13）**: 初回処置で SPEC-41（I-1 完全スキーマ）・SPEC-42（O-2 カバレッジ出力）・SPEC-43（I-7 テンプレート構造）を著作・spec.md v0.3.2 に反映したが、オーナーレビューで**テスタブルな粒度に達していない**として差し戻し。spec.md v0.3.3 で 3 SPEC を撤去・I-1/FR-1 の `→FND-18` backref も除去した。具体的な粒度違反:
- **SPEC-41**: 「必須フィールド存在」「`labels` は配列」「`scheduled` は文字列」「`edges` は配列」「未知キー→WARNING」を 1 ノードに 5 アサーション以上束ねており「1 アサーション 1 SPEC」違反。さらに normal（検証通過）と未知キー WARNING（failure 相当）を混在。
- **SPEC-43**: 「id プレースホルダ」「type」「edges」「型別必須フィールド（SPEC/TD は condition・TR は result/log_ref）」を束ね、normal と error を混在。
- **SPEC-42**: ヘッダ行・行フォーマット・ソート・gap セクションを束ね、gap 部分は既存 SPEC-29/30（FR-3 配下のグラフ網羅性点検）との重複の疑い。

**対応内容（再著作プラン・推奨）**: 各フォーマット定義を「1 アサーション 1 SPEC・condition 単位で分割」で再起票する。
- **I-1 完全スキーマ**: (a) normal＝必須5フィールドが揃い型が正しいノードは検証通過、(b) failure＝必須フィールド欠如を ERROR 検出、(c) failure＝フィールド型不正（例 `labels` が文字列）を ERROR 検出、(d) failure＝未知フィールドを WARNING 検出、を別 SPEC（-N 枝番可）に分割。各 SPEC は単一の期待結果・終了コード・出力フォーマット・具体例を持つ。
- **O-2 出力フォーマット**: カバレッジテーブルの行フォーマット（normal・列順・ソートキー）を 1 SPEC に限定。gap セクションは既存 SPEC-29/30 との重複を点検し、重複なら本 SPEC を起票せず SPEC-29/30 を出力フォーマット明記で補強。
- **I-7 テンプレート構造**: (a) normal＝必須プレースホルダ充足で通過、(b) failure＝必須構造欠如を検出、を分割。型別必須フィールド（condition/result/log_ref）はさらに型ごとに別 SPEC 化を検討。

**未決点（オーナー判断）**: SPEC-42（O-2 gap セクション）と既存 SPEC-29/30 の重複可否。重複なら新規起票せず既存補強に倒す。
