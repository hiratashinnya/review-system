...
FND 解消時に edges が逆転（FND→対象 → 対象→FND）し、辺情報から指摘時の版が失われるため本文に明記する（DD-3 制度化）。

- **MOD-1 "0.2"**（01-modules.md・MOD-1 バッジ v0.2 時点。realize 辺が D-4/D-6/D-9〜D-21 で D-5/D-7 を欠く状態）
- 補足対象の指摘時バッジ（subject 辺は MOD-1 のみだが、被覆漏れの相手側の版も provenance として記録）:
...
> **RULE-004（凍結 ref_version）の扱い**: 本 FND-100 の forward 辺 `→MOD-1` は**指摘時バッジ "0.2" を凍結記録**として保持する（DD-3・指摘時版の provenance）。MOD-1 は本処置で v0.3 にバンプするが、凍結値は更新しない。これは既存の **FND-96 → MOD-1（"0.1" 凍結・MOD-1 は v0.2 以降）と同型の扱い**である。FND-96 は `suppress` に RULE-004 を入れておらず（`suppress: []`）、config.yaml にも RULE-004 例外を持たない——指摘時版の凍結を resolved 状態・本文 provenance（DD-3）で正当化する運用に倣い、FND-100 も `suppress: []` のままとする（FND-96 と同一扱い）。なお凍結 ref_version は RULE-004 ドリフト候補として残る点も FND-96 と同型である。
...
4. **MOD-1 v0.2→v0.3**: realize 辺に **D-5・D-7 を追加**（各 D 現バッジ "0.1"）。D-17〜D-21 は既存辺あり。`→FND-100` バックリファレンス付与（これが FND-100 の RULE-006 解消辺＝FND→MOD-1 の逆転先）。
