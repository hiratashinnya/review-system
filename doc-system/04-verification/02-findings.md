...
<details><summary>⬡ FND-103 · v0.1</summary>
...
<details><summary>⬡ FND-104 · v0.1</summary>
...
しかし `docs/doc-system/05-verification.md` の RULE 一覧には、この「禁止接続/辺残留の存在」を検出・報告する RULE 番号が存在しない。
...
| （該当 RULE なし） | config 駆動・任意型・辺残留 | — | **空白** |

つまり RULE-006 が「欠如」、RULE-001/022 が「残存（ノード型固有）」を担い、**config 駆動の汎用「辺残存（禁止接続の存在）」を検出する RULE が構造的に空白**になっている。`fnd_lifecycle.resolved.must_not_link_to` はまさにこの空白に落ちるルールである。

影響: 検証ツール（spec-inspector・実装層 0 ノード・未実装）を実装する際、`must_not_link_to` 違反をどの RULE コードで報告するか未定義。FND-103 ②案で新設予定の dedicated SPEC（resolved 系）の期待動作も、参照すべき RULE 番号を引けない（暫定で RULE-006 と記載されたが RULE-006 は「欠如」で意味が逆）。

### 位置づけ（スコープ境界・FND-103 との層分離）

- これは FND-103（dedicated **SPEC** 被覆の不均一＝coverage 論点）とは**別層の RULE（検出機構）定義の不在**である。PR1「もの＋発生源で分ける」に従い、SPEC 被覆論点（FND-103）とは別ノードとして起票する。
...
- 検出機構（RULE）が定義されて初めて、それを参照する dedicated SPEC（FND-103 ②案）が RULE 番号を引ける。本 FND-104 の解消は FND-103 ②案の前提に位置づく。
