...
1. **バッジのノードバージョン正式化**: `⬡ PREFIX-N · vX.Y.Z` の `X.Y.Z` はノード固有バージョン（MAJOR.MINOR.PATCH）として正式化する。現在のバッジ値はすべてそのまま有効（FND-104 処置で x.y → x.y.0 移行済み）。
...
- **FND-104 是正（2026-06-23）**: 本 DD §1 が「MAJOR.MINOR（2 部）」と誤記し x.y.z バッジの z-bump 機構を機能不能にしていた。FND-104 処置（Case A）により全511ノードのバッジを x.y → x.y.0 に移行し、§1 記述を「MAJOR.MINOR.PATCH（3 部）」に訂正（FND-104 resolved）。
