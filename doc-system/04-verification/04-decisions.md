...
## DD-22: doc-system 設計層モデリングの skill 展開方針を確定（Q-6 から昇格・①-C ハイブリッド／②-A 新要件軸で PROMPT 型流用）

**status: decided**（2026-07-01 オーナー確定・Q-6 からの昇格・実施 sprint-1）

<details><summary>⬡ DD-22 · v0.1.0</summary>
...
> **辺の扱い**: 本 DD は decided。被参照は昇格元 Q-6 の `→DD-22`（昇格辺）で確保され RULE-005（完全孤立）は生じない。先例（Q-3→DD-20・Q-5→DD-21・Q-4→DD-16・DD-15〜DD-21 はいずれも `edges: []`）に倣い本 DD は `edges: []` とし、義務辺 DD→X を張らない。理由：(1) 本決定の in-graph 反映先である**新 FR＋傘 SPEC・各 skill 用 PROMPT ノードはいずれも後続著作で新設される未著作ノード**であり、存在しないノードへ義務辺は張れない（RULE-007）。(2) 参照した既存ノード（PROMPT-1〜7・SPEC-27・FR-13・NFR-3・SPEC-46・ORC-1/ORC-2）は本決定で**不変**のため、義務辺 DD→X（「未反映」の RULE-001 ERROR シグナル）を張る対象がない。(3) 昇格元 Q-6 が処置側から `→DD-22`（昇格辺）を張り返すことで本 DD の被参照辺が確保される（Q-3→DD-20・Q-5→DD-21 と同型）。義務辺を一旦張って即削除する二度手間を避け、最新の昇格 DD（DD-15〜DD-21）の慣行に揃える。
...
（辺の扱いは上記「辺の扱い」注記のとおり `edges: []`。上記は義務辺ではなく本文で追跡する provenance 参照であり、本決定で不変・または後続著作で新設される新ノードへの反映を待つため辺を張らない。）
...
in-graph（後続著作で新設）:
- **新 FR 1 件＋傘 SPEC 1 件**（skill＝LLM プロンプト設計資産の要件軸）を後続著作（requirements-author → spec-author → reconciliation）。SPEC-27→FR-13 の著作エージェント軸とは別軸として区別する。
- **対象 skill ごとに PROMPT 設計ノードを後続著作**（13 件・PROMPT-8 以降で採番。design-author → reconciliation）。各 PROMPT は上記新傘 SPEC を親に持つ（PROMPT→SPEC 既存規則）。
