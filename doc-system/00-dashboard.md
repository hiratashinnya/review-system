...
| FND-96/97/98/99/100 の lifecycle 版バンプ是正（DD-21 残課題）| 🔵 低 | FND | A-1 以前に lifecycle 辺逆転で MINOR バンプ済み（main 既存ドリフトの一部・DD-21 原則違反）。pre-existing かつ改訂履歴混在のため別途オーナー判断（是正 or 据置／FND 起票要否）。`scheduled` 未設定 |
