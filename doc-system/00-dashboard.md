...
| 作業 | 種別 | 状態 |
|---|---|---|
| PR #37 レビュー対応 — スレッド1（SPEC-59 分類）＋スレッド3（検証戦略トリガ整合） | FND-106 / SPEC-59 分類修正 | 🔄 一部 open（2026-06-23）。**スレッド1**＝SPEC-59 の親辺は FR-7 で確定（オーナー決定「fr-7でよい・新規 FND 不要」）。ダッシュボードの「SPEC-12/55 系統の独立 SPEC」表現を「FR-7 配下の独立 SPEC・辺残留検出は SPEC-12/55〔FR-5〕と同系の機構だが親辺は完結性検証 FR-7」へ2箇所修正（03-spec.md の `to: FR-7` は変更せず据置）。**スレッド3**＝**FND-106 起票**（open・WARNING・検証戦略の段階別トリガがフロー図の単一トリガと不整合・bump 非依存の辺残留 RULE すり抜けリスク・推奨案A・トリガ統一は独断実施せずオーナー判断待ち）。旧スレッド2 は本スレッド3 に差し替え済み |
...
| 横断スパイン | DD / Q / PEND | 24 | ✅ | DD-1〜18（DD-16: FND 専用ライフサイクル／DD-17: RULE-030 新設・案B・FND-104／DD-18: RULE-031 新設・案B・FND-105 型検証）・Q-1/Q-4 closed・Q-2/Q-3 open・PEND-1 resolved・PEND-2 deferred |
...
| 項目 | 優先 | 種別 | 次アクション |
|---|---|---|---|
| FND-106：検証戦略の段階別トリガがフロー図の単一トリガと不整合（bump 非依存の辺残留 RULE が bump トリガに括られ検出すり抜けリスク） | 🟡 中 | FND | 選択肢 A（段階①〜③を単一トリガに統一・reviewer 推奨）／B（bump 非依存 RULE を段階②へ移す/辺改変トリガ併記）／C（現状維持＋根拠明記）。推奨 A。実装＝トリガ統一は独断で行わない・`scheduled` 未設定 |
...
| FND-106 | WARNING | ⏳ open | 検証戦略 05-verification.md の段階別トリガ宣言がフロー図 L13-23 の単一トリガと不整合。bump 非依存の辺残留 RULE（RULE-030/001/002/022）が段階① bump トリガに括られ、bump を伴わない辺改変で検出すり抜けリスク。PR #37 起票・推奨案A（段階①〜③を単一トリガに統一）。オーナー判断待ち・`scheduled` 未設定 |
