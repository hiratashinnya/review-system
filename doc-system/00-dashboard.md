...
| 作業 | 種別 | 状態 |
|---|---|---|
| resolved 状態判定の dedicated SPEC 化 — `resolved` フィールド判定セマンティクスの in-graph 被覆ギャップ起票＋充足 | FND-105 / SPEC-60 | ✅ resolved（2026-06-22）。`fnd_lifecycle.resolved_field`（out-of-graph config）の判定セマンティクス（`true`=resolved／未設定・`false`=unresolved／省略時 false 既定）を規定する in-graph dedicated SPEC が不在だった完結性ギャップを **FND-105**（INFO）で起票。オーナー決定「FND と SPEC 両方起票」に従い、FR-7 配下に **SPEC-60 傘＋SPEC-60-1（true→resolved）/60-2（false・未設定→unresolved）** を新設して充足。SPEC-60→FND-105 backref・FND-105→SPEC-60 forward 保持（FND-102 baseline・`resolved: true` 未付与で FND-101 辺逆転コホート帰属） |
...
| 横断スパイン | DD / Q / PEND | 23 | ✅ | DD-1〜17（DD-16: FND 専用ライフサイクル／DD-17: RULE-030 新設・案B・FND-104）・Q-1/Q-4 closed・Q-2/Q-3 open・PEND-1 resolved・PEND-2 deferred |
...
- **RULE 定義**: [`docs/doc-system/05-verification.md`](../../../docs/doc-system/05-verification.md) — RULE-001〜030 の完全定義
