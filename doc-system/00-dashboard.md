...
>
> **最終更新**: 2026-06-14（PR #22 レビュー処置：FND-34〜37 起票・FND-34 resolved（VERIFY-5 SR-7→SR-2 訂正）・FND-35/37 は sprint-2・**FND-36 はオーナー判断待ち**（バッジ定義 A/B/C）｜旧: FND-24〜33 全件 resolved（H1/H2/H3 + 残件）｜ **current_stage**: `requirements`（`docs/doc-system/config.yaml`）
...
| PR #22 レビュー処置（FND-34〜37 起票・FND-34 resolved）| FND | 🔄 FND-36 オーナー判断待ち・FND-35/37 sprint-2 |
...
| FND-36（ノードバッジの意味＝ノード改訂回数 vs ファイル x.y） | 🟡 中 | FND（WARNING・open） | ⬜ 判断待ち | バッジ定義 A/B/C をオーナーが選択（推奨 A＝ノード固有リビジョン） |
| PEND-1（I-1-1/I-1-2/I-1-3 過分割） | — | PEND（**resolved**） | 子ノード改名＋親辺付与で解消・FND-6 resolved | ✅ クローズ（N4） |

### FND サマリ（計 37 件：✅ resolved 34 ／ ⏳ open 3）
...
| FND-32 | L2（INFO） | ✅ resolved | FR-1 バッジとファイル x.y の不一致 → notation.md バッジ説明追記（※FND-36 で再指摘） |
...
| FND-35 | WARNING | ⏳ open（sprint-2） | config `SPEC→SPEC` OR ループホール → 次スプリント（推奨 ②＋③） |
| FND-36 | WARNING | ⏳ open（**判断待ち**） | バッジ定義が実態と矛盾（FND-32 誤定義）→ A/B/C をオーナー判断 |
| FND-37 | INFO | ⏳ open（sprint-2） | tmp コミット運用が未継続（verify5.md コミット）→ 次スプリント |

> FND-1〜34 は resolved。open 3 件＝FND-35（sprint-2）・FND-36（オーナー判断待ち）・FND-37（sprint-2）。明細は `02-findings.md`。
