...
>
> **最終更新**: 2026-06-14（**DD-8 即時実施完了**：ノードバージョニング正式化・ファイルフロントマター全廃・ref_version をノードバッジ基準に一括移行（live 辺 170 件・ドリフト 0 検証）・RULE-004/meta-schema/notation/config をノード x.y 基準に更新｜FND-36 resolved｜旧: FND-34 resolved（VERIFY-5 SR-7→SR-2 訂正））｜ **current_stage**: `requirements`（`docs/doc-system/config.yaml`）
...
| PR #22 レビュー処置（FND-34〜37 起票・FND-34/36 resolved）| FND | ✅ 完了（FND-36 resolved・DD-8 confirmed）・FND-35/37 は要オーナー確認（下記） |
