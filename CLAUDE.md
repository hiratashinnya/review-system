...
- スキル（共有リファレンス・非対話）：`silent-failure-diagnosis`（`user-invocable: false`＝単独起動しない。**外部プロセス（委譲先 CLI/MCP・フック・CI）がエラーを出さずに誤答・無作動している疑いの切り分け規律**＝A21。委譲系エージェントが `skills:` でプリロードする。`spec-principles` と同じ共有リファレンス区分。ツール固有の既知事象は `/agy-delegate`・`/codex-review` が正本で、こちらは一般則のみ）
