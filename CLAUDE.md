...
- **委譲時のインプットは最小化**：分析そのものはサブエージェントに任せ、**主文脈からは関連ノードの ID 以外に過剰なインプットを渡さない**（主文脈で先回りして分析・本文を作り込まない）。※これは委譲（author/分析）への入力規律。判断を仰ぐ FND/Q の**本文**は別物で、そちらは「ID だけで投げず本文で説明してから判断を仰ぐ」（オーナー向け説明）を維持する。
