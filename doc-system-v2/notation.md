...
> doc-system v2 の著作記法。**1ノード1YAMLを正本とし、Markdown 本文は型別 body policy に従う**。
...
## 1. 1ノード = 1 YAML、本文は型別 body policy

各ノードは `{slug}.yaml` を正本にする。**属性・バッジ・YAML を本文に混ぜない**。

```
nodes/<stage>/<type>/[<status>/]{slug}.yaml    # サイドカー（属性・辺・正本）
nodes/<stage>/<type>/[<status>/]{slug}.md      # body_policy=required の同名本文
nodes/<stage>/<type>/[<status>/]shared.md      # body_policy=shared から参照される共有本文例
```
...
- `config.yml: body_policy`
  - `required`: 同名 `{slug}.md` が必須。
  - `shared`: `body_ref.file`（任意で `body_ref.anchor`）で共有本文を指す。同名 `{slug}.md` も互換として許可。
...
body_ref.file: "nodes/04-verification/td/test_program.md"  # shared 型のみ。共有本文ファイル。
body_ref.anchor: "scenario-a"                              # shared 型のみ。共有本文内の節。
```
...
- [`validate.py`](validate.py)（stdlib のみ）が schema（必須キー/型/enum/pattern・未知キー禁止）＋ path（stage/type/status 既知集合）＋ body policy（required/shared/none）＋ id 一貫性（stem == slugify(title)）を検査する。
...
|---|---|
| 1 ファイルに複数ノード（`##` 見出し＋`---` 区切り） | **1 ノード 1 YAML**。本文は型別 body policy。区切り `---` は廃止 |
