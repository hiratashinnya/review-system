**status: decided**（2026-07-21・DD-10 へ昇格）

**決着**: 本 Q の推奨①（`src → [mod, dm, port, orc]` へ拡張）を **DD-10「SRC シンボル適格性で src⇄設計リンクの誤充足を機械排除・src→mod 拡張」でオーナー確定**した。config.yml は反映済み（`must_link_to: src→[mod,dm,port,orc]`＋`src_symbol_eligibility` マップ）。したがって本 Q は **decided**（→DD-10 参照辺を付与）。DD-10 は拡張①に加え、設計種別×`source.kind` 適格性による誤充足の機械排除、および class 厳密化（PEND-a）・非 Python 担体の意味判定（PEND-b）の切り出しまで含めて確定している。
...
**依存**: 本 Q の決定は #160（SRC 著作）の前提。**決定前に SRC を著作しない**（張り先規則が未確定のまま SRC を著作すると再著作が発生する）。→ DD-10 決着により SRC 著作の前提が確定。
