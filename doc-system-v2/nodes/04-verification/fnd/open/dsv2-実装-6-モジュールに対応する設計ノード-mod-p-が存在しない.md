...
1. **設計外実装が追跡外のまま残る** — 稼働中の 6 モジュール（うち `dsv2 reverse` は CLAUDE.md 規約上の必須ツール）が in-graph に根拠を持たないため、仕様変更・責務変更が起きても影響範囲の追跡が効かない。トレーサビリティ（本システムの目的そのもの）が当該 6 本について成立していない。
2. **`#160` の整合作業で取りこぼされる** — SRC の materialize は MOD/P を起点に列挙されるため、起点ノードが無い 6 本は作業対象の母集合に入らない。乖離解消を目的とした作業自体が乖離を温存する。
3. **`current_stage: implementation` 到達時にブロッカー化する** — `mod←src` 系の必須辺が発火した時点で、6 本は「SRC を張る先の MOD が無い」状態となり、実装段への移行そのものが止まる。是正コストは移行直前に集中する。
