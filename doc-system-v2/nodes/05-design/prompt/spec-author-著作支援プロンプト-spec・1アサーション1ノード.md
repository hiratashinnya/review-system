...
**注意事項**: 親→子の辺は持たない（親子は子→親の同型依存辺で表す）。辺は無名依存辺（kind/status なし・to は単数・ref_version 必須）。`scheduled: ""` はオーナー承認済みの後送り/未計画のみ。全子に `condition` 属性必須。`suppress` / `suppress_reason` は issue #118 で廃止済みのため出力しない。SPEC←TD 被依存辺は verification 発火のため SPEC 著作時点では追加しない。
