...
**注意事項**: 辺方向は依存方向（O→P・P→E・O/E→ACTOR）。プロセス間中間データは D 型で必ず起票（O→ACTOR を持たない・DD-7）し価値経路（PR6）を連続させる。辺は無名依存辺（kind/status なし・to は単数・ref_version 必須）。E は刺激元 `→ACTOR` 必須（DD-020）。`scheduled: "sprint-1"` など現行スプリントを明示。
