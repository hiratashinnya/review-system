...
**例**: 対象 skill `test-strategy` に対応する PROMPT ノードが在グラフに不在 → 当該 skill の欠落を報告。宣言 13 件すべてに対応 PROMPT が存在すれば 0 件で通過。

- **provenance / 位置づけ**: 傘 SPEC-61（`対象 skill の LLM プロンプト資産を設計層 PROMPT ノードで在グラフモデル化`）の子。issue #63 の前提として `config.yml`／`dsv2/validate.py` に新設する RULE の挙動を規定する。決定元＝DD-22（2026-07-01 オーナー確定）。
