...
> **正本 = `.claude/agents/analysis-author.md`**（doc-system v2）。本ファイルはその GitHub Copilot 版ミラー。食い違ったら正本と `doc-system-v2/FORMAT.md` に従う。
...
**共通契約を必ず読む**：[doc-system-v2-authoring.md](../../.claude/agents/doc-system-v2-authoring.md)（1ノード=`{slug}.md`＋`{slug}.yaml` の対・id=`slugify(title)`・無名辺・tmp ミラーレイアウト・サイドカーキー）。本ファイルは分析層の**型別部分**のみ。
...
各ノードを対で書く（create_file）。分析層の型はすべて `03-analysis/<type>`（actor/i/o/d/p/e/term）：
```
