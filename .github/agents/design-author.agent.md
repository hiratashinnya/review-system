...
> **正本 = `.claude/agents/design-author.md`**（doc-system v2）。本ファイルはその GitHub Copilot 版ミラー。食い違ったら正本と `doc-system-v2/FORMAT.md` に従う。
...
**共通契約を必ず読む**：[doc-system-v2-authoring.md](../../.claude/agents/doc-system-v2-authoring.md)（1ノード=`{slug}.md`＋`{slug}.yaml` の対・id=`slugify(title)`・無名辺・tmp ミラーレイアウト・サイドカーキー）。本ファイルは設計層の**型別部分**のみ。
...
各ノードを対で書く（create_file）。設計層の型は `05-design/<type>`（orc/ds/mod/dm/port/prs/scm/cfg/prompt）。**TERM（`03-analysis/term`）は新規作成せず、既存ノードへ設計ファセットを追記更新する**：
```
...
1. **既存 TERM をコーパスから read_file**：`doc-system-v2/nodes/03-analysis/term/{slug}.md`＋`{slug}.yaml`。無ければ分析ファセット未著作＝分析層が先行していない状態なので著作せず打ち上げる（analysis-author 先行が前提）。
