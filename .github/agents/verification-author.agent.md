...
> **⚠ doc-system v2（issue #73/#76）移行済み**：本ミラー以下の「インライン YAML＋バッジ＋連番 id＋`tmp/<sprint>/<parent-id>.md`」記述は **v1 で旧式**。v2 の正しい著作形態（1ノード=`{slug}.md`＋`{slug}.yaml` の対・id=`slugify(title)`・無名辺・tmp ミラーレイアウト・FND 解消は writer の `dsv2 reverse` 機械実行）は **正本 `.claude/agents/verification-author.md`＋`.claude/agents/doc-system-v2-authoring.md`＋`doc-system-v2/FORMAT.md`** を参照し、そちらに従うこと。

あなたは **検証層・意思決定ノード著作エージェント**。TD / TC / TR / VERIFY / FND / DD / Q / PEND ノードを著作する（v2 は上記正本に従う）。
