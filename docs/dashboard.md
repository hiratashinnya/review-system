...
最終更新：2026-07-28（**Codex 第二意見レビュー(PR #258)の指摘を反映**：完了宣言の自己矛盾を解消し A3/A5/A6 を「MVP 未完」に、Q25/A8 の状態併存を一本化、**Q26 を起票**（S5 lint 未結線・07 の契約検証不足で A1/A2 の確定宣言を後退）、A9 の「新規設計は不要」を撤回。2026-07-27＝Q25/A9 起票・#6 の MVP 印付与）

> ⚠️ **2026-07-27 追記**：下記「実装進捗 13/13 ✅」は**層/モジュール単位の進捗**であり、**MVP 機能単位では F7（✋/💬 人手適用・P2 差別化）が未実装**。したがって「MVP(P1+P2)実装が完了」は現時点で正確でない → **[Q25](#未決事項決めないと進めない論点)**（方針は**決定＝A でオーナー確定済み**。実装が未着手＝[A9](#-ネクストアクション次にやる候補)）。
...
| # | アクション | 目的 | 状態 |
|---|---|---|---|
| A1 | 評価基準ファイルのスキーマを実際に書く | サンプルで曖昧さを潰す | 🟡 **記法は確定／S5 lint 未結線**（[Q26](#未決事項決めないと進めない論点)・2026-07-28 後退）：実装済み・運用中（[schema/](schema/README.md)＝`parsing/frontmatter.py`+`parsing/lint.py`+`persistence/criteria_repo.py` が準拠しテスト済）に加え、**schema/README.md の見出しを「v0 / 叩き台」→「確定・MVP 実装準拠」へ更新済み** |
| A2 | AI への入力設計を決める | 基準＋文書をどう渡すか / 長い文書・コードベースの扱い | 🟡 **契約は確定／記述と契約テストに不足**（[Q26](#未決事項決めないと進めない論点)・2026-07-28 後退）：実装済み・運用中（[07](requirements/07-ai-input-design.md)＝`core/evaluate.py`+`ports/platform.py`+`prompts/registry.py` が準拠）に加え、**07 冒頭に確定宣言を追記済み** |
| A3 | MVP の線引き | **確定：MVP＝P1＋P2**（[12-mvp-scope](requirements/12-mvp-scope.md)）。方向性：決定的ツール群（MCP）＋プロンプト雛形を作り、Claude が LLM 役で回す（[11](requirements/11-platform-adapter.md)） | 🟡 線引きは確定・**MVP は未完**（全13層は実装済みだが **F7 未実装**＝[Q25](#未決事項決めないと進めない論点)） |
...
| A5 | **実装前の凍結セット（8項目）を固める** | A モジュール構成・②外部IF・B PF駆動プロトコル・C 永続層・①オーケストレーション(スイムレーン)・③システムプロンプト・D ログ/版・④テスト戦略。索引＝[design/README](design/README.md)。判断ログ＝[design/decisions](design/decisions.md) | ✅ 8項目確定（[design/README](design/README.md) 全項目 ✅・DD1–DD23）→ **全13層の実装完了**（下表）。ただし**機能単位では F7 未実装＝MVP 未完**（[Q25](#未決事項決めないと進めない論点)） |
| A6 | **post-MVP スコープの着手判断** | **前提注意：MVP はまだ閉じていない**（F7 未実装＝[Q25](#未決事項決めないと進めない論点)／実装タスク＝A9）。MVP を閉じた後に、次の優先度をオーナーが決める：F10 参照突き合わせ／F11 合成時警告／F12+F13 育成ループ／F14 ひな形生成／F15 AI型推定／F16 異常系degrade／F17 team/project scope（いずれも [12-mvp-scope](requirements/12-mvp-scope.md) で P3/将来）。現状どれも未着手 | 🟡 未着手・オーナー判断待ち |
...
| **Q25** | **F7（✋/💬 人手適用）未実装 ↔「MVP(P1+P2)実装完了」宣言の矛盾** | **矛盾**：[12-mvp-scope](requirements/12-mvp-scope.md) は **MVP＝P1＋P2** と定め、F7「✋/💬 人手適用＝**承認/決定した修正がコードに反映される**」を P2 差別化の構成要素として明記（同 :26, :96「F7（承認付き適用＝低リスクで適用機構を作る）→ F6＋F8」）。設計も [03](design/03-external-interfaces.md) で `reviewer approve <report>` / `reviewer decide <report>` と `cmd_approve`/`cmd_decide` を定義済み。**しかし実装に両コマンドが無く**（`io/cli.py` は `version`/`review`/`revert`/`feedback` のみ）、`core/apply.py` は `apply_auto`（🤖 auto バケツ専用）だけで**✋/💬 を適用する経路が存在しない**（`feedback` は `feedback.jsonl` への記録のみ）。よって本ダッシュボードの「MVP(P1+P2)実装が完了」は**機能単位では成立していない**。<br>**選択肢**：**(A) F7 を実装して MVP を閉じる**（`approve`/`decide` を追加し、既存の適用機構＝`workspace_git`＋finding 単位コミット＋revert を ✋/💬 バケツへ再利用。F6/F8 が完成済みなので増分は小さい）／(B) F7 を post-MVP へ降格し、12-mvp-scope・03・DD10 に MVP 印を入れて実装と整合させる（PR8 フル論理＋MVP印）／(C) 現状維持（記録のみ）。<br>**推奨 = A**。根拠：MVP の価値主張は「直してくれて、**いつでも戻せる**」＋「✋/💬 に仕分けたものが**反映される**」で、F7 を落とすと仕分け 4 区分のうち人手側 2 区分が**出口を持たない**（PR6「価値経路を遮断しない」に反する）。また適用機構（DS3 内部 git・finding 単位コミット・revert）は F6/F8 で既に完成しており、F7 は**バケツの入れ替えと承認入力の解決**が主で新規機構が不要。**(B) は価値経路の切断を仕様側で追認する**ことになるため非推奨。<br>**波及**：A8 は本 Q へ移送。[#7](https://github.com/hiratashinnya/review-system/issues/7)（HTML フィードバック UI の決定肢＝`modify`/修正欄・✋/💬 の UI 分離）が**前提論点として先行**する（`decide` の入力は 💬 の「決定＋任意の修正欄」で、現状 UI にその欄が無い）。 | 🟢 **決定＝A（F7 を実装して MVP を閉じる）**（オーナー確定 2026-07-27）。実装は未着手＝[A9](#-ネクストアクション次にやる候補) で追跡。前提＝#7 の設計決定 |
| **Q26** | **「確定宣言」と実装の乖離（S5 lint 未結線・07 の契約検証不足）** | **経緯**：A1/A2 を「確定・MVP 実装準拠」と宣言した（2026-07-27）が、**2026-07-28 の Codex 第二意見レビューで宣言が過大と判明**し、実地検証で裏付けた。<br>**事実①（S5 未結線）**：`review_system/parsing/lint.py` の `lint_criteria` は**本番経路のどこからも呼ばれていない**（grep 済み）。[schema](schema/README.md) が「書いたら実行前 lint で停止」と規定する**ネスト段数上限・`extends` 先の存在・`override` の値域**は、実行時に強制されていない。S5 は「パーサが検証器を兼ねる」(Q5a) 前提だったが、**パーサが見るのは文法だけ**で意味検査は lint 側にある。<br>**事実②（07 の契約検証不足）**：[07](requirements/07-ai-input-design.md) の JSON 例と実装の型（`SuggestedFix{description, diff}`）の表記が一致していない。e2e は `FakePlatformAdapter` で決定化しており**実 JSON を通した契約検証にはなっていない**。<br>**選択肢**：**(A) lint を本番経路（`criteria_repo` のロード時）に結線し、07 の記述を実装の型へ揃え、実 JSON の契約テストを1本足す**（S5 が「MVP 推奨」である以上、宣言を実態に合わせるより機構を通す方が筋）／(B) 宣言の範囲を「記法とパース経路のみ確定」に限定して据え置き、結線は post-MVP（**現状の記述はこの形に更新済み**）／(C) 現状維持。<br>**推奨 = A**。根拠：S5 は「E14 を実行時でなく**作成時**に倒す」ための策で、結線しなければ目的を果たさない。異常系方針（Q17＝疑わしきは fail-close）とも、未検査のまま流すのは整合しない。ただし**製品コードの変更を伴う**ため、実施時期はオーナー判断。<br>**波及**：A1/A2 の「完了」表記は「記法・契約は確定／機構は未結線」に後退させた。 | 🔴 **未決・オーナー判断待ち**（2026-07-28 起票） |
