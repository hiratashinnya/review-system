...
最終更新：2026-06-04（LLM⇄システム境界10を追加・Q20確定(2段構え)/Q21起票/Q3確定）
...
| Q21 | 決定論的な修正の生成主体 | LLM⇄システム境界([10](requirements/10-llm-system-boundary.md))で発覚。答えが一意な修正(lint/format/誤字)は LLM 生成だと揺れる。**決定的ツール(formatter/linter)が生成**すべきでは＝境界の最適化 | 未決（設計） |
