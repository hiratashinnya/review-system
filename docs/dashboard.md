...
| A3 | MVP の線引き | 全機能のうち最初に何を作るか。**方向性：決定的ツール群（MCP）＋プロンプト雛形を作り、Claude が LLM 役で回す**（[11](requirements/11-platform-adapter.md) 双方向の口）。**価値ベースの線引きは [12-mvp-scope](requirements/12-mvp-scope.md)：提案 MVP＝P1 コア（文書→仕分けレポート）＋P2（戻せる自動修正）** | 🟡 範囲提案あり→確認待ち |
...
> **運用ルール（Q1/Q9/Q11/Q2）は機構＋デフォルトに留め、設計で詰めない。**（Q16 は削除＝滞留は観測不能）
