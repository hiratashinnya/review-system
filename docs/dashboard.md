...
最終更新：2026-06-04（処理パイプライン09を通し・Q20起票/Q3既定/Q7強化）
...
| Q3 | 自動修正サマリの粒度・revert 単位 | **既定境界：1レビュー実行の🤖適用を一括＝1トランザクション**（パイプライン[09](requirements/09-processing-pipeline.md)が1実行単位）。実装候補：内部に一時ローカル git を作りコミット管理して revert（外部ホスト依存とは別物） | 🟡 既定あり |
...
| Q20 | 同一実行内の自動修正衝突 | パイプライン[09](requirements/09-processing-pipeline.md)で発覚。🤖 が同一ファイル/行に重なる修正を出した場合の適用順序・衝突解決（⑦の前提）。Q3 のトランザクションとセット | 未決（設計） |
...
| 自動修正の機構 | Q3 / Q20 | 設計（機構） | 中 | revert＝1実行1トランザクション（既定）／同一実行内の衝突解決(Q20) |
