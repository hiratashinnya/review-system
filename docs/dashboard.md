...
最終更新：2026-06-04（LLM を外部 PF 委譲＝アダプタ方針11を追加・Q22起票/Q5一部決着）
...
| Q22 | PF 能力差の吸収 | アダプタ方針([11](requirements/11-platform-adapter.md))で発覚。PF ごとに構造化出力/ファイル適用/ツール実行の能力が違う。能力宣言の項目と必須/任意の線引き（例：構造化出力は必須、適用・ツールはシステム代替可） | 未決（設計） |
