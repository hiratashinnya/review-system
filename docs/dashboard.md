...
最終更新：2026-07-29（**Codex 第3巡レビューの指摘を反映**：①07 の `suggested_fix.diff` 例を実装契約＝**修正後ファイルの全内容**へ是正（前巡で unified diff 風に書いてしまった誤りを撤回）、②S5 lint の状態を**構文/値域＝実効・値レベル lint＝未結線・段数検査＝未実装・O-14 未達**の4項目へ分解し schema/dashboard/13/design を横断是正、③実装進捗表 #2 `parsing` と Q5・分類表の旧表記を同期、④Q25 の「増分は小さい／新規機構不要」を撤回し「既存 git 機構の再利用可否」と「F7 の前提設計の欠落」を分離、⑤Q26 の選択肢(A)に**作業範囲 A-1〜A-4** を明記——新たに判明した2ギャップ（`doc_type`/`scope` 欠落の無言スキップ・基準ロード失敗が O-14 にならない）を**別 Q を立てず本 Q に取り込み**、オーナー指示で確定）
...
> ⚠️ **2026-07-27 追記（2026-07-29 更新）**：下記「実装進捗 13/13」は**層/モジュール単位の進捗**であり、**MVP 機能単位では2件が未完**——**F7（✋/💬 人手適用・P2 差別化）が未実装**（→ **[Q25](#-未決事項決めないと進めない論点)**。方針は**決定＝A でオーナー確定済み**・実装が未着手＝[A9](#-ネクストアクション次にやる候補)）、および **S5 の値レベル lint が本番経路に未結線・入れ子段数検査が未実装**（→ **[Q26](#-未決事項決めないと進めない論点)**・オーナー未決）。したがって「MVP(P1+P2)実装が完了」は現時点で正確でない。
...
> **13/13 行のモジュールが実装済み**（`review_system/` 全モジュール・`tests/unit/` の review_system 対象テスト109件が全パス）。
> ⚠️ ただしこれは**層/モジュール単位**の進捗で、**MVP 機能単位では2件が未完**：
...
| 2 | `parsing` | 自前 mini-YAML パーサ（`frontmatter.py`）＋値レベル lint（`lint.py`） | 🟡 **モジュールは実装済み・TC-parsing-001（19）**／⚠️ 機能単位では未完＝**①構文パーサは本番経路で実効**（サブセット外は `MiniYamlError`）／**②`lint.py` の値レベル lint は本番経路から呼ばれていない（未結線）**／**③入れ子段数の検査は未実装** ＝ [Q26](#-未決事項決めないと進めない論点) |
...
| A2 | AI への入力設計を決める | 基準＋文書をどう渡すか / 長い文書・コードベースの扱い | ✅ **完了**（2026-07-28 再確定）：契約は確定し、実 JSON の契約検証も存在する（`tests/unit/test_cli_p2.py`）。**07 の JSON 例が実装型 `SuggestedFix{description, diff}` と食い違っていた誤記は本 PR で修正済み**|
| A3 | MVP の線引き | **確定：MVP＝P1＋P2**（[12-mvp-scope](requirements/12-mvp-scope.md)）。方向性：決定的ツール群（MCP）＋プロンプト雛形を作り、Claude が LLM 役で回す（[11](requirements/11-platform-adapter.md)） | 🟢 線引きは確定・**P1+P2 は下表13/13 の層/モジュール単位で実装済み**。⚠️ ただし**機能単位では F7（✋/💬 人手適用・P2 差別化）が未実装**＝[Q25](#-未決事項決めないと進めない論点)／[A9](#-ネクストアクション次にやる候補)、**S5 の値レベル lint も未結線**＝[Q26](#-未決事項決めないと進めない論点) |
...
| Q5 | 技術スタック | **確定：アプリ本体＝Python・原則標準ライブラリのみ**（外部依存は最小）。AI レイヤは外部 PF へ委譲済み（[11](requirements/11-platform-adapter.md)）。git は `subprocess` で CLI 呼び。**Q5a 確定＝自前の最小フロントマターパーサ**（stdlib のみ・PyYAML 不採用）：対応文法を**意図的に小さく固定**（フラット key:value＋単純リスト＋override の1段ネスト）し、**範囲外は S5 lint で fail-close**。**パーサが検証器(S5)を兼ねる**。要：対応文法を [schema](schema/README.md) に明文化。<br>⚠️ **実装状況（2026-07-29 精緻化）**：「パーサ＝検証器」は**構文サブセットの範囲でのみ実効**（`parsing/frontmatter.py` が `MiniYamlError`）。**値レベルの lint（必須キー・`extends`・`id` 一意性・`version` MAJOR）は `parsing/lint.py` にあるが未結線**、**入れ子段数の検査は未実装**、**拒否は未捕捉例外で O-14 形式ではない** ＝ [Q26](#-未決事項決めないと進めない論点) | 🟢 確定（Q5a＝自前パーサ）／⚠️ 実効範囲は Q26 |
...
| S5 | **基準の事前 lint**（実行前検証）：frontmatter 妥当・override∈{locked,tighten-only,open}・extends 存在 | E14 を実行時でなく**作成時**に倒す | Q17,schema | **MVP 推奨**／⚠️ **未達**＝[Q26](#-未決事項決めないと進めない論点)（構文・値域のみ実効） |
...
| **基準 lint の結線（S5）** | **Q26** | **設計（機構）** | 中 | 🔴 **未決**。項目別＝**構文サブセット外＝実効（パーサが拒否）／値域（override 等）＝実効（ローダの Enum 変換）／必須キー・`extends`・`id` 一意性・`version` MAJOR＝`lint_criteria` が未結線／入れ子段数＝検査そのものが未実装**。加えて**拒否は未捕捉例外で O-14 形式ではない** |
...
| 実装 | Q5 | 設計 | 中 | 🟢 Python・原則 stdlib のみ。**Q5a＝自前の最小フロントマターパーサ**（対応文法を schema に明文化）。「検証器 S5 を兼ねる」は**構文サブセットの範囲でのみ実効**——値レベル lint は未結線・段数検査は未実装＝**Q26** |
