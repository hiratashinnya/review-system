...
最終更新：2026-07-28（**Codex 第二意見レビューの指摘を反映**：**Q26 を起票**（S5 lint 未結線）、A9 の「新規設計は不要」を撤回、`10-llm-system-boundary`・`05-io-overview` の I-15/L3 に MVP 印（#6 の5・6箇所目）、06 の凡例を F15 限定に是正、Q25 の状態併存を一本化。再レビューで自分の事実誤認（実 JSON 契約検証の不在）を撤回し 07 の JSON 例を実装型へ修正、A2 は完了へ復帰）
...
| # | アクション | 目的 | 状態 |
|---|---|---|---|
| A1 | 評価基準ファイルのスキーマを実際に書く | サンプルで曖昧さを潰す | 🟡 **記法は確定／S5 lint 未結線**（[Q26](#-未決事項決めないと進めない論点)・2026-07-28 後退）：記法とパース経路は実装済み・運用中（[schema/](schema/README.md)＝`parsing/frontmatter.py`+`persistence/criteria_repo.py` が準拠しテスト済。**`parsing/lint.py` は本番経路から呼ばれていない**＝Q26）。**schema/README.md の見出しを「v0 / 叩き台」→「記法は確定・S5 lint は未結線」へ更新済み** |
...
| **Q26** | **S5 事前 lint が本番経路に結線されていない** | **事実**：`review_system/parsing/lint.py` の `lint_criteria` は**本番経路のどこからも呼ばれていない**（grep 済み）。[schema](schema/README.md) が「書いたら実行前 lint で停止」と規定する検査が、**S5/O-14 の統制された経路として通っていない**。<br>**正確化（2026-07-28 再レビュー）**：<br>・`override` は値域外なら `OverrideRule` Enum への変換時点で弾かれる＝**「強制されない」は不正確**。問題は「弾かれ方が S5 の O-14 明示ではない」こと。<br>・**ネスト段数上限は lint を結線しても効かない**——`parsing/frontmatter.py` にも `parsing/lint.py` にも**深さ検査自体が存在しない**（要新規実装）。<br>・当初併記した「07 の実 JSON 契約検証が無い」は**誤りだったので撤回**（`tests/unit/test_cli_p2.py` が `FilePlatformAdapter` で実 JSON をパースし `suggested_fix.diff` まで検証している）。07 の JSON 例と実装型の不一致は**機械的な文書誤記**として本論点から切り離し、[07](requirements/07-ai-input-design.md) を直接修正済み。<br>**選択肢**：**(A) lint を本番経路（`criteria_repo` のロード時）へ結線し、あわせて欠けているネスト段数検査を実装する**／(B) 結線は post-MVP とし、[schema](schema/README.md) の宣言を「記法のみ確定・lint 未結線」に限定して据え置く（**現状の記述はこの形**）。<br>**推奨 = A**。根拠：S5 は「E14 を実行時でなく**作成時**に倒す」ための策で、結線しなければ目的を果たさない。異常系方針（Q17＝疑わしきは fail-close）とも、未検査のまま流すのは整合しない。**製品コードの変更を伴う**ため実施時期はオーナー判断。<br>**波及**：A1 は「記法は確定／S5 未結線」に後退。A2 は 07 の誤記を直したため**完了へ戻した**。 | 🔴 **未決・オーナー判断待ち**（2026-07-28 起票） |
...
| **基準 lint の結線（S5）** | **Q26** | **設計（機構）** | 中 | 🔴 **未決**。`lint_criteria` が本番経路から呼ばれず「書いたら止まる」が成立していない。ネスト段数検査は未実装 |
