...
最終更新：2026-06-06（Q 棚卸し：Q2/Q11 クローズ・Q5 確定(Python/stdlib)・Q17 方針あり・価値安定化 S1–S6 起票／資産化完了）
...
| Q5 | 技術スタック | **確定：アプリ本体＝Python・原則標準ライブラリのみ**（外部依存は最小）。AI レイヤは外部 PF へ委譲済み（[11](requirements/11-platform-adapter.md)）。git は `subprocess` で CLI 呼び（内部 git／PF CLI とも stdlib で足りる）。**派生 Q5a**：YAML フロントマター（[schema](schema/README.md)）は stdlib で素読みできない→TOML(`tomllib`) へ寄せるか PyYAML を例外許可か（要決定） | 🟢 確定（Q5a 派生） |
...
| Q17 | 異常系イベントの振る舞い | **原案あり**（上「🧯 異常系の方針」）：原則 **「疑わしきは fail-close＋明示エラー(O-14)／空文書だけ benign no-op」**。⑦自動適用は**クリーンな F4 が前提**＝失敗時は書込なし。MVP=全段 fail-close＋O-14 明示、per-unit degrade は F16(post-MVP)。安定化 S3/S5 と一体 | 🟡 方針あり（要確認） |
...
| **価値実現の安定化** | **S1–S6** | **設計** | **高** | LLM 受け口(S1)・安全側仕分け(S2)・異常系(S3)・トランザクション(S4)。MVP 必須 |
| ライフサイクル機構 / 異常系 | Q17 | 設計 | 中 | 🟡 方針あり（fail-close＋O-14／空文書 no-op）。S3/S5 と一体 |
| ~~滞留督促~~ | ~~Q16~~ | — | — | 🗑 削除（滞留はシステムが観測不能） |
| 実装 | Q5 | 設計 | 中 | 🟢 Python・原則 stdlib のみ。派生 **Q5a**＝frontmatter 形式（TOML or PyYAML 例外） |
