...
最終更新：2026-06-04（双方向の口を追加＝MVPはツール群+プロンプトでClaudeがLLM役・Q23起票）
...
| A3 | MVP の線引き | 全機能のうち最初に何を作るか。**方向性：決定的ツール群（MCP）＋プロンプト雛形を作り、Claude が LLM 役で回す**（[11](requirements/11-platform-adapter.md) 双方向の口）。オーケストレータ/UI は後 | 🟡 方向性あり→範囲確定待ち |
...
| Q23 | 双方向の口の境界・順序ガード | PF→System を function calling/MCP で公開する際、どの関数を出すか／適用前に検証を強制するか（例 `apply_fix` は検証済みトークン要求）。MVP はプロンプト＋人手で順序担保、本番はツール側ガード（[11](requirements/11-platform-adapter.md)） | 未決（設計・MVPは軽量で可） |
