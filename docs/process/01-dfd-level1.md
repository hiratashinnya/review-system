...
## プロセス責務（1行）・提供価値・単一責務性

| P | 責務（1行） | 提供価値 | 単一責務か |
|---|---|---|---|
| P1 | 提出物を「対象集合・参照集合・型・scope」に正規化する | 後段が前提にできる土台＝価値経路の入口 | ✕ → L2 分解 |
| P2 | doc_type×scope から観点パック+メタ表を毎回合成する | ぶれない一貫した評価基準の供給（org 権威） | ✕ → L2 分解 |
| P3 | 観点パックと対象から findings/unmatched を得る | レビューの中核価値（観点に基づく指摘）の生成 | ✕ → L2 分解 |
| P4 | findings を検証・除外し 🤖/✋/💬/❓ に仕分ける | 信頼できる指摘に絞り人の負荷を最小化 | ✕ → L2 分解 |
| P5 | 🤖／承認✋／決定💬 を適用し、レポートを組み、revert に応じる | 価値の出口＝直す/見せる/取り消す・**人の判断の反映** | ✕ → L2 分解 |
| P6 | フィードバックを集め、ひな形・合成時警告を扱う（基準編集は系外） | 現場の声を基準に還流しシステムを育てる | ✕ → L2 分解 |

すべて複数責務 → [02-decomposition](02-decomposition.md) で単一責務まで割る。
各 L2/L3 プロセスの提供価値は [02](02-decomposition.md) 各節に記載。価値経路の全トレースは [00 コンテキスト](00-context.md#コンテキストイベントリスト価値経路トレース)。
