...
#### P5.2 適用・コミット＋衝突解決
**責務**：fix を適用し finding 単位でコミット、衝突を解決する。 **提供価値**：取り消し可能性（revert 単位）と同一箇所衝突の安全処理。 **手法**：STS → 各段でワーニエ。

```mermaid
flowchart LR
  IN[確定fix] --> p521((P5.2.1 衝突検出))
...
| # | イベント | 発生源 | 処理 | 出力 → 宛先 |
|---|---|---|---|---|
| P5.2-1 | 確定fix〔各〕 | P5.1 | P5.2.1 衝突検出（同 location グルーピング） | 単独 → P5.2.3 ／ 衝突 → P5.2.2 |
...
#### P6.3 編集の取り込み記録
**責務**：基準編集を記録し逸脱を検知する。 **提供価値**：変遷の追跡可能性と org 方針逸脱の早期警告。 **手法**：STS → 末端ワーニエ。

```mermaid
flowchart LR
  DS1[(基準ファイル)] --> p631((P6.3.1 差分検出))
  p631 -->|差分| p632((P6.3.2 履歴追記))
  p632 --> p633((P6.3.3 逸脱判定))
  p633 -->|逸脱| O1[警告候補 → P6.5]
  p633 -->|適合| O2[記録のみ]
```

| # | イベント | 発生源 | 処理 | 出力 → 宛先 |
|---|---|---|---|---|
| P6.3-1 | 基準/ポリシー編集 | メンテナ | P6.3.1 差分検出（旧↔新） | 差分 → P6.3.2 |
| P6.3-2 | 差分 | P6.3.1 | P6.3.2 変遷履歴追記（O-10） | 履歴 → P6.3.3 |
| P6.3-3 | 履歴付き差分 | P6.3.2 | P6.3.3 逸脱判定（org 緩め等） | 警告候補 → P6.5 ／ 記録のみ |
...
| # | イベント | 発生源 | 処理 | 出力 → 宛先 |
|---|---|---|---|---|
| P6.5-1 | 警告候補〔各〕 | P2.1 / P2.2 / P6.3 | P6.5.1 content_hash 算出 | hash → P6.5.2 |
