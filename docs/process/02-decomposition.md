...
| トリガ | プロセス | 出力 |
|---|---|---|
| 利用者が提出 | P1.1 | 型（PF候補→手動上書き優先） |
| 〃 | P1.2 | scope（MVP=org） |
| 型・scope 確定 | P1.3 | 対象集合・参照集合 |
...
| トリガ | プロセス | 出力 |
|---|---|---|
| P1 完了（型・scope） | P2.1 | マージ済ルール ／ 緩め・locked 違反は警告候補 |
| マージ済に同 id 兄弟本文 | P2.2 | 共存 or 親フォールバック（hash 未ヒット時のみ PF 再判定） |
| 確定ルール | P2.3 | 観点パック・メタ表 |
...
| トリガ | プロセス | 出力 |
|---|---|---|
| P2 完了（観点パック）＋対象/参照 | P3.1 | プロンプト |
| プロンプト | P3.2 | 生応答 |
| 生応答 | P3.3 | findings[] / unmatched[] |
...
| トリガ | プロセス | 出力 |
|---|---|---|
| P3 出力 | P4.1 | 有効findings ／ ❓未分類 |
| 有効findings | P4.2 | 対象内findings（参照由来は破棄） |
| 対象内findings | P4.3 | 🤖/✋/💬 仕分け済 |
...
| トリガ | プロセス | 出力 |
|---|---|---|
| P4 出力（🤖あり） | P5.1→P5.2 | 適用＋DS3 コミット |
| P4 出力 | P5.3 | 評価レポート・diff・原案 |
| 利用者が revert 要求 | P5.4 | DS3 復元＋結果 |
...
| トリガ | プロセス | 出力 |
|---|---|---|
| レビュアー判断/対象外 | P6.1 | DS5 蓄積 |
| DS5 蓄積（しきい値/オンデマンド） | P6.2 | 観点FB提案 |
| Maintainer が DS1 編集 | P6.3 | 変遷履歴＋逸脱→P6.5 |
| 新 scope 立ち上げ | P6.4 | 基準ひな形 |
| 警告候補（P2/P6.3） | P6.5 | 新規警告（既出は抑制） |
