# プロセス設計 02 — 単一責務までの分解（L2）＋プロセス単位の点検

L1 の各プロセスを**単一責務（primitive）**まで割り、各プロセスに**イベントリスト**と**データディクショナリ**を付ける。
データディクショナリ記法：`名前 = { 要素 + 要素 + … }`、`?`=任意、`|`=択一、`{x}`=x の集合。

---

## P1 受付・正規化

```mermaid
flowchart TB
  User[利用者]
  PF[PF・LLM]
  P11((P1.1 文書タイプ判定))
  P12((P1.2 スコープ解決))
  P13((P1.3 対象/参照 集合確定))
  User -->|提出物| P11
  P11 -->|型推定依頼| PF
  PF -->|型候補| P11
  P11 -->|型| P13
  User -->|scope指定/文脈| P12
  P12 -->|scope| P13
  User -->|文書群・参照群| P13
  P13 -->|対象集合・参照集合・型・scope| OUT[→ P2/P3]
```

| 子 | 単一責務 |
|---|---|
| P1.1 文書タイプ判定 | PF の型推定＋手動上書きで `型` を1つ確定（[Q4](../dashboard.md)） |
| P1.2 スコープ解決 | `scope` を確定（MVP は org 固定） |
| P1.3 集合確定 | 文書を**対象集合**と**参照集合**に振り分ける |

**イベントリスト**

| # | イベント（刺激・入力） | 発生源 | 処理（プロセス） | 出力 → 宛先 |
|---|---|---|---|---|
| P1-1 | 評価対象を提出 | 利用者 | P1.1 文書タイプ判定（PF 推定＋手動上書きを優先） | 型 → P1.3 / P2 / P3 |
| P1-2 | scope 指定 or 文脈 | 利用者 | P1.2 スコープ解決（MVP=org 固定） | scope → P1.3 / P2 |
| P1-3 | 型・scope 確定 | P1.1 / P1.2 | P1.3 集合確定（対象/参照へ振り分け） | 対象集合・参照集合 → P2 / P3 |

**データディクショナリ**

- `提出物 = { 文書群 + 参照群? + 型上書き? + scope指定? }`
- `文書群 = { ファイル }` / `参照群 = { ファイル }`
- `型 = code | spec | minutes | …`
- `scope = org`（MVP） `| team:<名> | project:<名>`
- `対象集合 = { ファイル }` / `参照集合 = { ファイル }`

> 🔎 発見：L1 図に **P1→PF（型判定）** の線が無かった（[04](04-gaps-found.md) G1）。型判定も PF 呼び出し。

---

## P2 基準合成

```mermaid
flowchart TB
  DS1[(基準/ポリシーファイル)]
  DS2[(矛盾チェックキャッシュ)]
  PF[PF・LLM]
  P21((P2.1 継承マージ+方向ゲート))
  P22((P2.2 本文矛盾チェック))
  P23((P2.3 パック+メタ表 組立))
  WARN([警告候補])
  DS1 -->|型・scopeの基準群| P21
  P21 -->|reject=緩め/locked違反| WARN
  P21 -->|マージ済ルール| P22
  P22 -->|照合・更新| DS2
  P22 -->|矛盾判定依頼| PF
  PF -->|矛盾 yes/no| P22
  P22 -->|矛盾→親フォールバック| WARN
  P22 -->|確定ルール| P23
  P23 -->|観点パック| OUT3[→ P3/P4]
  P23 -->|メタ表| OUT4[→ P4]
```

| 子 | 単一責務 |
|---|---|
| P2.1 継承マージ＋方向ゲート | org→team→project を union し、**緩め/`locked` 違反を機械拒否**（org 権威）。拒否は警告候補へ |
| P2.2 本文矛盾チェック | 兄弟同 id の本文矛盾を PF で判定（`content_hash` で DS2 キャッシュ照合）。矛盾は親フォールバック＋警告候補 |
| P2.3 パック+メタ表 組立 | LLM 入力用**観点パック**（メタ抜き）と内部**メタ表**を作る |

**イベントリスト**

| # | イベント（刺激・入力） | 発生源 | 処理（プロセス） | 出力 → 宛先 |
|---|---|---|---|---|
| P2-1 | 対象集合・型・scope 確定 | P1 | P2.1 継承マージ＋方向ゲート（緩め/`locked` 違反を機械拒否） | マージ済ルール → P2.2 ／ 警告候補 → P6.5 |
| P2-2 | マージ済に同 id 兄弟本文あり | P2.1 | P2.2 本文矛盾チェック（DS2 照合→未ヒット時のみ PF 判定） | 確定ルール → P2.3 ／ 矛盾＝親フォールバック＋警告候補 → P6.5 |
| P2-3 | 確定ルール受領 | P2.2 | P2.3 観点パック＋メタ表 組立 | 観点パック → P3 / P4 ／ メタ表 → P4 |

**データディクショナリ**

- `合成ルール = { id + title + 本文 + 例 + メタ + provenance }`
- `メタ = { determinism + severity + override + enabled }`
- `観点パック = { (id + title + 本文 + 例) }`（メタ抜き＝PF へ渡す）
- `メタ表 = { id → (determinism + severity + override + provenance) }`
- `警告候補 = { 種別(緩め拒否|矛盾|衝突) + rule_id + content_hash + provenance }`

> 🔎 発見：**方向ゲート判定は合成時（P2）**が正（[schema](../schema/README.md) のマージ時 reject と一致）。育成側 P6 に置くと二重化（[04](04-gaps-found.md) G2）。
> 🔎 発見：**警告候補→既出判定→発行**は P2 と P6 で共通の cross-cutting（[04](04-gaps-found.md) G3）。下記「警告発行」に集約。

---

## P3 評価

```mermaid
flowchart TB
  PF[PF・LLM]
  P31((P3.1 プロンプト構築))
  P32((P3.2 PF呼び出し・アダプタ))
  P33((P3.3 出力パース・整形))
  IN[観点パック+対象+参照] --> P31
  P31 -->|プロンプト| P32
  P32 -->|プロンプト| PF
  PF -->|生応答| P32
  P32 -->|生応答| P33
  P33 -->|findings/unmatched| OUT[→ P4]
```

| 子 | 単一責務 |
|---|---|
| P3.1 プロンプト構築 | 役割制約＋観点パック＋対象＋参照＋出力スキーマを1プロンプトに |
| P3.2 PF 呼び出し | アダプタ経由で PF を実行（[11](../requirements/11-platform-adapter.md)） |
| P3.3 出力パース | structured 化。`location.file` 必須を検証（無ければ補正/未分類化） |

**イベントリスト**

| # | イベント（刺激・入力） | 発生源 | 処理（プロセス） | 出力 → 宛先 |
|---|---|---|---|---|
| P3-1 | 観点パック＋対象/参照 受領 | P2 / P1 | P3.1 プロンプト構築（役割制約＋パック＋対象＋参照＋スキーマ） | プロンプト → P3.2 |
| P3-2 | プロンプト受領 | P3.1 | P3.2 PF 呼び出し（アダプタ経由） | 生応答 → P3.3 |
| P3-3 | 生応答受領 | PF | P3.3 出力パース・整形（`location.file` 必須を検証） | findings[] / unmatched[] → P4 |

**データディクショナリ**

- `プロンプト = { 役割制約 + 観点パック + 対象 + 参照 + 出力スキーマ }`
- `finding = { rule_id + location + quote? + rationale + suggested_fix? }`
- `location = { file + line_range? }`（**file 必須**）
- `unmatched = { description + location + suggested_fix? }`

---

## P4 検証・仕分け（順序固定：4.1→4.2→4.3）

```mermaid
flowchart TB
  META[(メタ表)]
  POL[(ポリシー)]
  P41((P4.1 rule_id検証))
  P42((P4.2 参照除外))
  P43((P4.3 仕分け))
  IN[findings/unmatched] --> P41
  P41 -->|id外れ→未分類| UNM([❓未分類])
  P41 -->|有効findings| P42
  P42 -->|参照由来→除外| DROP([破棄])
  P42 -->|対象内findings| P43
  META --> P43
  POL --> P43
  P43 -->|🤖/✋/💬| OUT[→ P5]
  UNM --> OUT
```

| 子 | 単一責務 |
|---|---|
| P4.1 rule_id 検証 | finding.rule_id がパックに在るか検証。外れ＋元 unmatched → ❓未分類 |
| P4.2 参照除外 | `location.file ∈ 参照集合` の finding を破棄 |
| P4.3 仕分け | `rule_id→メタ→determinism×severity→ポリシー→mode` で 🤖/✋/💬 |

**イベントリスト**

| # | イベント（刺激・入力） | 発生源 | 処理（プロセス） | 出力 → 宛先 |
|---|---|---|---|---|
| P4-1 | findings/unmatched 受領 | P3 | P4.1 rule_id 検証（パック照合） | 有効findings → P4.2 ／ ❓未分類（id 外れ＋unmatched）→ P5 |
| P4-2 | 有効findings 受領 | P4.1 | P4.2 参照除外（`location.file ∈ 参照集合` を破棄） | 対象内findings → P4.3 |
| P4-3 | 対象内findings 受領 | P4.2 | P4.3 仕分け（メタ表 × ポリシー → mode） | 🤖/✋/💬 仕分け済 → P5 |

**データディクショナリ**

- `mode = auto_log_only | auto_suggest | human_only`
- `仕分け済指摘 = finding + mode`
- `❓未分類 = { description + location + suggested_fix? + 由来(id外れ|自己申告) }`

---

## P5 適用・レポート

```mermaid
flowchart TB
  DS3[(finding-commit ワークスペース)]
  User[利用者]
  Reviewer[レビュアー]
  P51((P5.1 修正生成))
  P52((P5.2 適用・コミット))
  P53((P5.3 レポート組立))
  P54((P5.4 revert))
  IN[仕分け済+未分類] --> P51
  P51 -->|確定fix（🤖）| P52
  P52 -->|コミット| DS3
  P52 -->|適用結果| P53
  IN --> P53
  P53 -->|評価レポート| User
  P53 -->|✋diff・💬原案| Reviewer
  User -->|revert要求| P54
  P54 -->|復元| DS3
  P54 -->|revert結果| User
```

| 子 | 単一責務 |
|---|---|
| P5.1 修正生成 | 🤖 の確定 fix を用意：**決定的ツールがあればツール生成**、無ければ LLM 原案を採用（[Q21](../dashboard.md)） |
| P5.2 適用・コミット | 🤖 を対象へ書込み、**finding 単位でコミット**（DS3）。同一箇所に重なる fix は**衝突解決**（[Q20](../dashboard.md) 2段構え：LLM マージ→迷いは人） |
| P5.3 レポート組立 | 3区分＋未分類＋サマリ＋diff＋原案を組む |
| P5.4 revert | 要求された finding／実行単位の commit を戻す |

**イベントリスト**

| # | イベント（刺激・入力） | 発生源 | 処理（プロセス） | 出力 → 宛先 |
|---|---|---|---|---|
| P5-1 | 仕分け済（🤖 を含む）受領 | P4 | P5.1 修正生成（決定的ツール優先／無ければ LLM 原案） | 確定fix → P5.2 |
| P5-2 | 確定fix 受領 | P5.1 | P5.2 適用・コミット＋衝突解決（Q20 2段構え） | DS3 コミット ／ 適用結果 → P5.3 |
| P5-3 | 仕分け済＋適用結果 受領 | P4 / P5.2 | P5.3 レポート組立（3区分＋未分類＋サマリ） | 評価レポート → 利用者 ／ ✋diff・💬原案 → レビュアー |
| P5-4 | revert 要求 | 利用者 | P5.4 revert（DS3 の commit を戻す） | DS3 復元 ／ revert 結果 → 利用者 |

**データディクショナリ**

- `確定fix = { finding_id + 生成元(tool|llm) + diff }`
- `finding_id = rule_id + location`
- `評価レポート = { 🤖済[] + ✋diff[] + 💬原案[] + ❓未分類[] + サマリ }`
- `revert要求 = { 対象: finding_id | 実行ID | all }`

> 🔎 発見：**revert 要求は 05 の入力(I-#)に無い**（O-6 だけ）。入力として未台帳（[04](04-gaps-found.md) G4）。

---

## P6 育成・ガバナンス

```mermaid
flowchart TB
  Reviewer[レビュアー]
  Maintainer[基準メンテナ・org]
  PF[PF・LLM]
  DS1[(基準/ポリシーファイル)]
  DS4[(警告レジャー)]
  DS5[(フィードバック蓄積)]
  P61((P6.1 フィードバック収集))
  P62((P6.2 観点FB提案起草))
  P63((P6.3 編集の取り込み記録))
  P64((P6.4 ひな形生成))
  P65((P6.5 警告発行))
  Reviewer -->|判断・対象外| P61
  P61 --> DS5
  DS5 -->|傾向| P62
  P62 -->|草案依頼| PF
  PF -->|草案| P62
  P62 -->|観点FB提案| Maintainer
  Maintainer -->|編集| DS1
  DS1 -->|変更| P63
  P63 -->|変遷履歴| Maintainer
  Maintainer -->|新scope立ち上げ| P64
  P64 -->|草案依頼| PF
  P64 -->|基準ひな形| Maintainer
  WARNIN[警告候補：P2合成] --> P65
  P63 -->|逸脱| P65
  P65 -->|既出判定| DS4
  P65 -->|新規警告のみ| Maintainer
```

| 子 | 単一責務 |
|---|---|
| P6.1 フィードバック収集 | 判断・対象外フラグを DS5 に蓄積 |
| P6.2 観点FB提案起草 | DS5 傾向から PF で変更原案を起草（O-12） |
| P6.3 編集の取り込み記録 | DS1 変更を記録し変遷履歴に（O-10） |
| P6.4 ひな形生成 | 新 doc_type/scope に PF で基準草案（O-11） |
| P6.5 警告発行 | **警告候補（P2 合成＋P6.3）を DS4 で既出判定し、新規のみ発行**（既出はレポートに混ぜるだけ・Q9） |

**イベントリスト**

| # | イベント（刺激・入力） | 発生源 | 処理（プロセス） | 出力 → 宛先 |
|---|---|---|---|---|
| P6-1 | 指摘への判断・対象外フラグ | レビュアー | P6.1 フィードバック収集 | DS5 へ蓄積 |
| P6-2 | DS5 しきい値到達 or オンデマンド | DS5 / 利用者 | P6.2 観点FB提案起草（PF で草案） | 観点FB提案 → 基準メンテナ |
| P6-3 | 基準/ポリシー編集 | 基準メンテナ | P6.3 編集の取り込み記録 | 変遷履歴 → メンテナ ／ 逸脱 → P6.5 |
| P6-4 | 新 doc_type/scope 立ち上げ | 基準メンテナ | P6.4 ひな形生成（PF で草案） | 基準ひな形 → メンテナ |
| P6-5 | 警告候補 受領 | P2.1 / P2.2 / P6.3 | P6.5 警告発行（DS4 で既出判定） | 新規警告 → メンテナ（既出は抑制） |

**データディクショナリ**

- `フィードバック = { finding_id + 判断(承認|却下|対象外) + 時刻 }`
- `観点FB提案 = { 対象rule_id + 変更案 + 根拠 }`
- `警告レジャー項 = { rule_id + content_hash + first_seen }`
- `警告 = { 種別 + rule_id + content_hash + provenance }`

> 🔎 発見：**P6.5 警告発行**は P2 合成からも P6.3 からも警告候補を受ける cross-cutting プロセス。
> 単一責務（既出判定＋発行）として切り出した（[04](04-gaps-found.md) G3）。

---

## L3 以降：STS × ワーニエ法を交互に適用

L2（P1.1〜P6.5）は**まだ primitive ではない**。「仕分け」「マージ」等は内部に
繰返し・選択を抱える。STS とワーニエ法を**交互**に当てて単一アクションまで割る。
（各 L2 表の右列は責務の**要約**で、実体の primitive は以下の L3〜L4。）

### 適用ルール（どちらをいつ使うか）

- **STS（変換中心分割）**：ノードが「入力整形 → 中心変換 → 出力整形」の**データフロー**で割れるとき。`[入] [変] [出]` を付す。
- **ワーニエ法（データ構造分割）**：ノードが**データ構造に支配される**とき。`順次` / `繰返し〔1..N〕` / `選択` で割る。
- **交互の原則**：STS で出た子のうち「集合を回す・場合分けする」ものは次に**ワーニエ**へ。ワーニエで出た子のうち「小さなパイプライン」は次に**STS**へ。**flow と structure が切り替わる節で手法を持ち替える**。
- **終端（primitive）**：それ以上 flow も構造分岐も無い 1 アクション。

---

### P1 系

#### P1.1 文書タイプ判定 ── STS（小パイプライン）→ 末端でワーニエ
```
P1.1
├ [入] P1.1.1 特徴抽出（拡張子・パス・先頭内容）
├ [変] P1.1.2 PF 型推定
└ [出] P1.1.3 上書き突合 ── ワーニエ・選択
        ├ 手動上書きあり → 採用
        └ なし → 推定を採用
```

#### P1.2 スコープ解決 ── ワーニエ（選択）
```
P1.2
└ scope 決定 ── 選択
    ├ 明示指定あり → その scope
    └ なし → 既定 org（MVP は常にこれ）
```
（primitive）

#### P1.3 対象/参照 集合確定 ── STS → ワーニエ
```
P1.3
├ [入] P1.3.1 グロブ展開（提出指定 → ファイル列）
└ [変] P1.3.2 振り分け ── ワーニエ・繰返し＋選択
        └ 各ファイル〔1..N〕
            ├ 参照指定 or 参照パスルール該当 → 参照集合
            └ それ以外 → 対象集合
```

---

### P2 系

#### P2.1 継承マージ＋方向ゲート ── STS（3段）→ 各変換でワーニエ
```
P2.1
├ [入] P2.1.1 階層読込（org→team→project）
├ [変] P2.1.2 union マージ ── ワーニエ・繰返し＋選択
│        └ 各 rule id〔1..N〕
│            ├ 下位に同 id あり → 継承解決（下位で上書き）
│            └ なし → そのまま採用
└ [出] P2.1.3 方向ゲート ── ワーニエ・繰返し＋選択
         └ 各上書き〔1..N〕
             ├ 緩め or `locked` 違反 → reject＋警告候補
             └ 適合 → 採用
```

#### P2.2 本文矛盾チェック ── ワーニエ（繰返し）× STS
```
P2.2
└ 各 同id兄弟ペア〔1..N〕 ── 繰返し
    ├ P2.2.1 content_hash 算出 …………………… 順次
    ├ P2.2.2 判定取得 ── 選択（内部 STS: [入]hash [変]PF [出]DS2書込）
    │   ├ DS2 ヒット → キャッシュ結果
    │   └ ミス → PF 判定 → DS2 追記
    └ P2.2.3 結果処理 ── 選択
        ├ 矛盾 → 親フォールバック＋警告候補
        └ 非矛盾 → 共存
```

#### P2.3 パック＋メタ表 組立 ── ワーニエ（繰返し・2 出力）
```
P2.3
└ 各 確定ルール〔1..N〕 ── 繰返し
    ├ P2.3.1 メタ表へ (id→メタ) 追記 …………… 順次
    └ P2.3.2 パックへ (id/title/本文/例) 追記 … 順次
```

---

### P3 系

#### P3.1 プロンプト構築 ── ワーニエ（順次＋繰返し＋選択）
```
P3.1
├ P3.1.1 役割制約ブロック ……………… 順次
├ P3.1.2 観点直列化 ── 繰返し（各 rule）
├ P3.1.3 対象本文添付 ………………… 順次
├ P3.1.4 参照添付 ── 選択（あり→添付 / なし→省略）
└ P3.1.5 出力スキーマ添付 …………… 順次
```

#### P3.2 PF 呼び出し ── STS → 末端でワーニエ
```
P3.2
├ [入] P3.2.1 アダプタ要求整形
├ [変] P3.2.2 PF 実行
└ [出] P3.2.3 受領 ── ワーニエ・選択
        ├ 成功 → 生応答
        └ 失敗/タイムアウト → リトライ → なお失敗なら degrade（O-14）
```

#### P3.3 出力パース ── STS → ワーニエ
```
P3.3
├ [入] P3.3.1 構文パース（structured 化）
├ [変] P3.3.2 スキーマ検証 ── 繰返し＋選択（各 item, `file` 欠落 → 補正/未分類化）
└ [出] P3.3.3 findings/unmatched 振り分け
```

---

### P4 系

#### P4.1 rule_id 検証 ── ワーニエ（繰返し＋選択）
```
P4.1
└ 各 finding〔1..N〕
    ├ rule_id ∈ パック → 有効
    └ 外れ → ❓未分類
```
（primitive）

#### P4.2 参照除外 ── ワーニエ（繰返し＋選択）
```
P4.2
└ 各 finding〔1..N〕
    ├ location.file ∈ 参照集合 → 破棄
    └ それ以外 → 残す
```
（primitive）

#### P4.3 仕分け ── ワーニエ（繰返し＋順次＋選択）
```
P4.3
└ 各 finding〔1..N〕 ── 繰返し
    ├ P4.3.1 メタ取得（rule_id → メタ表）…………… 順次
    ├ P4.3.2 スコア算出（determinism × severity）… 順次
    └ P4.3.3 mode 決定 ── 選択
        ├ determinism=high ∧ policy=auto …… 🤖 auto_log_only
        ├ severity=high ∨ override ………… ✋ human(approve)
        └ それ以外 ………………………………… 💬 human_only
```

---

### P5 系

#### P5.1 修正生成 ── ワーニエ（繰返し＋選択）× STS
```
P5.1
└ 各 🤖 finding〔1..N〕 ── 繰返し
    └ 選択
        ├ 決定的ツールあり → ツール生成（内部 STS: [入]ツール選択 [変]実行 [出]diff 抽出）
        └ なし → LLM 原案を fix として採用
```

#### P5.2 適用・コミット＋衝突解決 ── STS → 各段でワーニエ
```
P5.2
├ [入] P5.2.1 衝突検出 ── ワーニエ（同 location でグルーピング, 選択 衝突 有/無）
├ [変] P5.2.2 衝突解決（Q20 2 段）── 選択（LLM マージ可 → 確定 / 不可 → 人へ）
└ [出] P5.2.3 書込み＋commit ── 繰返し（各 fix を finding 単位コミット）
```

#### P5.3 レポート組立 ── ワーニエ（順次＋繰返し＋選択）
```
P5.3
├ P5.3.1 サマリ集計 ………………………… 順次
├ P5.3.2 区分振り分け ── 繰返し＋選択（各指摘 → 🤖済/✋/💬/❓）
└ P5.3.3 提示物添付（diff/原案を該当区分へ）… 順次
```

#### P5.4 revert ── STS → 入口でワーニエ
```
P5.4
├ [入] P5.4.1 対象 commit 特定 ── 選択（finding_id / 実行ID / all）
├ [変] P5.4.2 git revert ── 繰返し（対象 commit 群）
└ [出] P5.4.3 結果報告
```

---

### P6 系

#### P6.1 フィードバック収集 ── ワーニエ（選択＋順次）
```
P6.1
├ P6.1.1 種別判定 ── 選択（承認 / 却下 / 対象外）
└ P6.1.2 DS5 追記 …………………… 順次
```
（primitive）

#### P6.2 観点FB提案起草 ── STS
```
P6.2
├ [入] P6.2.1 傾向抽出 ── 繰返し（各 rule_id の却下/対象外率を集計）
├ [変] P6.2.2 PF 草案
└ [出] P6.2.3 提案整形（対象 rule_id ＋変更案＋根拠）
```

#### P6.3 編集の取り込み記録 ── STS → 末端でワーニエ
```
P6.3
├ [入] P6.3.1 差分検出（旧 ↔ 新）
├ [変] P6.3.2 変遷履歴追記（O-10）
└ [出] P6.3.3 逸脱判定 ── 選択（org 緩め等 → 警告候補 / 適合 → 記録のみ）
```

#### P6.4 ひな形生成 ── STS
```
P6.4
├ [入] P6.4.1 近傍（同型 / 上位 scope）の既存基準収集
├ [変] P6.4.2 PF 草案
└ [出] P6.4.3 ひな形整形
```

#### P6.5 警告発行 ── ワーニエ（繰返し＋選択）
```
P6.5
└ 各 警告候補〔1..N〕 ── 繰返し
    ├ P6.5.1 content_hash 算出 ……………… 順次
    └ P6.5.2 既出判定 ── 選択
        ├ DS4 ヒット → 抑制（レポートに混ぜるのみ）
        └ ミス → DS4 追記＋発行
```

---

## 終端（primitive）の確認

- 階層は **L1 → L2 → L3（→ 一部 L4：選択肢内・STS-in-ワーニエ）**。2 段では終わらない。
- 手法は節ごとに **STS ⇄ ワーニエ** を持ち替え（例：P1.1 STS→末端ワーニエ、P2.2 ワーニエ→内部 STS、P5.2 STS→各段ワーニエ）。
- L3〜L4 の各葉はいずれも flow も構造分岐も持たない 1 アクション＝**primitive**。実装はこの粒度を関数/ステップに対応させる。

