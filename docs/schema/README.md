...
| マッピング | `キー: 値`（キーは `[a-z_][a-z0-9_]*`・小文字スネーク） | `doc_type: code` |
...
- ポリシー：`マッピング → マッピング(`matrix:` の `determinism → severity → mode`) → スカラ` ＋ `列(`overrides:`) → マッピング`。
- すなわち**マッピングの入れ子は最大2段＋ブロック列1種**まで。これを超える構造は**非対応＝fail-close**。

**非対応（書いたら実行前 lint で停止・S5）**

フロースタイル `{…}` / `[…]`、アンカー/エイリアス `&` `*`、複数行スカラ `|` `>`、明示タグ `!!str` 等、
マージキー `<<`、複数ドキュメント（境界の2本目以降）、3段以上のマッピング入れ子、タブインデント。
