...
| 値 | 意味 |
|---|---|
| `locked` | 上書き不可（機械拒否）。セキュリティ・コンプラ等の必須ライン |
| `loosen-needs-approval` | **既定**。厳しくは自由 / 緩めは「要確認」 |
| `open` | 厳しくも緩めも自由（現場裁量に完全委任する推奨ルール） |

> `override` が宣言するのは**機構（方向の検出と `locked` の機械拒否）まで**＝設計。
> 「要確認」を**誰がどう承認するか**は運用ルール（前提条件・既定値）で、設計では詰めない（MVP は単一ユーザー、強制は将来）。

### 承認フローとの接続

リスク側の上書きは「変更を有効化する前に人間の確認を通す」（dashboard **Q1** / `04` フィードバックループ）。
流し読みの「対象外」フラグも、緩める方向の提案ならこの確認ゲートを通る。

> **MVP 注：方向ゲートは「検出」と「誰が承認するか（actor 強制）」を分離する。**
> - **検出**＝機械判定なのでアクター不要。`locked` の上書きは機械的に拒否、緩めは「要確認」として surfacing、厳しくは自由。ここは MVP で成立する。
> - **誰が承認するか**＝アクター区別が要る。だが MVP は**アクターを区別しない**（単一ユーザー・Git ホスト権限にも乗せない）。
>   よって「管理者が承認」という**ロール強制は将来**（専用 RBAC とセット）。MVP では「人間が確認して有効化」までで、承認者の特権化はしない。
> - 役割ラベル（管理者/レビュア/メンテナ）は当面**記述的**な呼称にすぎない。
