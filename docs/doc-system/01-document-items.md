...
| 要素型 | ID プレフィックス | 1 ノードの単位 | 上流参照 | 必須リンク |
|---|---|---|---|---|
| 機能仕様 | `FR-` | 1 要求文（システムが満たす条件） | → `SR-`（直接） | ✅ 必須 |
...
|---|---|---|---|
| 用語／データ辞書 | `TERM-` | 1 用語（定義・型・制約） | → `FR-`（直接） |
...
| 入力 | `I-` | 1 入力（もの＋発生源） | → `FR-` | ✅ P へのリンク必須 |
| 出力 | `O-` | 1 出力（もの＋受け手） | → `FR-` | ✅ P へのリンク必須 |
| 論理プロセス | `P-` | 1 責務（単一責務まで分解） | → `FR-` | ✅ I/O/E へのリンク必須 |
| イベント | `E-` | 1 外部トリガ→反応ペア | → `FR-` | ✅ P へのリンク必須 |
...
| スキーマ | `SCM-` | 1 外部ファイルスキーマ | → `FR-` |
| コンフィグ | `CFG-` | 1 設定項目/設定ファイル | → `SCM-`・`DM-` |
| プロンプトテンプレート | `PROMPT-` | 1 プロンプト雛形 | → `FR-` |
