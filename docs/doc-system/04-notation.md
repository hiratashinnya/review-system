...
各 Markdown ファイルは**1責務**（1ファイル＝1概念グループ）。複数ノードを同一ファイルに置く場合は `---` で区切る。フロントマター（ファイルバージョン）は廃止済み（DD-8）——版管理はノード単位のバッジ `vX.Y.Z` が真実源。
...
<details><summary>⬡ PREFIX-N · vX.Y.Z</summary>
...
| `v<x.y.z>` | ノード固有バージョン（MAJOR.MINOR.PATCH）。著作・更新のたびにノード単位で独立して管理される。z は伝播判定で不問（z バンプはドリフト非誘発） |

> summary 行のバージョンは**ノード固有バージョン（MAJOR.MINOR.PATCH）**であり、ファイルの `version` とは独立した体系（DD-8）。ノードを著作・改訂するたびに増加し、ファイル x.y.z と一致しなくても RULE 違反にはならない。  
...
| 状態 | 表現 |
|---|---|
| 反映済み | `ref_version` の x.y が参照先ノードの x.y と一致（z は伝播判定に不問） |
...
# ノードバージョン発見
summary 行の `⬡ <ID> · vX.Y.Z` の x.y.z がノード固有バージョン（DD-8・フロントマター version は廃止）

# エッジ発見
edges: リスト内の to/ref_version/note を読み取る（kind/status は存在しない）
# ref_version は参照先ノードのバッジ x.y（2パート）を指す（DD-8・z は伝播判定に不問）
```
