...
各 Markdown ファイルは**1責務**（1ファイル＝1概念グループ）。複数ノードを同一ファイルに置く場合は `---` で区切る。フロントマター（ファイルバージョン）は廃止済み（DD-8）——版管理はノード単位のバッジ `vX.Y` が真実源。
...
| `v<x.y>` | ノード固有バージョン（MAJOR.MINOR）。著作・更新のたびにノード単位で独立して管理される。z は省略 |

> summary 行のバージョンは**ノード固有バージョン（MAJOR.MINOR）**であり、ファイルの `version` とは独立した体系（DD-8）。ノードを著作・改訂するたびに増加し、ファイル x.y と一致しなくても RULE 違反にはならない。  
...
# ノードバージョン発見
summary 行の `⬡ <ID> · vX.Y` の x.y がノード固有バージョン（DD-8・フロントマター version は廃止）

# エッジ発見
edges: リスト内の to/ref_version/note を読み取る（kind/status は存在しない）
# ref_version は参照先ノードのバージョン（summary バッジ x.y）を指す（DD-8）
```
