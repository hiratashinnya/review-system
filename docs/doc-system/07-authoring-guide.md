...
**対応内容**: [どう直したか・直さない理由]
**指摘時 ref_version**: {ノードID} "{ref_version 値}"（{ファイル名} v{version} 時点）  ← 必須（DD-3）
```
...
>
> **FND 起票時の必須操作（DD-3 制度化）**: FND 解消時に edges が逆転（FND→対象 → 対象→FND）するため、元の指摘時の ref_version が辺情報から失われる。
> **FND 起票時に `edges[].ref_version` の値を本文にも明記する**ことで解消後も指摘時の版を追跡できる。
> 記録形式: `**指摘時 ref_version**: {ノードID} "{ref_version 値}"（{ファイル名} v{version} 時点）`
