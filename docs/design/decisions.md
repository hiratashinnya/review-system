...
| DD24 | ISO 25010 コード構築原則の機構化順序（Issue #539） | 流入1位＝3 role contract の原則 checklist、流出2位＝既存負債を固定する `maintainability_lint` baseline-ratchet。採否は ISO/オーナー決定、repo 実測は順序だけに使用 | [ISO 25010 gap分析](../methods/iso-25010-code-principles.md)・`.ai/agents/`・`maintainability_lint/` |
