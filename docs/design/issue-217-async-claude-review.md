...
stateDiagram-v2
    ...
    running --> cancelling: cancel / owner lost with child alive / deadline exceeded
...
flowchart TD
  ...
  subgraph API[MCP stdio server / 制御面]
    ...
    A8[launcher failure: failed/worker_start_failed<br/>slot_release_pending=true]
    A9[全 job-scoped process 不在確認]
    A10[finalize_job_release]
    AE[tool error / 明示失敗]
  end
...
| 所見 | 本改訂での処置 | 主な反映先 |
|---|---|---|
| 1. tool境界が`--allowedTools`で、対応CLI version/canonical名/effective setが未証明 | `--allowedTools`を禁止して`--tools Read,Glob,Grep,LS`へ固定。さらに§6.2不変条件6と§13.1でexact binaryごとのliteral probe argv/schema/期待値とcertificationを必須化し、実CLIが機械検証不能なら対応外 | 2.2, 6.2不変条件6, 11.1, 13.1, 16 |
...
| 5. OAuth refreshのnamespace消滅前handoff、CAS、失効/slot順序が未定義 | 当初はbroker DBの二相状態機械を検討したが、scope narrowingによりcredential transaction、auth lease/rotation/recovery一式を#217から除外し#229へ差し戻した | 2.3, 18.2 |
...
| 所見 | 本改訂での処置 | 主な反映先 |
|---|---|---|
| 1. OAuth credentialがClaude native read toolsと同一namespaceにあり、deny破りで読める | 別process/別mount namespace brokerをtarget designとして検討したが、2026-07-14実測で同一host UIDの限界が残った。別host UID/cgroup/seccomp/token非返却実OAuth transportを含むcredential blockerはscope narrowingにより#229へ差し戻した | 2.3, 18.2 |
| 2. authのDB/FS跨ぎ更新がcrash recovery不能でlease/rotation/late handoffが競合する | generation/revision、二相更新、auth transaction recoveryを検討したが、auth lease/rotation/recovery一式を#217のjob release契約から除外し#229へ差し戻した | 2.3, 18.2 |
...
| 4. actual CLI capability probeのargv/schema/副作用境界が実装可能な契約でない | #217のCLI compatibilityは§6.2不変条件6と§13.1でexact binaryごとのliteral argv、機械可読schema/期待値、tool境界の検証を定義した。credential不可視性を含むactual OAuth capability probeは#229へ差し戻した | 2.2, 2.3, 6.2不変条件6, 13.1, 18.2 |
...
| 所見 | 本改訂での処置 | 主な反映先 |
|---|---|---|
| 1. auth job leaseがrefresh publishやcancel等で早期解放され、正常/失敗/spawn loss/rotationを含むCAS/recoveryが一意でない | 当初はjob-scoped broker childの消滅とauth transaction recoveryを`finalize_job_release`へ結ぶ案を検討したが、scope narrowingによりbroker child、auth lease/rotation/recoveryを#229へ差し戻した。#217の`finalize_job_release`が解放するのはactive slotとcleanup leaseだけである | 2.3, 5.1, 6.4, 18.2 |
...
| 3. admission前に外部実行するClaude/bwrap probeが二段gateとdurable identityを迂回する | admission/server起動/statusはrecord/binary metadata等の非実行検査だけに限定。`bwrap --version`、namespace trial、Claude offline probeをclaim後の`bwrap_probe`/`claude_probe` durable roleへ移し、通常roleと同じ二段gate、cleanup lease、全process消滅finalizeの対象にした。credential capability probeは#229の責務とする | 3, 4.1, 6.1-6.2, 6.4, 7.1, 8.1-8.2, 11.1, 13.1, 14, 16, 18.2 |

### 17.4 2026-07-14 owner非依存実測と設計差戻し

| 実測/所見 | 本改訂での処置 | 主な反映先 |
|---|---|---|
| Claude 2.1.208に`--tools`、`--no-session-persistence`、`--bare`が存在する | CLI surface確認とeffective保証を分離した。`--bare`はOAuth/keychainを読まないためOAuth brokerの成立証拠にせず、#217のCLI compatibilityは§6.2不変条件6と§13.1で検証する | 2.1-2.3, 6.2不変条件6, 11.1, 13.1, 16 |
| `claude auto-mode config/defaults`がclassifier設定をJSONで返す | 機械可読inspectionとして記録する一方、個別runのtool/settings/auth attestationには使わない。CLI compatibility検証とcredential blockerを分離した | 2.1-2.3, 6.2不変条件6, 13.1, 18.2 |
| outer bwrapでuser/mount/PID namespaceとprivate `/proc`が成立した | outer bwrap採用は維持した。UID mappingがhost 1000→inner 1000であるため、同一UIDからのcredential隔離は証明されないと明記し、credential blockerは#229へ引き継いだ | 2.1-2.3, 3, 11.1-11.2, 18.2 |
| 別host UID/cgroup/seccomp/token非返却実OAuth transportが未証明 | Opus Blockerを可視化した後、owner判断U-13=Bによりcredential物理隔離とbroker transportを#217から除外し#229へ移管した。#217に実装開始fail-close gateは設けず、#229内の設計・検証事項として引き継ぐ | 1, 2.3, 14, 16, 18.2-19 |
