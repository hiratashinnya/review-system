...
- 状態: 実装前設計改訂案（Codex レビュー11所見、Issue comment 4957631132 の5所見と 4958583056 の3所見、outer bubblewrap 実測、2026-07-12 および 2026-07-13 のオーナー決定、2026-07-14 の owner 非依存実測を反映。credential 物理隔離は **Opus Blocker 未解消**であり、U-12/U-13 と実機スパイク gate の完了前に関連実装へ着手しない）
- 調査日: 2026-07-14
...
6. **OS 境界と Claude permission の二重防御を目標にする。** outer bubblewrap で workspace 外を隔離し、workspace 内の機微ファイルは Claude permission deny で遮断する。OAuth credential を別 principal・別 mount namespace の broker だけに保持させる物理隔離は target design であって、同一host UIDの実測では未証明である。U-12 の token非返却transport、別host UID/cgroup/seccomp/実OAuth spikeが成立しなければ credential 隔離機能を実装・起動しない。
7. **同期 API は #221 まで維持する。** `claude_review` の削除は #221 の全利用者棚卸し・移行完了・owner承認後に限る。同期/非同期を同じ credential 隔離経路へ載せるか、非同期化と物理隔離を別 scope に分割するかは U-13 の owner 判断であり、本書は先取りしない。
...
10. **CLI surface と実効保証を区別する。** Claude 2.1.208 の `--tools`、`--no-session-persistence`、`--bare` と JSON auto-mode は存在を実測済みだが、それだけでは canonical tool 名、最終 effective tool/settings 集合、session 副作用0件、broker transport、credential 不可視性を証明しない。対応 CLI binary digest/version ごとの実機 spike と凍結した契約で証明できない版は対応外とし fail-close する。

### 2.3 Opus Blocker（未解消）
...
これは「追加 hardening」ではなく credential 物理隔離の成立条件である。U-12/U-13 と 11.5 の実装開始 gate が完了するまで、Opus Blocker を解消済み、設計 freeze 済み、実装可能とは報告しない。
...
| Sandbox adapter | outer bubblewrap の Claude 専用 user/mount/PID namespace、private `/proc`、job 固有の不変 workspace snapshot、credential-free synthetic HOME、tmpfs、固定 deny/allow policyを担う。credential隔離部分は別host UID/cgroup/seccomp/CLI effective settings/broker transport の実機証明後にだけ実装対象へ昇格する。admission 前は設定・record・binary metadata の非実行検査だけを行い、`bwrap`/Claude を実行する probe は claim 後の durable roleとして起動する |
| Environment builder | role ごとに空の map から最小 env を構築し、親環境、認証/config/proxy/runtime option の暗黙継承を拒否する |
| Auth broker（target design、未採用） | Claude とは別 host UID・別 mount namespace/cgroup/seccomp 境界で credential root を専有し、token非返却transportにだけ認証済み通信を提供する候補。transport は U-12、#217とのscope関係はU-13のowner判断待ちであり、2026-07-14時点では実OAuth transport未証明のため構成要素として確定・実装開始しない。採用された場合の二相状態機械・lease/finalize契約は11.3の設計候補を満たす |
...
status は Claude quota を消費せず、job/probe を実行しない。cleanup/reconciliation の副作用を status に持たせない。`bwrap` の検出版、user namespace、credential-free synthetic HOME、採用された場合のauth broker transport、CLI probe attestation の可否は、最後にdurable完了したprobe結果と静的recordの照合結果としてsecret/pathを露出しない形で報告し、status呼出し時に外部 executable を起動しない。U-12未決または11.5 gate CLOSEDは明示的にunsupported/blockedと表示する。
...
### 4.6 同期 API 維持と共通経路候補（削除は決定済みgate、共通化はU-13）

#217 では非同期 `start/get/cancel` を追加しても同期 `claude_review` を削除せず、#221の全利用者棚卸し・移行完了・owner承認まで同期APIを維持する。同期adapterを共通job/security lifecycleへ載せる設計は候補として保持するが、その変更を#217のcredential物理隔離と同時に行うかはU-13のscope判断であり未決である。U-13決定前に、旧来の直接`gh`/`claude`実行を廃止した扱い、共通adapterを実装開始可能な扱いにしない。

ownerがU-13で共通化を#217 scopeに含めた場合も、共通security/lifecycleに載せることとlegacyの呼出意味論を変えることを分離する。その候補では同期adapterは版管理された`LegacySyncV1` execution profileを明示指定し、#221の移行完了まで次を凍結する。
...
共通化が選択され、かつ11.5のgateがOPENした場合の候補では、`LegacySyncV1`と`AsyncV1`は結果写像とtimeout/PR取得scopeだけを分け、outer bwrap、不変workspace snapshot、`--tools`/actual CLI probe、空から構築するrole env、採用済みcredential隔離、二段durable launch、active slot、rate-limit無状態分類、cancel/reconciliationを同じ実装で通す。feature flag off時の挙動もU-13決定後の移行設計でfreezeし、先に旧直実行fallback禁止を確定しない。

#221 で repository 内外を含む全利用者・設定・文書・自動化を棚卸しし、非同期APIへの移行と検証が完了し、ownerが削除を承認したことを削除変更の前提gateとする。共通化の有無にかかわらず、非同期実装完了だけを削除根拠にしない。
...
### 11.1 二層 read boundary（workspace境界は実測済み、credential境界はOpus Blocker）

対象 WSL2 では bubblewrap 0.9.0、user namespace 利用可、AppArmor 無効を実測済みとする。2026-07-14 の owner 非依存実測でも outer bubblewrap の user/mount/PID namespace inode が外側と異なり、内側のprivate `/proc`にはPID namespace内のprocessだけが見えることを確認した。実測上 `socat` relay は不要なので導入せず、ネットワーク namespace/egress 制御を #217 の保証へ追加しない。workspace 外は allowlist した実行 runtime と device/proc の最小集合だけを構成し、host `/etc/passwd` は bind しない。

実 HOME と `~/.claude` 全体は mount しない。OAuth credential root を broker 専用 mount namespace にだけread-write mountし、worker/bwrap/Claude namespaceから到達不能にすることは target design である。ただし2026-07-14実測のuser namespaceはhost UID 1000を内側UID 1000へmappingしており、Claudeとbrokerを同じhost UIDで動かす限り、mount/PID namespaceとprivate `/proc`だけを別principal境界とは扱えない。brokerをClaudeとは**別host UID**、別mount namespace/cgroup/seccomp境界で動かし、`0700` root、`0600` immutable generation file、token値のDB/log非保存を成立させる実機spikeは未完了である。

Claude CLI への認証は、U-12でownerが選び、11.4のexact binary version/digestに対して実OAuthで機械検証された **token非返却 broker transport** だけを候補にできる。この transport はClaude namespaceに非秘密の固定endpoint/handleだけを公開し、brokerがnamespace外でOAuth付与・refresh・provider通信を行い、credential file内容、access/refresh token、credentialをopenしたfdをClaudeへ返さず、`SCM_RIGHTS`も禁止する。2026-07-14時点ではこのtransportを確認できていない。Claude 2.1.208の`--bare`はOAuth/keychainを読まず、Anthropic authをAPI keyまたは`apiKeyHelper`に限定するため、`--bare`の存在をOAuth broker成立の証拠にしてはならない。

Claude namespaceにはhost procfsをbindしない。private PID namespaceとprivate `/proc`の作成は実測済みだが、それだけで同一host UIDのbroker保護を証明しない。actual CLIがprocfsなしで作動すること、またはprivate procfs上で`/proc/self/{environ,fd,fdinfo,mountinfo,maps,mem,root,cwd}`、`/proc/<broker-pid>`、`/proc/thread-self`等のaliasを物理的に隠せることを実機で確認する。さらに`mount`, `pivot_root`, `open_by_handle_at`, `name_to_handle_at`, `ptrace`, `process_vm_readv`, `pidfd_getfd`, namespace joinを実際に拒否するseccomp/capability、別host UID/cgroup、secret-bearing fd 0件を証明する。これらは未証明であり、compatibility recordへ推測で記入しない。
...
対応可否はversion文字列だけでなく、security ownerが凍結した`ClaudeCliCompatibilityRecord`で決める。recordは少なくとも`binary_sha256`, `version_stdout_sha256`, `platform`, `probe_contract_version`, **先頭を検証済みClaude絶対pathとする全要素literalな**`offline_probe_argv[]`, `certification_probe_argv[]`, `output_schema_id/hash`, `expected_tool_names=[Read,Glob,Grep,LS]`, `expected_settings_hash`, `broker_transport_id/version`, `proc_policy`, `bwrap_policy_hash`, `certification_attestation_hash/expiry`を持つ。argvにshell、PATH探索、caller入力、未展開placeholder、任意追加optionを含めない。versionごとに実binaryで取得・レビューしたargvとschema fixtureをrepositoryへfreezeするまでそのversionのrecordは存在しないものとして扱う。2026-07-14にはClaude 2.1.208で`--tools`、`--no-session-persistence`、`--bare`と`auto-mode config/defaults`のJSON出力までを実測したが、権威あるeffective tool/settings probe、session副作用0件、token非返却broker transport、実OAuthは未確認である。したがって初期supported setは空であり、確認・freezeまで対応済みと扱わない。

`--bare`は安全側のcustomization抑止候補だが、2.1.208ではOAuth/keychainを読まず、Anthropic authをAPI keyまたは`apiKeyHelper`に限定する。OAuth前提の本設計で採用可能かはU-12のtransport spikeで決める。`claude auto-mode config/defaults`のJSONはclassifier設定のinspection用であり、個別review invocationが実際に使ったtool/settings/auth経路のattestationではない。help/config JSONを`ClaudeCliCompatibilityRecord`の代用にしない。
...
### 11.5 Opus Blocker 実装開始 gate（fail-close）

gate idを`G-OPUS-CREDENTIAL-ISOLATION`とし、2026-07-14時点の状態を **CLOSED（未通過）** とする。次の全条件をsecurity ownerが証跡付きで承認して初めてOPENへ遷移できる。

1. U-12でtoken非返却broker transportをownerが選択し、Claude 2.1.208または明示した対応版のexact binary digestに対するtransport契約をfreezeする。
2. Claudeとbrokerを別host UIDにし、mount/PID namespaceとprivate `/proc`に加えてcgroup/seccomp/capability境界を構成する。禁止syscallのnegative testと、同一UID構成では満たせないことを記録する。
3. 専用実OAuth accountでlogin、review、refresh、次job、cancel/crash recoveryを実行し、Claude側のenv/fd/fs/proc/event/diagnosticへtoken/credential-bearing FDが現れず、broker監査でも許可以外のsecret openが0件であることを確認する。
4. `--tools`のcanonical/effective 4 tool、`--no-session-persistence`の永続副作用0件、`--bare`の採否、JSON auto-modeの非attestation境界を機械可読fixtureとレビュー記録にする。
5. U-13で#217の非同期化とcredential物理隔離を同一scopeにするか分割するかをrepository ownerが決定し、Issue境界と受入条件を更新する。

gate CLOSED中は auth broker、token transport、別UID launcher、credential用cgroup/seccomp、OAuth generation/refresh repository、それらを前提にする共通同期/非同期security adapterの実装・schema migration・feature flag追加を開始しない。U-13で「同一scope」が選ばれた場合は#217の全実装を開始しない。「分割」が選ばれた場合に開始可能な範囲はownerが別Issue/改訂設計で明示した非credential部分だけであり、本書から暗黙に許可しない。テスト用stubや「後で差し替える」transportも実装開始に含み、gateを迂回できない。
...
- tool surface: 非同期3 tool + statusと同期`claude_review`の併存、#221の全利用者移行完了前に同期toolを削除しないこと。U-13で共通化を選んだ場合だけ、同期も共通job/admission/採用済みcredential隔離/二段gate lifecycleを通ることと、`LegacySyncV1`のPR view 60秒→diff 120秒、Claudeだけのtimeout scope、exit 0 raw non-JSON成功をprofile contract testで凍結する
...
- 別host UID・別mount/PID namespace/private `/proc`・cgroup/seccomp/capability境界のbrokerと、U-12で選択したtoken非返却transportだけでOAuth refresh/更新を発生させ、二相publish後に次jobも認証でき、Claude側fs/env/fd/proc/tool eventへcredentialが現れないことを実アカウントで確認する。同一UID構成は不合格とし、これはquota/credentialを使う明示実行gateとする。

live smoke は quota を消費するため明示実行のみとし、失敗も TR と log に残す。テスト実行前 commit というプロジェクト規約は、実装フェーズ担当者が守る。

## 14. 移行手順

1. 未決事項 U-1〜U-5、U-7〜U-13 をownerが決定する。特にU-12のtoken非返却transportとU-13のscope分割は本書で先取りしない。idempotency lifecycle はowner決定A（24時間 replay、後続7日拒否、合計8日で削除・再利用）、rate-limitはstateless、outer bwrap採用、同期APIは#221まで維持、という不変条件は未決へ戻さない。
2. Claude Opus に設計レビューを依頼し、**Opus Blocker未解消**を明記したまま所見、採否、設計修正をIssueまたは将来PRに **Codex AI agent** 由来と明記して記録する。flag/config surface確認だけでBlockerをcloseしない。
3. 11.5の`G-OPUS-CREDENTIAL-ISOLATION`実機spikeを行う。別host UID、mount/PID namespace/private `/proc`、cgroup/seccomp/capability、実OAuth token非返却transport、login/refresh/次job/cancel/crashを証跡化し、security ownerがOPENを承認する。**この完了前に関連実装を開始しない。**
4. repository ownerがU-13を決定する。同一scopeならgate OPEN後にだけ#217実装へ進む。分割なら新しいIssue境界・依存・受入条件・「非credential部分」の範囲をownerが明記してから、その範囲だけを開始する。決定前にjob schema、broker stub、共通adapter等を先行実装しない。
5. gate OPENかつU-13の許可範囲確定後、現行rate-limit実装の成功stdout全文regex/reset parse誤検知を先行是正し、成功レビュー本文中の語句・時刻をrate-limitと判定しない回帰testを通す。この是正前に非同期review coreを分岐・複製しない。
6. 許可範囲にjob repository/state machine、独立idempotency/process identity schemaとmigrationが含まれる場合だけ追加する。既存`rate-limit.json`は読まず、移動/変換せず、新schemaにcooldown fieldを作らない。
7. 許可範囲にcredential物理隔離が含まれる場合だけ、worker mode、二段gate launcher/reconciler、durable cleanup owner/signal journal、全process消滅後のauth/slot finalize CAS、sandbox adapter、別host UID/namespace/cgroup/seccompのauth brokerと二相auth repositoryを追加する。actual CLI/bwrap probeはadmission前に実行せず、claim後のdurable roleとする。
8. 許可範囲に非同期APIが含まれる場合だけ`claude_review_start/get/cancel`を追加する。同期`claude_review`は#221まで維持し、U-13の決定を超えて共通security adapterへ変更しない。READMEには運用・保持・冪等24時間/拒否7日/8日削除・復旧・security、bwrap prerequisite、未解消scopeを記載する。
9. fake-only unit/integration、feature flag、manual e2eはgate OPEN後かつownerが許可したscopeに限る。実OAuth refreshを伴うe2eと300秒超の検証を通しても、#217では同期`claude_review`を削除しない。
10. Issue #221 で repository 内外の全利用者・設定・文書・自動化を棚卸しし、告知、非同期 API への移行、移行完了検証、owner承認を行う。
...
| U-12 | token非返却broker transport: (A) Claude 2.1.208または対応版が正式に提供する非秘密endpoint/handle型transportを実OAuthでcertify、(B) 別host UIDのbroker/gatewayへprovider通信を全委譲しClaudeへtoken/credential-bearing FDを返さない別transportを設計・certify、(C) 対応transportなしとしてcredential物理隔離を未実装/後続Issueへ送る。API key、`apiKeyHelper`の返値、env/stdin/memfd/`SCM_RIGHTS`でClaudeへsecretを渡す案はtoken非返却に数えない | 未推奨。2.1.208のhelp/`--bare`/JSON auto-modeでは選べない。実OAuth spike結果を比較してownerが決める | repository owner + security owner / `G-OPUS-CREDENTIAL-ISOLATION` OPEN前 |
| U-13 | #217の非同期化とcredential物理隔離のscope: (A) 同一scopeのまま全#217実装をOpus Blocker解消まで停止、(B) 非同期job/control planeとcredential物理隔離を別Issueへ分割し、ownerが明記した非credential範囲だけ先行、(C) #217自体を設計のみで保留し、transport/隔離Issue完了後に再開 | 未推奨。security差分、移行価値、二重実装リスクをownerが比較する。本書はA/B/Cを選ばない | repository owner / いかなる関連実装・schema migration開始前 |

次は既存owner決定またはレビューで維持する不変条件である。ただしcredential物理隔離の方式、broker transport、#217とのscope関係はU-12/U-13の未決事項であり、確定契約に読み替えない。

- #217 では同期 `claude_review` を残し、#221 の全利用者棚卸し・移行完了・オーナー承認後だけ削除する。
- 同期 `claude_review` を共通 durable job adapterへ変更するか、#221まで現行security semanticsで維持するかはU-13のscope決定に従う。少なくとも#221完了前に同期APIを削除しない。
- outer bubblewrap による workspace 外物理隔離と、workspace 内機微ファイル/auth config の permission deny を併用する。
- `~/.claude` 全体も最小credential bootstrapもClaude namespaceへmountしないtargetは維持するが、credential-free synthetic HOME、別host UID/namespace/cgroup/seccompのauth broker、token非返却transportは未証明である。U-12/U-13と11.5のgate完了前に関連実装へ着手しない。
...
- Claude tool境界の候補は`--tools`とする。2.1.208でflagの存在は実測済みだが、exact binaryごとのactual CLI probe/certificationでcanonical/effective set、`--no-session-persistence`の副作用0件、`--bare`の採否、broker transport/credential不可視性を証明できなければ対応外としてfail-closeする。JSON auto-mode出力はこの証明に代用しない。
...
- U-13で同期adapter共通化を選んだ場合は共通security/lifecycle上の`LegacySyncV1`とし、現行PR取得順/cap、Claudeだけのtimeout scope、exit 0 raw non-JSON成功を#221完了まで凍結する。選択前は共通化決定と扱わない。
...
6. outer bwrap、seal済みworkspace snapshot、typed deny、actual CLI probe、空env、credential-free synthetic HOMEに加え、別host UID/cgroup/seccomp/token非返却実OAuth transportが、4 tool/alias、refresh/cancel/crash/rotationを含め物理的秘密隔離を満たすか。同一UID namespaceだけで満たした扱いになっていないか。
...
12. U-1〜U-5、U-7〜U-13の残判断、特にconcurrency、async non-JSON、固定denyに追加する集合、bwrap version gate、retention、start成功定義、自動モデル選択、token非返却transport、#217とcredential物理隔離のscopeが妥当か。U-12/U-13はowner決定前に確定扱いしない。
13. `G-OPUS-CREDENTIAL-ISOLATION`がCLOSED中に関連実装、stub、schema migration、feature flagを開始できず、U-13の選択に応じた許可範囲だけが明示的に開くfail-close gateになっているか。

## 19. 設計完了条件

- start/get/cancel の外部契約、同期 API 併存、Issue #221 の棚卸し・移行完了後だけ削除する責務境界を owner が承認している。
- state machine、failure taxonomy、retention、read-only boundary がfreezeされている。ただしcredential物理隔離はOpus Blockerが解消するまでfreeze済みと扱わない。
- U-1〜U-5、U-7〜U-13 が決定または明示的に後続Issue化されている。特にU-12のtransportとU-13のscopeはowner決定が必要である。idempotency A（24h/7d/8d）、stateless rate-limit、outer bwrap、#221まで同期API維持は固定する。
- `G-OPUS-CREDENTIAL-ISOLATION`の別host UID/cgroup/seccomp/実OAuth token非返却transport spikeが証跡付きでOPENとなるか、U-13により明示的に別scopeへ分割されるまで、関連実装が開始されていない。
- Codexレビュー11所見、Issue comment 4957631132の5所見、4958583056の3所見、2026-07-14 owner非依存実測が17節から本文契約・未解消Blocker・回帰gateへ追跡可能である。
- Claude Opus 設計レビューの所見と対応が記録され、Opus Blockerを解消済みと誤記していない。
