...
- 同一箇所に重なる fix は**衝突解決**（[Q20](../dashboard.md) 2段：LLM マージ→迷いは人）してから適用（[02](../process/02-decomposition.md) P5.2）。
  ⚠️ **実装状況（2026-07-29）**：衝突解決は**未実装**。かつ `diff` が「修正後ファイルの全内容」である以上、
  衝突単位は「同一箇所」では足りず**同一ファイル**まで広げないと、別 location の先行修正が後勝ちで消える
  （[05 DS3](../design/05-persistence.md)・[Q27](../dashboard.md#-未決事項決めないと進めない論点)）。
