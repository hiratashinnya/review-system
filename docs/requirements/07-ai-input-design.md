...
| 評価対象文書 | 本文＋識別情報（ファイルパス・行番号 or 引用可能な形） | 粒度は [Q12](../dashboard.md) |
...
LLM 出力(findings) ── rule_id でメタ参照 ──▶ determinism×severity×ポリシー
                                            ──▶ 🤖/✋/💬 の3区分（O-1..O-5）
LLM 出力(unmatched) ─────────────────────▶ 未分類バケツ（O-7 / Q7）
```
...
- **観点パック**：`doc_type` で対象基準は既に絞られる。それでも肥大化する場合の取捨は [Q18](../dashboard.md)。
- **評価対象文書**：粒度（Q12）とチャンク戦略に依存。ファイル/PR diff 単位なら自然に収まりやすい。

## このドキュメントで浮いた論点（→ dashboard）

- **Q18 観点パックの肥大化対策** — スコープに数百ルール付くと入力が膨らむ。
  関連しそうな観点だけ事前選別するか（retrieval）、全部載せるか。MVP は「全部載せ」で十分か要検証。
