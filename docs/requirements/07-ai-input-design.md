...
>
> ⚠️ **「実装準拠」と言い切れない範囲（2026-07-28・Codex 第二意見レビュー指摘）**：本書の JSON 例は `suggested_fix` を文字列で示すが、
> 実装の型は `SuggestedFix{description, diff}`（`domain/review.py`）で**表記が一致していない**。また e2e テストは
...
> LLM は自前実装せず**外部 PF（Claude Code/Copilot 等）にアダプタ経由で委譲**する（[11](11-platform-adapter.md)）。
> 以下の入出力契約は「**アダプタが PF に対して満たす契約**」＝PF が変わっても同じ。
...
未分類になるのは2経路：**(a) LLM が自己申告した unmatched**、**(b) finding の `rule_id` が送った観点パックに存在しない**（プログラムが検証して回送。[09](09-processing-pipeline.md)⑤）。
