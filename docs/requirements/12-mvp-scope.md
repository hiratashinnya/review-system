...
| F16 | 異常系 **per-unit graceful degrade**（[Q17](dashboard.md)） | 1ファイル障害で全体停止せず「N/M 適用」で続行。**O-14 明示エラー通知自体は MVP（[13 S3](13-stabilization.md)）**、degrade のみ将来 | 各プロセス | — | 将来 |
...
- **F16 異常系**：Q17 確定で **fail-close＋O-14 明示は MVP**（[13 S3](13-stabilization.md)）。後回しは **per-unit graceful degrade**（1ファイル障害で全体を止めずに続行）だけ。
