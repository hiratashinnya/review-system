...
- shell prefix assignmentと`env`はcommandから捨てず、実行実体へ作用する環境としてclosed grammarへ含める。bare executableに対する`PATH`変更、`GIT_EXEC_PATH`・`GIT_EXTERNAL_DIFF`等の`GIT_*`、dynamic loaderや未知の環境代入は任意実装・外部commandを否定できないため`UnknownPotentialManaged`とする。固定trusted absolute Gitは`PATH`によるGit本体の差替えから独立するため、既知builtinの非evaluator形に限って`PATH`変更を許可する。`LANG`、`LANGUAGE`、`LC_*`だけを実行実体に影響しない明示的な無害集合とし、それ以外を推測で通さない。`env -i`は固定trusted absolute Gitだけに許可する。assignment-only、`export`、`unset`、`hash`、`alias`等のshell状態変更を含むcompoundは後続leafの実効環境を独立分類できないためcomplete invocationをERRORにする一方、`PATH=/tmp /usr/bin/git status; echo done`のようなcommand-scoped assignmentは後続leafへ持ち越さない。
