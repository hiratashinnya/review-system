...
- `git` は実行名だけでblanket-safeにしない。既知builtinだけをclosed allowlistでnonmanagedとし、`git -c alias.*=!…`、`--config-env`、shellを評価するoption、未知subcommand/aliasは内容を安全に再分類できない限り`UnknownPotentialManaged`とする。`git bisect run <cmd> [<arg>…]`のcommand operandは同じclosed grammarへ再帰投入し、静的nonmanagedだけを通す。shell evaluator option registryはGit parse-optionsが受理する一意なlong略記と`--long=value`、builtin固有short/attached/clusterを同じ意味へ束縛する。少なくともrebase `-x`/`--exec`、clone `-u`/`--upload-pack`、fetch/pull `--upload-pack`、push `--receive-pack`、grep `-O`/`--open-files-in-pager`、diff/log系`--ext-diff`/`--textconv`を含む。fetch/pullの`-u`は`--update-head-ok`、pushの`-u`は`--set-upstream`、rebaseの`-X`はstrategy optionなのでnon-evaluatorとして通し、`--`以後の同名tokenはrevision/path dataとして評価対象外にする。
