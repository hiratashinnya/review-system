...
4. 新規sessionが使用したaudit JSONLの各最新 `pre_use_decision` がfixtureの`expected_audit`またはprobe固有`expected_audit`と一致することを確認する。必須値は`pr-merge-audit/4`、classifier `1.8`、現在sessionのasset hash、実際のhook event ID、期待reason、`permit_issued=false`、`operation_dispatched=false`、`merge_api_called=false`であり、同invocationの`post_use_completion`は0件である。
