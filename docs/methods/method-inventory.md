...
- **手順**：**D0 hard block の確認（最優先）** → D1 入力を読んだか（非機密アンカー照合）→ D6 本番の起動経路特定 → D5 永続ログの時系列で回帰境界 → D4 版・更新時刻の同一性 → D3 再現（**read-only・冪等・低コストの場合に限り**最低2回・1因子ずつ・試行上限を事前に決める）→ D2 status 項目の読み直し → D7/D8 対策の副作用と最小権限。**D0 該当時は D1〜D8 の新規プローブを行わず、既存証拠の読み取りと停止報告だけを行う。**
...
- **成果物**：原因（未特定なら絞り込めた範囲）・検証マトリクス（形式×試行回数×結果）・対策の選択肢と推奨。対応スキル：`silent-failure-diagnosis`（`user-invocable:false`・委譲系エージェントが `skills:` でプリロード）。
