...
### A21. 静かな失敗の診断（外部プロセスが「動いているのに間違っている」とき）
- **目的**：委譲先 CLI/MCP・フック・CI が**エラーを出さずに**誤答・無作動・空応答している疑いを切り分ける。最悪の失敗はエラーではなく**自信のある誤答**で、気付けないまま成果に混入する。
- **トリガ**：外部プロセスの結果に違和感がある／診断コマンドの総合判定と実挙動が食い違う／「以前は動いていた」。
- **手順**：**D0 hard block の確認（最優先）** → D1 入力を読んだか（非機密アンカー照合）→ D6 本番の起動経路特定 → D5 永続ログの時系列で回帰境界 → D4 版・更新時刻の同一性 → D3 再現（**read-only・冪等・低コストの場合に限り**最低2回・1因子ずつ・試行上限を事前に決める）→ D2 status 項目の読み直し → D7/D8 対策の副作用と最小権限。**D0 該当時は D1〜D8 の新規プローブを行わず、既存証拠の読み取りと停止報告だけを行う。** **D0 の例外は実行者がその場で新設できない**——正本スキルへの事前記載＋オーナーの明示承認が揃ったものだけが有効（現在は `/codex-review` の rollout 回収 1 件のみ）。
- **判断基準**：**適用範囲を混ぜない**——権威ある静的な主張（公式ドキュメント・spec・確定判断）は**再検証しない**／外部プロセスのランタイム挙動は**実測・再現する**。前者を実証し直すのは浪費、後者を文書で済ませるのは事故。
- **点検観点**：出力の正しさより先に前提（入力を読んだか）を確かめたか・単発結果で因果を断定していないか・比較対象の版が同一か・直した先が本番経路か・対策が最小権限か。
- **成果物**：原因（未特定なら絞り込めた範囲）・検証マトリクス（条件×試行回数×結果×**実際に使われた非機密値／秘匿済み事実**×判定。秘密値は常に `<redacted>`・**パスも一律に非機密ではなく** repo-relative 等の安全確認済み表記だけを書き、絶対パス/ホーム配下は秘匿）・対策の選択肢と推奨。対応スキル：`silent-failure-diagnosis`（`user-invocable:false`・委譲系エージェントが `skills:` でプリロード）。
- **由来**：2026-07-27〜28 の agy 調査（誤った根本原因を一度断定し、追調査で覆した経験）から抽出。ツール固有の事実は `/agy-delegate`・`/codex-review` が正本で、ここは一般則のみ。
