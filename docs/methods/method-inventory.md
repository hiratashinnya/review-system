...
- **成果物**：原因（未特定なら絞り込めた範囲）・検証マトリクス（条件×試行回数×結果×**実際に使われた非機密値／秘匿済み事実**×判定。秘密値は常に `<redacted>`）・対策の選択肢と推奨。対応スキル：`silent-failure-diagnosis`（`user-invocable:false`・委譲系エージェントが `skills:` でプリロード）。
