...
|---|---|
| 計画版 | `1.1` |
| 状態 | `active / Step 1 in progress (external actions not started)` |
...
| 対象 | 版 | MAJOR | MINOR |
|---|---|---|---|
| 本計画 | `1.1`（構造不変の台帳状態更新） | Step/gate/脅威モデルの構造変更 | 文言・補足・状態更新 |
...
| 1 Containment | `in_progress / external actions not started` | 本transition PR。外部操作はtransition PR merge＋owner承認後 | branch `agent/claude-review-step1-ledger`。MCP disable、#211 hold、config backup、restartはいずれも未実施 | ownerへ対象・副作用・backup/rollback・restart要否を提示して明示承認取得 |
...
台帳更新は観測済み証跡に基づく。PR #238内ではStep 0を`in_progress`のまま保ち、remote merge確認後の本ledger transitionでのみ`done`へ更新した。本transitionにはPR #238 URL、final head、merge commit、merge時刻を記録している。本transition PRがmainへmergeされるまでStep 1の外部操作とStep 2以降を停止する。Step 1の`in_progress`は承認準備の開始を表し、containment実施済みを意味しない。作業開始予定を`done`として記録せず、失敗・rollbackも結果として残す。

## 12. 現時点の次アクション

**提案:** 本transition PRを独立文脈でレビューし、mainへmergeした後、ownerへcontainmentの対象・副作用・config backup/rollback・restart要否を再提示して明示承認を得る。承認前はMCP disable、PR #211 hold記録、config backup、restartを開始しない。Step 1の3条件を満たして`done`/`Disabled`証跡を台帳へ記録するまで、offline correctnessを含むStep 2〜6は開始しない。
