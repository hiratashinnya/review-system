...
2. Step 1で既存supervisorのcached構成破棄後にMCP disabled（対象process 0本・再spawnなし・新規CLI status）を確認し、PR #211 merge hold記録、rollback材料確保の3条件をすべて満たして、台帳へStep 1=`done`と`Disabled`証跡を記録するまでStep 2〜6を始めない。disable/hold確認の失敗またはowner未承認時は全下流をSTOPし、offline fixtureによるcorrectness作業も開始しない。
...
- **done gate:** 既存supervisorのcached構成を破棄した後、対象`server.py` process 0本・一定操作後の再spawnなし・新規CLI statusによってMCP disabledを確認し、PR #211 merge holdを記録し、旧config backup・復元手順等のrollback材料を確保した3条件が揃う。これらと`Disabled`（MCP disabled、#211 merge hold、新policy disabled）を台帳へ記録してStep 1を`done`にする。
...
Step 1を完了するには、owner承認範囲内で親側のCodex supervisor/sessionをrestartまたは停止してcached MCP構成を破棄し、対象`server.py` processが0本であること、一定の操作後にも再spawnしないこと、新規CLIで`claude_review = disabled`かつ`agy` / `serena = enabled`が維持されることを確認する。これらの実測証跡を台帳へ追加するまでStep 2〜6はSTOPする。
...
| 対象 | 版 | MAJOR | MINOR |
|---|---|---|---|
| 本計画 | `1.3`（Step 1未完了の実測を反映した台帳訂正） | Step/gate/脅威モデルの構造変更 | 文言・補足・状態更新 |
...
台帳更新は観測済み証跡に基づく。PR #238のmergeとStep 0 transition merge後、owner承認の下でStep 1の外部操作を開始した。configのdisabled表示、PR #211 merge hold、rollback材料は確認済みだが、既存supervisor配下で設定変更後の対象process再spawnを確認したため、MCP呼出不能のverificationと`Disabled` done gateは未達である。restart/停止後のprocess不在・再spawnなし・新規CLI statusの実測証跡を記録するまでStep 1を`in_progress`とし、Step 2以降を開始しない。失敗・rollbackが生じた場合も結果として残す。

## 12. 現時点の次アクション

**次アクション:** owner承認範囲内で親側のCodex supervisor/sessionをrestartまたは停止し、cached MCP構成を破棄する。その後、対象`server.py` processが0本で一定の操作後も再spawnしないこと、新規CLIで`claude_review = disabled`かつ`agy` / `serena = enabled`であることを実測する。証跡を本台帳へ追加してStep 1を`done / Disabled`へ更新し、独立レビュー後にmainへmergeするまでは、offline correctnessを含むStep 2〜6を開始しない。
