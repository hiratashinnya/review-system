...
|---|---|
| 計画版 | `1.4` |
| 状態 | `active / Step 1 in progress / restart and process absence verification pending / Step 2 stopped` |
...
- **done gate:** 既存supervisorのcached構成を破棄した後、後述するfinal verification sequenceの全checkpointで対象`server.py` process 0本、`claude_review = disabled`、`agy` / `serena = enabled`かつread-only operation成功を確認し、PR #211 merge holdを記録し、旧config backup・復元手順等のrollback材料を確保した3条件が揃う。これらと`Disabled`（MCP disabled、#211 merge hold、新policy disabled）を台帳へ記録してStep 1を`done`にする。
...
- したがって、新規CLIによるconfigのdisabled表示、backup、PR #211 merge hold、新policy disabledは達成済みだが、既存sessionのcached MCP構成破棄と対象process不在の確認は未達である。Step 1と現在状態は`in_progress`のままとし、`Disabled` / `done`とは判定しない。

Step 1を完了するには、owner承認範囲内で親側のCodex supervisor/sessionをrestartまたは停止してcached MCP構成を破棄し、次のfinal verification sequenceを完了する。これらの実測証跡を台帳へ追加するまでStep 2〜6はSTOPする。
...
2. **共通checkpoint証跡:** 下記(a)〜(d)の各checkpointで、観測時刻、fresh supervisor/sessionのPID/PPID、対象`server.py` process一覧と件数、`codex mcp list` / `get`相当のstatus、`agy` / `serena`の副作用を起こさないread-only operation結果を記録する。各checkpointの前後で、対象processが0本、`claude_review = disabled`、`agy` / `serena = enabled`かつusableであることを確認する。
3. **checkpoint (a) — fresh起動直後:** fresh Codex supervisor/sessionを起動し、通常turnやtool discoveryを行う前に共通checkpoint証跡を採取する。
4. **checkpoint (b) — MCP config/tool discovery後:** MCP configを読み込み、登録tool discoveryを明示的に実行した後に共通checkpoint証跡を再採取する。
5. **checkpoint (c) — enabled MCPと通常経路の刺激後:** `agy`と`serena`のread-only operationを実行し、通常turnと別文脈生成をそれぞれ完了した後に共通checkpoint証跡を再採取する。
6. **checkpoint (d) — 観測窓後:** checkpoint (c)完了後に30秒間連続観測し、その終了時に共通checkpoint証跡を再採取する。
7. **判定:** いずれかのcheckpointで対象processが1本以上、`claude_review`がenabled/callable、他MCPがdisabled/unusable、旧parent PIDが残存、または必要な操作・証跡が欠落した場合はStep 1を`in_progress`、Step 2〜6をSTOPのまま維持する。実際の再spawn triggerを特定できない場合も`done`にしない。
...
| 対象 | 版 | MAJOR | MINOR |
|---|---|---|---|
| 本計画 | `1.4`（Step 1 final verification sequenceの具体化） | Step/gate/脅威モデルの構造変更 | 文言・補足・状態更新 |
...
| 1 Containment | `in_progress / restart and process absence verification pending` | Step 0 `done`＋owner明示承認 | 達成済み: config backup `config.toml.pre-claude-review-disable.20260720T060143Z`（mode `600`） / 新規CLIで`claude_review = disabled`、`agy`・`serena = enabled` / [#211 merge hold](https://github.com/hiratashinnya/review-system/pull/211#issuecomment-5019447581) / 新policy disabled。未達: 旧parent PID消滅とfinal verification sequence (a)〜(d) | 親側restart/停止でcached構成を破棄し、fresh sessionの(a)起動直後、(b)tool discovery後、(c)enabled MCP read-only operation・通常turn・別文脈生成後、(d)30秒観測後の全証跡を採取してから`done / Disabled`へ更新 |
...
台帳更新は観測済み証跡に基づく。PR #238のmergeとStep 0 transition merge後、owner承認の下でStep 1の外部操作を開始した。configのdisabled表示、PR #211 merge hold、rollback材料は確認済みだが、既存supervisor配下で設定変更後の対象process再spawnを確認したため、MCP呼出不能のverificationと`Disabled` done gateは未達である。旧parent PID消滅とfinal verification sequence (a)〜(d)の実測証跡を記録するまでStep 1を`in_progress`とし、Step 2以降を開始しない。失敗・rollbackが生じた場合も結果として残す。

## 12. 現時点の次アクション

**次アクション:** owner承認範囲内で親側のCodex supervisor/sessionをrestartまたは停止し、旧parent PID消滅とcached MCP構成破棄を確認する。その後、fresh sessionでfinal verification sequence (a)〜(d)を順番どおり実施し、各checkpointの前後で対象process 0本、`claude_review = disabled`、`agy` / `serena = enabled`かつusableを実測する。全証跡を本台帳へ追加してStep 1を`done / Disabled`へ更新し、独立レビュー後にmainへmergeするまでは、offline correctnessを含むStep 2〜6を開始しない。
