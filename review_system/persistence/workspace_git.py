...
class GitWorkspaceRepository:
    def __init__(self, root: Path) -> None:
        ...
        self._targets: dict[str, set[Path]] = {}   # exec → 展開した評価対象集合（書込許可セット）
    ...
    def open(self, exec_id: ExecutionId, files: dict[str, str]) -> None:
        """対象を作業コピーへ**作り直して**展開し git init、基準点コミットを打つ（残骸排除）。"""
        wd = self.workdir(exec_id)
        if wd.exists():                          # N1：過去の残骸が混入しないよう作り直す
            shutil.rmtree(wd)
        ...
        for rel, content in files.items():
            ...
            allowed.add(p)
        self._targets[exec_id.value] = allowed
        self._run(wd, "init", "-q")
        self._run(wd, "add", "-A")
    ...
    def commit_fix(self, exec_id: ExecutionId, finding_id: str, rel_path: str, new_content: str) -> str:
        ...
        if target not in self._targets.get(exec_id.value, set()):   # N1：評価対象集合のみ書込
            raise ValueError(f"評価対象集合に無いパスへの書込は不可: {rel_path!r}")
