...
- Issue/karte snapshotにcanonical・別role・attackerの実handoff file candidate、またはhost authority/prompt reserved fieldのformat placeholderがあればprompt組立前にfail-closeし、Popenへ到達しない。候補はpath separatorの重複・dot component・filename-only quote/backtickをtokenizeしてquote解除・正規化した後に判定する。bareな`tmp/_handoff/`説明、reservedでない一般placeholder、通常コード断片は単一format passのreplacementとして許可する。
...
4. 両roleのIssue/karte snapshotへcanonical path再掲、別`tmp/_handoff/` path、attacker path、dot/empty component、filename-only quote/backtick、reserved placeholder、role違いpathを注入し、tokenize/normalize後にprompt組立前のsource invalidで拒否されることを確認する。bare directory mention、一般placeholder、通常コード断片は両roleで成功し、canonical pathはexact 1回だけ残ることを確認する。
