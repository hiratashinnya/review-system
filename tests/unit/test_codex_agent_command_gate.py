...
class CodexAgentCommandGateTests(unittest.TestCase):
    ...
    def test_bare_pipe_to_interpreter_bypass_is_denied(self):
        ...
        # Issue #222 二次設計転換（PR #223 opus 三次レビュー対応・2026-07-13・Claude 版と同一
        # 設計）: 以前は「bash がスクリプトファイル引数を伴う場合は過検知しない」という個別の
        # 位置引数チェックだったが、これは列挙ベースの判定の副産物であり、パス正規化バリエー
        # ションで再度バイパスされた（PR #223 opus 三次レビュー指摘A'）。判定原理を「consumer
        # が安全な passthrough だと証明できるか」に反転したことで、bash はどんな引数を伴っても
        # 安全な passthrough とは証明できず一律 deny になる（over-deny 増加を許容する Issue #222
        # の決定事項として意図的に受け入れる）。
        self.assert_denied(
    ...
    def test_subshell_paren_grouping_does_not_over_deny_unrelated_commands(self):
        ...
        # Issue #222 二次設計転換（PR #223 opus 三次レビュー対応・2026-07-13・Claude 版と同一
        # 設計）: bash は安全な passthrough と証明できないため、内容が無害（"safe text"）でも
        # 一律 deny になる（オーナー方針「内容やコマンド種別に関わらず一律deny」の直接の帰結。
        # over-deny 増加を許容する Issue #222 の決定事項として意図的に受け入れる）。
        self.assert_denied(
    ...
    def test_single_line_filter_passthrough_does_not_over_deny(self):
        ...
        # Issue #222 二次設計転換（2026-07-13・Claude 版と同一設計）: wc は安全な passthrough と
        # 証明できないため deny になる（over-deny 増加を許容する決定事項。内容が無害でも
        # consumer の形自体で判定するオーナー方針の帰結）。
        self.assert_denied(
    ...
    def test_stdin_alias_positional_arg_bypass_is_denied(self):
        # PR #223 opus 再レビュー指摘A（2026-07-13・最重要・Claude 版と同一設計）: `/dev/stdin`・
        # `/dev/fd/N`・`/proc/self/fd/N` は OS が提供する「自プロセスの標準入力」を指す疑似
        # ファイルパスであり、`bash /dev/stdin` はファイルではなく実質的に標準入力をスクリプト
        # として読む。
        #
        # 初回の是正（A対応）は `_STDIN_ALIAS_PATH_RE` で個別列挙する方式だったが、opus 三次
        # レビュー（指摘A'）で `/dev//stdin`・`/dev/./stdin`・`//dev/stdin`・`/dev/fd//0` の
        # ようなパス正規化バリエーションで再度バイパスされることが実 bash で確認された
        # （列挙方式の限界）。
        #
        # Issue #222 二次設計転換（オーナー承認・2026-07-13）により、`_STDIN_ALIAS_PATH_RE` は
        # 完全に廃止し、判定原理を「consumer が安全な passthrough だと証明できるか」に反転した。
        # bash はどんな引数（stdin エイリアスのどんな綴りであろうと、実在のスクリプトファイル
        # であろうと）を伴っても安全な passthrough とは証明できないため、個別のパス綴りを
        # 列挙する必要なく一律 deny になる。
        issue_implementer_commands = [
    ...
    def test_xargs_without_placeholder_bypass_is_denied(self):
        # PR #223 opus 再レビュー指摘B（2026-07-13・Claude 版と同一設計）: `-I` を使わない
        # xargs は、stdin から読んだトークンをコマンドの末尾に追記して実行する。
        # xargs_reexec_bodies は `-I` の有無で判定しており、この形式は一切見ていなかった
        # ため素通りしていた（実 bash で実際に実行されることを確認済み）。オーナー方針転換
        # により、xargs が実行しようとしているコマンドの先頭が git/gh/interpreter である
        # 場合、producer 値の確定可否によらず一律 deny する。
        issue_implementer_commands = [
    ...
    def test_dynamic_sink_and_stdin_loop_bypass_is_denied(self):
        ...
        # Claude 版と同一設計）: パイプ consumer の先頭コマンド語が `$(...)`／バッククォート／
        # `${...}` で動的に構成されている場合、それが interpreter か無害なコマンドかを静的に
        # 判定できない。また `while`/`until` から始まり `read` を含む stdin 読み取りループは、
        # 中身を精査せず一律 deny する（実 bash でいずれも実際にペイロードが実行されることを
        # 確認済み）。
        issue_implementer_commands = [
    ...
    def test_pr223_round3_enumeration_variants_are_denied_structurally(self):
        # PR #223 opus 三次レビュー指摘A'/C'（2026-07-13・Claude 版と同一設計）: ラウンド2で
        # 追加した個別列挙（stdin エイリアスパターン・動的コマンド構成パターン・loop
        # キーワード）は、いずれも「見つかった具体的な綴りだけを塞ぐ」対症療法であり、以下の
        # 隣接構文で3ラウンド連続で再度バイパスされた（実 bash で確認済み）。Issue #222
        # 二次設計転換により、これらは個別列挙の追加なしに構造的に閉じる。
        commands = [
            "echo 'git merge feature' | bash /dev//stdin",
            "echo 'git merge feature' | bash /dev/./stdin",
            "echo 'git merge feature' | bash //dev/stdin",
    ...
    def test_novel_execution_capable_interpreters_deny_without_enumeration(self):
        # 受け入れ基準の実演（Claude 版と同一設計）: 今回のレビューで一度も試されていない
        # 全く新しい interpreter/構文パターンでも、safe passthrough と証明できない限り
        # 自動的に deny されることを実演する。
        commands = [
    ...
    def test_quoted_pipe_character_does_not_over_deny(self):
        # 実装中の敵対的自己検証で発見（Issue #222 二次設計転換・重大・Claude 版と同一設計）:
        # split_top_level_pipes が引用符の状態を追跡していなかったため、`grep "a\|b"` のように
        # 引用符の内側にあるリテラルな `|` までトップレベルの分割対象にしてしまい、パイプを
        # 一切含まない普通のコマンドが誤って deny される重大な回帰があった。単一/二重引用符の
        # 状態を追跡するよう是正したため、これらは引き続き allow のままでなければならない。
        commands = [
            'grep -n "a\\|b" file.txt',
    ...
    def test_known_severe_over_deny_bare_pipe_to_non_passthrough_filter(self):
        ...
        # Claude 版と同一設計）: 「consumer が安全な passthrough だと証明できなければ一律
        # deny」という新原理は、grep/wc/awk/jq 等の無害なフィルタコマンドを末尾に置く
        # 「ごく普通の」パイプまで、危険なコンテンツが一切含まれていなくても deny してしまう。
        # over-deny 増加を許容する Issue #222 オーナー方針の直接の帰結として意図的に受け入れる
        # （この重大な実運用影響は PR 説明でオーナーに明示的に開示している）。
        commands = [
