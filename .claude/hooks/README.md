...
> 同レビューで挙がった **設計課題(status-bar 表示の恒久性・アイドル入力中への連結)** と
> **元のフックに元々あった潜在バグ(バナー検出窓の 15/40 不一致・`resets at` 取りこぼし・
> 翌日ロールオーバ・flock フォールバック・前景コマンド固定)** は、本ブランチのスコープ外
> として別 Issue に切り出して対応する。
