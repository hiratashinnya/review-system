...
`SessionStart` イベントのうち **`resume` 以外**(`startup`/`clear`)で発火し、主文脈の役割を
...
| ファイル | 役割 |
|---|---|
| `orchestrator-context.sh` | `SessionStart(startup\|clear)` フックハンドラ。コンテキスト本文を読み込み、`hookSpecificOutput.additionalContext` として JSON で標準出力へ返す。 |
| `orchestrator-context/orchestrator-task-delegation-rules.md` | 注入するコンテキスト本文(要件③によりスクリプトから分離)。内容を変えたい場合はこのファイルのみ編集すればよい。 |

## 発火条件

- `settings.json` の `SessionStart` に `matcher: "startup\|clear"` のエントリを追加している。
