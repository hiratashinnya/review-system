...
# 実体は共有ガード lib-pane-guard.sh の rl_is_claude_pane(on-rate-limit.sh と共通)。
# is_working(=フッタの中断ヒント)はペインが消えていると capture 空で false になり
# 「アイドル」と誤判定するため、注入の可否は前景コマンドで確定させる。
is_claude_pane() { rl_is_claude_pane "$PANE"; }
...
while [ "$i" -le "$RESET_POLL_MAX" ]; do
  ...
  text="$(rl_tmux capture-pane -p -t "$PANE" 2>/dev/null | tail -n 40)"
