...
#   ゲートの対象外＝常に許可する（下記 2026-07-11 のオーナー判断）。
#
...
#     リスト）で遮断済み。ただし gitgate は `python3 -m unittest|pytest|coverage` と同じく Python 実行を
...
if agent_type not in GATED_ROLES:
    # agent_type が対象2ロール以外（欠如を含む・main context 自身がこれに該当）は対象外＝常に許可
    # （ヘッダのオーナー判断・不変条件#1）。command 欠落でも許可する（Codex 版と dispatch 順を統一・
    # Issue #227 レビュー F3。2ロールについては下で command 欠落→deny を維持）。
    pass
