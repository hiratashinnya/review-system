...
#   「B: fail-closedへの設計転換」）：上記までの是正は全て「producer/passthrough 追跡ロジックが
#   安全なパイプ段だと静的に確定できない場合、追跡を諦めて ALLOW する」という fail-open の初期値の
#   上に、新しい未知のシェル構文（サブシェル→sequencing演算子→stderrリダイレクト→fd複製/クローズ→…）
#   が見つかるたびに個別の point-fix（列挙の拡張）を重ねる構造だった。この構造そのものが
#   whack-a-mole の根本原因（シェル構文の亜種は原理的に列挙で塞ぎきれない）と判断し、初期値を
#   反転した：**「パイプの中間段が、literal producer から interpreter/git/gh までの間で安全な
#   passthrough だと積極的に確定できない限り、そのまま到達しうるとみなし deny する」**。
#
...
#   返し、bare_interpreter_reexec_bodies()/xargs_reexec_bodies() の呼び出し元が「sink（interpreter
#   や git/gh）は確定しているが、そこに至る内容を静的に安全と確認できなかった」ことを検知して
#   fail_closed フラグを立て、最終的に find_violation() が独立した deny 理由として返す（既存の
#   「git merge」等の具体的な違反メッセージとは別立てのメッセージにした。役割非対称メッセージ
#   （"is reserved for pr-reviewer" 等）は「もう一方のロールなら許可される」という誤った含意を
#   持つため、fail-closed の理由には使わない）。
#
#   唯一の例外（真に安全と証明できる場合のみ ALLOW 側に倒す）: 丸括弧で囲まれていないステージが
#   `&&`/`||`/`;`/改行 演算子を含む場合（`classify_passthrough_stage` が `_DISCONNECTED_OPERATOR_SEQUENCE`
#   を返す）。これはシェルの結合優先順位（`|` が `&&`/`||`/`;` より強く結合する）により、その時点で
#   実際には別々の独立したパイプラインに分かれていることが構造的に証明できるケースであり、
#   「確定できない」のではなく「確定的に無関係」なので fail-closed の対象にしない
#   （`echo x | cat && true | bash` 等・既存の安全側設計を維持）。
#
...
def heredoc_pipeline_has_downstream_interpreter(line_after_marker):
    ...
    2026-07-13 頑健化（Issue #222 敵対的自己検証で新規発見）: 素朴な `line_after_marker.split("|")`
...
def split_top_level_pipes(text):
    ...
    2026-07-13 是正（Issue #222 敵対的自己検証で新規発見）: `|&`（bash の `cmd1 |& cmd2` ＝
    `cmd1 2>&1 | cmd2` の省略記法。stdout と stderr をまとめて次段へパイプする）が未対応
    だった。旧実装は `|` の直後の文字が `&` の場合を全く考慮しておらず、`|` 自体は素朴に
    パイプ境界として分割する一方、直後の `&` はどちらのステージにも属さず次ステージの
    バッファの先頭に紛れ込み（例: `cat |& bash` → ["cat ", "& bash"]）、`& bash` という
    不正な字句になって consumer 側の interpreter 認識が壊れ、結果的に producer→passthrough→
    consumer の追跡が打ち切られて fail-closed 判定（_UNVERIFIABLE_PRODUCER）にすら至らず
    そのまま ALLOW してしまっていた（実行して確認済み・実際にペイロードが走る重大バイパス）。
    `|` の直後が `&` の場合は両方の文字を消費してパイプ境界とすることで是正する。

    2026-07-13 追加是正（Issue #222 敵対的自己検証で新規発見）: 丸括弧 `(` `)` の深さしか
    追跡していなかったため、ブレースグループ `{ ... }` の内側にある `|` をトップレベルとして
...
def strip_matching_parens(stripped):
    ...
    2026-07-13 拡張（Issue #222 敵対的自己検証で新規発見）: ブレースグループ `{ ... }` は
    Issue #218 時点では意図的にスコープ外（丸括弧のみ対応）だったが、丸括弧非対応のままだと
    `{ cmd; }` の中の `;` が「サブシェル外側の &&/;/|| は別パイプラインに分かれる」という
    無関係な判定（classify_passthrough_stage の _DISCONNECTED_OPERATOR_SEQUENCE）に誤って
...
# fail-closed センチネル（Issue #222）: classify_passthrough_stage の (None, None) には意味が
# 2種類混在している——(a) トップレベル（丸括弧の外側）の `&&`/`||`/`;`/改行演算子を含む段は、
# シェルの結合優先順位（`|` が `&&`/`||`/`;` より強く結合する）により実際には別々の独立した
# パイプラインに分かれることが構造的に証明できる＝「確定的に無関係」であって fail-closed の
# 対象ではない。(b) それ以外の全て（未知のコマンド・ファイルオペランド・引数の組み合わせ・
# 構文エラー等）は「安全と確定できない」であって fail-closed の対象。呼び出し元
# literal_producer_value_through_passthrough がこの2つを区別できるよう、(a) には専用の
# センチネル値を返す。
...
def xargs_reexec_bodies(command_text):
    """戻り値は (bodies, fail_closed) のタプル（Issue #222・fail-closed 設計転換）。
    bodies: 従来どおり、producer 値を静的に確定できた場合の reexec 候補テキストのリスト
    （find_violation が再帰的に scan して実際に git/gh 違反かを厳密判定する・既存の高速パス）。
    fail_closed: True の場合、xargs -I{} の代入先が危険な形（xargs_template_is_dangerous_shape）
    をしているにもかかわらず、上流 producer の値を静的に確定できなかった（
    literal_producer_value_through_passthrough が _UNVERIFIABLE_PRODUCER を返した）ことを示す。
    このとき find_violation は安全側で deny する（内容を具体的に特定できなくても、xargs の
    代入先の形自体が git/gh/interpreter に直結しているため）。"""
    bodies = []
    fail_closed = False
...
# プロセス置換対応（Issue #222 実装時の敵対的自己検証で新規発見）: `<(CMD)`/`>(CMD)`
# （bash のプロセス置換。`<(CMD)` は CMD の標準出力を読める名前付きパイプ／fd のパスに展開され、
# consumer からはファイルパスを渡されたように見える）は、`bash <(echo 'git merge evil')` を実
# bash で実行すると実際にペイロードが走ることを確認済みの重大バイパスだった。
# bare_interpreter_reexec_bodies の「位置引数（スクリプトファイル等）がある場合は標準入力を
# 読まないため対象外」という既存の安全側ロジック（Issue #213 由来）が、`<(...)` を「普通の
# 位置引数＝実在のファイルパス」と誤って同一視してしまうため素通りしていた。実際には `<(...)` の
# 中身は command_text 自身に literal に書かれているコマンドであり（パイプ上流のように実行時
# にしか定まらない値ではない）、producer 遡及は不要でヒアドキュメント本文と全く同じ扱いで
...
def find_violation(command_text, role, depth=0):
    ...
    （具体的に何が検出されたかを表す既存の高精度パス）または None。
    fail_closed: True の場合、xargs_reexec_bodies/bare_interpreter_reexec_bodies のいずれかが
    「sink（interpreter や git/gh）は確定しているが、そこに至る内容を静的に安全と確認できな
    かった」ことを検知したことを示す（violation が None でも fail_closed=True ならこの呼び出しの
    親は deny すべき）。再帰呼び出し（reexec_bodies/quoted_subcommands 経由）のいずれかで
    fail_closed が立った場合も上位へ伝播する。"""
    if depth > 3:
        return None, False
    if depth == 0:
...
if not isinstance(command, str) or not command:
    ...
    elif agent_type in {"issue-implementer", "pr-reviewer"} and fail_closed:
        # Issue #222・fail-closed 設計転換: 具体的な違反コマンドを特定できなくても、パイプ/xargs
        # プレースホルダが interpreter や git/gh に到達することは確定しており、そこに至る内容を
        # 安全と静的に確認できなかった。役割非対称メッセージ（"is reserved for pr-reviewer" 等）は
        # 「もう一方のロールなら許可される」という誤った含意を持つため使わず、専用の理由文を返す。
        reason = (
