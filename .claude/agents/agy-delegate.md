...
| **UC-3 ノード素案** | FND/DD/Q 等の下書きテキスト生成 | `ask`(規律読込)→`continue`(素案) | **G-full** ＋ **A-\<type\>** | ノード素案テキスト（YAML＋本文） | `*-author` が tmp 化→`reconciliation` が検証・確定 |
...
2. **UC 特定・スコープ判定**：依頼を「ユースケースと必読規律」表の UC に同定。**正本書き込み・確定著作・無検証コード採用**に該当するなら移譲を断り、正しい経路（`*-author` / `reconciliation`）を案内する。
