...
- 診断が**すべて正常**、または negative（`[!!]`/`[!]`）な項目が **`newest transcript` だけ**のとき。
  後者は **agy が UNC 配下で transcript をドライブレター無しパスに書こうとして失敗する既知バグ**が原因で、
  ブリッジは stdout 経路を優先するため**実際の委譲は成功する**（2026-07-27 実測：この状態で通常の質問が正常応答）。
  ここで一律停止すると使える経路まで塞ぐので、**これだけを明示的に例外化する**。

**それ以外はすべて停止する（移譲しない）**：

- agy CLI が見つからない／base dir・brain dir が無い＝そもそも起動できない。
- `newest transcript` 以外の項目に negative がある。
- **本エージェントが知らない項目・将来追加される診断・出力形式が変わって判定できない**場合。
- クラウド/ヘッドレス環境で実行されている。

停止時は**移譲せず即停止**し、status 全文を添えて呼び出し元へ報告する。**推測で移譲を試みない。**
...
> **分類表に無い未知の agy ツール**を deny する。`codex_*` / `copilot_*` / `cursor_*`（と `agent_swarm` の
...
1. **疎通チェック**：`antigravity_status` を実行。**許容条件（全項目正常／negative が `newest transcript` だけ）に一致しなければ停止**し、理由と status 全文を添えて報告（移譲しない）。未知・将来追加の診断も停止側。**認証状態は status では判定できない**ので停止条件に数えない（Issue #271）。
...
- [ ] 移譲前に `antigravity_status` を実行し、**許容条件（全項目正常／negative が `newest transcript` だけ）に一致する**ことを確認した（一致しない・未知の診断があるなら移譲せず停止・報告した）。`Overall` の値は報告に載せたが、可否の判定条件には使っていない。
