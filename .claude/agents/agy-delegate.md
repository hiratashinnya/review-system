...
### 止める条件（hard block・移譲しない）

- **agy CLI が見つからない／ログインしていない／base dir・brain dir が無い**＝そもそも起動できない。
- クラウド/ヘッドレス環境で実行されている。

これらに該当したら**移譲せず即停止**し、status 全文を添えて呼び出し元へ報告する。**推測で移譲を試みない。**

### 止めない条件（`Overall` の値だけで判断しない）

**`Overall: OK` を移譲可否の判定条件にしてはならない。** これは repo を読めることの代理指標として無効で、両方向に誤る：

- **偽陽性**：`newest transcript` 項目だけが `[!!]` の `PROBLEMS FOUND` は、**agy が UNC 配下で transcript をドライブレター無しパスに書こうとして失敗する既知バグ**が原因で、
  ブリッジは stdout 経路を優先するため**実際の委譲は成功する**（2026-07-27 実測：この状態で通常の質問が正常応答）。ここで一律停止すると使える経路まで塞ぐ。
- **偽陰性（より危険）**：status は**ワークスペース到達性を一切検査していない**。`Overall: OK` でも、agy が対象リポジトリを開かないまま
  **別のディレクトリについて自信満々に答える**ことがあり得る（下記）。OK は安全の保証にならない。

→ **`Overall` は情報として報告に載せるだけ**にし、可否は上の hard block と下の「ワークスペース確認」で判断する。
...
1. **疎通チェック**：`antigravity_status` を実行。**hard block（CLI 不在・未ログイン・base/brain dir 欠落・クラウド）**に該当すれば理由を添えて停止・報告（移譲しない）。`newest transcript` だけの `PROBLEMS FOUND` では止めない。
...
- [ ] 移譲前に `antigravity_status` を実行し、**hard block に該当しない**ことを確認した（該当なら移譲せず停止・報告した）。`Overall` の値は報告に載せたが、可否の判定条件には使っていない。
