...
以下のいずれかを満たすなら必ず分割する：
- 期待動作に **複数の RULE** が列挙されている（「RULE-009/010」「順に RULE-015・016・019 を報告する」等）
...
**NG 例**（分割すべき）:
```
期待動作: RULE-015 WARNING を報告し、RULE-016 WARNING を報告し、RULE-019 WARNING を報告する
→ SPEC-15-1（RULE-015）/ SPEC-15-2（RULE-016）/ SPEC-15-3（RULE-019）に分割
```
...
親ノードは子それぞれへ `kind: decomposes` 辺を持つ。
子ノードは親 SPEC を `kind: refines` で参照する（FR を直接 refines しない）。
...
condition: normal     # normal | boundary | failure | error（RULE-016 必須）
edges:
  - to: SPEC-X        # 直接の親（FR ではなく親 SPEC）
    kind: refines
    status: pending
...
RULE-015（TD verifies 欠如）は `stage_scope.disable` でシステム的に無効化される。ノード単位で `suppress: [RULE-015]` は付けない。
...
親ノードに子への `decomposes` 辺が不足していれば追加する。
...
- [ ] 親ノードに全子への `decomposes` 辺が存在する
- [ ] 子ノードが親 SPEC を `refines`（FR を直接参照していない）
- [ ] `scheduled: ""`（空文字のみ。値あり禁止）
- [ ] `condition` 属性が全子ノードに存在（RULE-016）
- [ ] `suppress: [RULE-015]` を使っていない（stage_scope に委ねる）
