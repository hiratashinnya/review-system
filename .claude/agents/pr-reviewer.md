...
- **`gh pr checkout` は許可しない**（Issue #502 観測2）。本ロールは `isolation` 指定なしで起動し**メインワークツリー上で動く**ため、`gh pr checkout` は呼び出し元の primary checkout を PR ブランチへ切り替えてしまう。差分は `gh pr diff` / `gh pr view` / `python3 -m gitgate diff|log` で読む。選択の根拠と実測経緯は [rationale](../../.ai/rationale/pr-reviewer.md) を参照する。
