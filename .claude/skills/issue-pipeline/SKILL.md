...
- **実測（Issue #323 / PR #358 の是正ラウンド）**：主文脈がメインワークツリーを PR ブランチへ
  ...
  起動した `issue-fixer` は**セッション開始時点（`main`）でロードされた旧契約**（`handoff_path` を
  「メインワークツリーの絶対パス・完全一致」で検証する契約）のまま動き、新契約の形で渡された
  `handoff_path`（例：`tmp/_handoff/issue-fixer--issue-323-r1.yaml`）を「絶対パスでない／ファイル名が
  旧契約の完全一致条件を満たさない」として拒否し STOP した。
- **`issue-fixer` 側の判断自体は正当**：ロードされている契約に照らせば正しい fail-close であり、
  欠陥はエージェント定義の記述にはない。欠陥は「作業ツリーの現在の内容」と「そのセッションで実際に
  適用される契約」がズレうるという、ハーネスのロード挙動そのものにある。
- **帰結**：**エージェント契約（`.claude/agents/*.md`）を変更する PR は、その変更を実装している
  セッション自身の中では新契約として検証できない。** `/issue-pipeline` は自分自身の運用資産
  （`issue-implementer`/`issue-fixer`/`pr-reviewer` の契約）を改修対象にもする（ドッグフーディング）
  ため、この制約はパイプライン運用に直接効く——同一セッション内でこれらの契約を変える Issue を実装し、
  同じセッションで変更後の版を dispatch しても、その dispatch は変更前の契約で動く。
- **回避策（実際に採ったもの）**：呼び出し元は、**そのセッションでロードされている契約が受理する形**
  で `handoff_path` 等の入力を組み立てて渡す。作業ツリー上のファイル内容が新契約になっていても、
  起動する subagent が実際に従うのは旧契約なので、入力は旧契約の受理条件に合わせる。
- **新契約の実地検証にはセッションの再起動が要る**：新契約が実際に機能するかを確認したい場合は、
  一度セッションを終了し、新しいセッションで改めて対象 subagent を dispatch する（新セッションは
  起動時点の作業ツリー内容を読むため、そこで初めて新契約がロードされる）。
- **本節は制約の明文化と回避手順の共有に留める**：エージェント定義のセッション固定を機構として
  解消すること（動的リロード等）は本節の対象外。機構側の対処が要るかはオーナー判断で別途（Issue #360
  Out of scope）。
