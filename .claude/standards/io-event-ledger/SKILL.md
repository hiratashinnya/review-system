...
```yaml
---
id: FR-1              # 型 prefix + 連番（下表）。採番後は変更禁止
...
condition: normal     # SPEC のみ必須: normal | boundary | empty | failure | error（RULE-016）
                      # 1 ノード = 1 condition。複数条件は SPEC を分割する
---
```
...
4. SPEC は `condition` 属性必須（RULE-016）
...
| SPEC | → FR (refines) | `condition: normal\|boundary\|empty\|failure\|error` ✅ | RULE-015（TD verifies 必須）/016 |
