...
> ※ `silent-failure-diagnosis`（スキル・**共有リファレンス**＝`user-invocable: false`・2026-07-28 新規追加・**未テーラリング active**・テーラリング対象外）は「**外部プロセス（委譲先 CLI/MCP・フック・CI）がエラーを出さずに誤答・無作動している疑い**の切り分け規律」＝[method-inventory A21](../docs/methods/method-inventory.md)。`spec-principles` と同じ共有リファレンス区分で、単独起動せず委譲系エージェントが `skills:` でプリロードする（現在 `agy-delegate`）。**移植可否＝`.agents/skills/` へ移植（`.github` は `user-invocable:false` の構造的 exempt・`.codex/agents` は skill 非適用で N/A）**＝`spec-principles` と同じ扱いのため `asset_parity/exceptions.py` への追記は不要（`python3 -m asset_parity check` で 0 MISSING を確認済み）。ツール固有の既知事象（agy の `Overall` 偽陽性・ワークスペース盲点／Codex の `ERROR:flagged` と rollout フォールバック）は **`agy-delegate`・`codex-review` 側が正本**で、本スキルは一般則のみを持つ（複製しない）。doc-system-v2 の ORC ノード化・`prompt_coverage_targets` 対象外（in-graph の観測可能成果物を持たない repo 運用ツール＝`agy-delegate`/`issue-pipeline`/`bloom-model-tier` と同区分）。由来＝2026-07-27〜28 の agy 調査（誤った根本原因を一度断定し追調査で覆した経験）。
