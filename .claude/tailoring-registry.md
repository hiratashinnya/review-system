...
> ※ `gh-create-issue`（スキル）は GitHub Issue の起票規約を扱う**ユーザー明示起動専用**の repo 運用資産（未テーラリング active・テーラリング対象外）。**Claude Code 版 `.claude/skills/gh-create-issue/SKILL.md` を正本**とし、**Codex 版 `.agents/skills/gh-create-issue/SKILL.md` は手書き mirror**（Claude の `disable-model-invocation: true` に対し Codex は `agents/openai.yaml` の `policy.allow_implicit_invocation: false`）。GitHub Copilot は今回、明示起動方式・Project fields/relations 操作・ユーザー対話境界を未評価のため**非移植**とし、`.github/` に skill/prompt を作らない。`asset_parity/exceptions.py` には `(gh-create-issue, SKILL, GITHUB)` のみ転記し、Codex の `.agents/skills/` は実 mirror があるため例外化しない。
