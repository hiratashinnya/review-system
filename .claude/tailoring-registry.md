...
> ※ `issue-pipeline`（スキル）＋ `issue-implementer`／`pr-reviewer`（エージェント）は Issue 運用の dev-tooling メタパイプライン（Issue #120 で新規追加・**未テーラリング active**・テーラリング対象外）。**Codex CLI へは移植済み**（`.agents/skills/issue-pipeline/SKILL.md`・`.codex/agents/issue-implementer.toml`／`pr-reviewer.toml`。Codex は subagent 委譲・gh CLI 実行を同等に扱え、権限境界は機械ゲートでなくプロンプト規律で担保・Issue #155）。**Copilot（`.github/`）へは非移植のまま**：gh CLI／Claude Code フック（`agent-command-gate.sh`）／Task 委譲／`bloom-model-tier` に依存し **Copilot 等に等価物なし**のため（`agy-delegate` と同様・横展時は「環境依存・非移植」注記が要る）。doc-system-v2 の ORC ノード化・`config.yml` の `prompt_coverage_targets` 対象外（in-graph の観測可能成果物を持たない repo 運用ツールのため・`agy-delegate`/`bloom-model-tier` と同区分）。
...
> ※ `asset_parity`（Issue #155・実体 `asset_parity/`・スキル/エージェントではない裸ツール＝`dsv2`/`docidx` と同区分）は上記4資産ツリー（`.claude/` 正本／`.github/`／`.codex/agents/`／`.agents/skills/`）の presence/absence を read-only 検出する（`python3 -m asset_parity check`）。本台帳の非移植決定（`agy-delegate`・`issue-pipeline`＋`issue-implementer`／`pr-reviewer` の Copilot 非移植）を `asset_parity/exceptions.py` に転記して「意図的な非ミラー」として除外している——**本台帳が一次情報源**であり、`exceptions.py` はそれをコード化したもの（新しい非移植を決めたら本台帳を先に更新し、`exceptions.py` を追従させる）。使い方は `asset_parity/README.md`。**CI 組み込み済み**（Issue #155 フォローアップ・`.github/workflows/asset-parity.yml`）：4ツリーいずれかのパスを触る `push`/`pull_request` で自動起動し、`MISSING` があれば非0終了でビルド失敗（`--fail-on-stale` は未指定のため staleness flag はビルドを止めない・判断根拠は `asset_parity/README.md` の「CI wiring」節）。マトリクスは `$GITHUB_STEP_SUMMARY` にも出力。
> ※ `silent-failure-diagnosis`（スキル・**共有リファレンス**＝`user-invocable: false`・2026-07-28 新規追加・**未テーラリング active**・テーラリング対象外）は「**外部プロセス（委譲先 CLI/MCP・フック・CI）がエラーを出さずに誤答・無作動している疑い**の切り分け規律」＝[method-inventory A21](../docs/methods/method-inventory.md)。`spec-principles` と同じ共有リファレンス区分で、単独起動せず委譲系エージェントが `skills:` でプリロードする（現在 `agy-delegate`）。**移植可否＝`.agents/skills/` へ移植**（Codex 版は frontmatter から `user-invocable` を落とし description を和文化＝`spec-principles` の Codex 版と同じ規約）。`.codex/agents` は skill 非適用で N/A。**`.github`（Copilot）は `user-invocable:false` の構造的 exempt** で `asset_parity` は MISSING を出さないが、**これは `.github/copilot-instructions.md` に内容がインライン展開されていることを検証したものではない**（`asset_parity` は inline 内容を見ない・2026-07-28 Codex レビュー指摘）。**オーナー確定（2026-07-28）＝Copilot へは展開不要・非移植**。本スキルは `copilot-instructions.md` へ展開せず、Copilot 側では参照できない状態を意図として受け入れる。`asset_parity` は `user-invocable:false` の構造的 exempt で MISSING を出さないため `exceptions.py` への追記は不要だが、**その 0 MISSING は「Copilot から参照できる」ことの根拠ではない**（inline 内容を検査しないため）。本台帳のこの記述が非移植決定の一次情報源。ツール固有の既知事象（agy の `Overall` 偽陽性・ワークスペース盲点／Codex の `ERROR:flagged` と rollout フォールバック）は **`agy-delegate`・`codex-review` 側が正本**で、本スキルは一般則のみを持つ（複製しない）。doc-system-v2 の ORC ノード化・`prompt_coverage_targets` 対象外（in-graph の観測可能成果物を持たない repo 運用ツール＝`agy-delegate`/`issue-pipeline`/`bloom-model-tier` と同区分）。由来＝2026-07-27〜28 の agy 調査（誤った根本原因を一度断定し追調査で覆した経験）。
