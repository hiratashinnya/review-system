...
> ※ `issue-pipeline`（スキル）＋ `issue-implementer`／`pr-reviewer`（エージェント）は Issue 運用の dev-tooling メタパイプライン（Issue #120 で新規追加・**未テーラリング active**・テーラリング対象外）。**Codex CLI へは移植済み**（`.agents/skills/issue-pipeline/SKILL.md`・`.codex/agents/issue-implementer.toml`／`pr-reviewer.toml`。Codex は subagent 委譲・gh CLI 実行を同等に扱え、権限境界は機械ゲートでなくプロンプト規律で担保・Issue #155）。**Copilot（`.github/`）へは非移植のまま**：gh CLI／Claude Code フック（`agent-command-gate.sh`）／Task 委譲／`bloom-model-tier` に依存し **Copilot 等に等価物なし**のため（`agy-delegate` と同様・横展時は「環境依存・非移植」注記が要る）。doc-system-v2 の ORC ノード化・`config.yml` の `prompt_coverage_targets` 対象外（in-graph の観測可能成果物を持たない repo 運用ツールのため・`agy-delegate`/`bloom-model-tier` と同区分）。**加えて Codex 側のサブエージェント中断対策指示（「完了確認まで中断しない／時間超過時に進捗確認する」）は Codex 運用差分として `.codex/agents/*.toml` のみで管理し、他 PF へはミラーしない。**
