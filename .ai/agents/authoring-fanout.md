...
5. **`target_key` を呼び出しごとに採番する**。手順は 5-1〜5-4。
   ...
   5-4. **採番後に必ず検査する（fail-close。1件でも該当したら STOP）**：
        - `target_key` の集合が**一意**か。重複があれば **STOP**。同一 `retry_of` を2件以上の target に指定した場合も **STOP**。
        - `(parent_id, kind, brief)` が完全一致する target が2件以上あれば **STOP**。
...
`targets` の各要素を、`author` で指定された **`*-author` エージェントへ同一の委譲単位で並列に**渡す。各 target の `parent_id`・`sprint`・**Step 1-5 で確定した `target_key`**（新規は 5-2 の採番値・再試行は 5-3 の再利用値）（・`retry_of` を伴う再試行なら呼び出し元から受け取った `error` をそのまま）を渡す。`target_key` は各 author のハンドオフファイル名になる（`tmp/_handoff/<author>--<target_key>.yaml`）ので、**必ず渡す**。
...
返ってきたパスが渡した `target_key` と食い違う場合は **STOP**。
...
和集合にして渡す**（重複は除く）。design-author の TERM 設計ファセット追記、spec-author の親サイドカー更新、backref 付与等を含める。
**fanout は値を判断せず、集めて渡すだけ**（更新宣言の契約は `reconciliation-validator.md` Step 2-2）。
...
**`written_by_parent` / `applied_self_fix` / `blocked_reason` はそのファイルを読み取って取る**
  ...
  `written_by_parent` に**このバッチの全 parent_id が揃っているか**を必ず確認する。欠けていれば **STOP 報告**する。
  `status: blocked` なら呼び出し元へ **STOP 報告**する。
