...
未指定なら利用可能なファイル探索機能で既存資産を発見し、対象を冒頭に列挙する。

## 手順
1. **既存資産の棚卸し**：スキル/エージェント/**汎用標準（自動適用されないものを含む）**/テーラリング台帳/規約/手順ドキュメントを読み、各々 `name | 種別 | 責務1行` に台帳化。
2. **新資産ごとに判定**：
...
このエージェントは読み取り専用であり、監査結果を資産ファイルへ書き込まない。検索・索引機能を利用できる環境では、既存資産の比較に使ってよい。
ただし外部の検索基盤へ索引を登録する機能は、リポジトリを変更しなくても外部に永続的な副作用を持ち得る。利用する場合はPF wrapperの実行契約に従い、同じ対象の重複登録を避ける。
