...
要件から MVP スコープまでを順に回す。**主文脈で実行**し、**対話（オーナー判断・Q/DD 起票・停止）が要る段を主文脈に残す**。**非対話の並列著作 fan-out は
`authoring-fanout` エージェント（`author: requirements-author` / `author: spec-author`）へ委譲**する（下記 5）。
分担の設計経緯と移行履歴は [spec-pipeline の rationale](../../rationale/spec-pipeline.md) に置く。
...
- **非対話の並列 fan-out＝エージェントへ委譲**：台帳/分析が「何を著作するか」を出したら、複数親ノードの要求層著作（VAL/SR/FR/NFR・SPEC）を `authoring-fanout`（`author: requirements-author` / `author: spec-author`）に一括ファンアウトさせる（著作→検証→書込を sub-tree 内で完結し、主文脈にはコンパクト要約だけ返る）。

## 手順（順に・各段でチェックポイント）
1. `/align` — 段取りと確定パラメータ（**対話・skill**）。
2. **I/O 台帳＋イベントリスト＋カバレッジ** — 台帳とイベントリストを起こし、ACTOR/I/O/E ノードを **analysis-author** で著作する（2段確定：`reconciliation-validator`→`reconciliation`）。
