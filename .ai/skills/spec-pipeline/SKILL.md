...
（各 PF の subagent workflow の設定に従う）。**対話（オーナー判断・
...
2. **I/O 台帳＋イベントリスト＋カバレッジ** — 台帳とイベントリストを起こし、ACTOR/I/O/E ノードを **analysis-author** で著作する（2段確定：`reconciliation-validator`→`reconciliation`）。※旧 `/io-event-ledger` skill は 2026-06-11 に廃止され、著作規約は型別 `*-author` エージェントへ移管済み（移植履歴は各 PF の資産台帳を比較用に確認）。
