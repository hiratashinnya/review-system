...
> 設計理由は [architecture-design の rationale](../../rationale/architecture-design.md) に置く。
...
4. **外部連携/プロトコル（B）**：副作用・外部判断を**抽象ポート**に。必要なら**駆動プロトコル**（制御を握る側／従う側）と**公開ツール**（決定的処理を外へ）を定義。能力差は系が吸収・**能力宣言**で最低要件を固定。
5. **永続層（C）**：状態（structured-analysis の状態インベントリ）を**リポジトリ port**の裏に隔離。**保存形式**（追記専用/キャッシュ/トランザクション）と**実体**（git 等の枯れた機構）を選ぶ。読み出し1件は frozen。
6. **実行/インポート規約**：正しい package 構造＋絶対 import＋`python -m` 起動。**`sys.path` ハックを使わない**。
...
- 外部システム等の出力が**必ず系の検証を通る**経路になっている（直行経路が無い）。
