...
著作先の `sprint` は doc-system-v2 の構造上の `current_stage` ではなく、運用スケジュールである `docs/doc-system/config.yaml` の `current_phase` から補完する。stage と sprint は別の軸であり、旧手順との比較は規範本文からここへ移した。
