...
def heredoc_command_word(command_text, marker_start):
    ...
    Claude 版と同一設計）: 旧実装は「行内の最後の非フラグ語」をコマンド語とみなしていたが、
    これは `source /dev/stdin <<< 'git merge evil'` のように、コマンドと `<<`/`<<<` の間に
...
def heredoc_pipeline_has_downstream_interpreter(line_after_marker):
    ...
    consumer 側に適用したのと同一原理を、ここにも一貫して適用・Claude 版と同一設計）:
    「先頭コマンド語が HEREDOC_INTERPRETER_COMMANDS という既知のインタプリタ名に一致する場合
    のみ危険」という列挙ベースの判定は、bare_interpreter_reexec_bodies と全く同じ欠陥
    （read+eval・mapfile+eval・for ループ・ブレースグループ等の列挙されていない亜種を見逃す）
    を潜在的に抱える。is_stage_provably_safe_passthrough と同じ「安全な passthrough だと
    証明できるか」を問う原理に統一する：下流ステージが cat/tee/sort/head/tail/uniq 等の安全な
    passthrough 系でなければ、内容やコマンド種別に関わらず「最終的に再実行されうる」とみなし、
    ヒアドキュメント本文を再帰走査対象にする（過検知は許容——ヒアドキュメント本文への再帰
    走査は「即 deny」ではなく「本文を git/gh 違反として scan する」だけなので、本文が実際に
    無害なプロースであれば再帰走査してもそのまま allow される。標準の
    `gh pr create --body "$(cat <<'EOF' ... EOF)"` パターンはヒアドキュメント行にパイプを
    含まないため本関数のループ自体に入らず、この変更の影響を受けない）。"""
...
def split_top_level_pipes(text):
    ...
    2026-07-13 追加是正その2（Issue #222 二次設計転換の実装中に自己発見・重大・Claude 版と
    同一設計）: 単一/二重引用符の状態を一切追跡していなかったため、`grep "a\\|b"` のように
    引用符の**内側**にあるリテラルな `|`（シェル上はパイプ演算子ではなく、grep の正規表現内の
    文字通りの `|`）まで トップレベルの分割対象にしてしまっていた。旧来の「危険パターンを
    列挙してdenyへ倒す」設計ではこの誤分割の結果生まれる偽の consumer 段は「既知の
...
# stdin エイリアスの疑似ファイルパス対応（PR #223 opus 一次/二次レビュー指摘A・2026-07-13）:
# `/dev/stdin`・`/dev/fd/N`・`/proc/self/fd/N`・`/proc/PID/fd/N` を個別列挙する `_STDIN_ALIAS_PATH_RE`
# 方式は、opus 三次レビュー（指摘A'）で `/dev//stdin` 等のパス正規化バリエーションにより再度
# バイパスされることが判明し廃止した（Claude 版と同一設計）。以後は
# is_stage_provably_safe_passthrough（下記 bare_interpreter_reexec_bodies）が「bash はどんな
# 引数であろうと安全な passthrough と証明できない」という原理で、個別のパス綴りを列挙する
# ことなく一律 deny する。


def bare_interpreter_reexec_bodies(command_text):
    """一般パイプ経由対応（Issue #213 受け入れ基準6・Issue #222 二次設計転換で判定原理を刷新・
    PR #223 opus 三次レビュー対応・2026-07-13・Claude 版と同一設計）。
    ...
    **判定原理（is_stage_provably_safe_passthrough のコメント参照）**: パイプ consumer 段
    ...
    唯一の例外: preceding_stage_feeds_nothing（直前ステージの実際の送り手が noop=true/:/false
    と確定できる場合）。オーナー明示の唯一の例外（_DISCONNECTED_OPERATOR_SEQUENCE 相当）を
    FORWARD 方向にも一貫して適用する。
...
# 真のバイパス）。`<(CMD)` は bash が CMD をサブプロセスとして起動しその stdout を fd 経由で
# consumer に渡す仕組みであり、パイプ経由で stdin を渡すのと安全性の観点では等価（consumer は
# 結局 CMD の出力を読む）。
#
# 判定原理（2026-07-13 Issue #222 二次設計転換・PR #223 opus 三次レビュー対応・Claude 版と同一
# 設計）: 旧実装は「`<(...)` を消費するコマンドが HEREDOC_INTERPRETER_COMMANDS という既知の
# インタプリタ名に一致する場合のみ危険」という列挙ベースの判定だった。これは
# bare_interpreter_reexec_bodies の旧設計と全く同じ欠陥（動的に構成されたコマンド名
# `$(echo bash) <(...)` 等で列挙をすり抜ける）を抱える。is_stage_provably_safe_passthrough と
# 同じ「安全な passthrough だと証明できるか」を問う原理に統一する：`<(...)` を消費するコマンドが
# cat/tee 等の安全な passthrough 系（狭い許可リスト）でなければ、内容やコマンド種別に関わらず
# 一律 fail-closed とする。
