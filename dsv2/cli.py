...
サブコマンド:
  ...
  * ``clean-tmp <path>`` 書込後の tmp 著作ミラー（``tmp/<sprint>/<parent-id>``）をガード付きで
      削除（既定 dry-run・``--apply`` で削除）。保護名（``_handoff``・``_karte``・``_worktree``・
      ``_feedback``）・``tmp/`` 外・階層違いは fail-close で拒否する
      （reconciliation Step 3-3 の掃除手段）。
...
def cmd_clean_tmp(args) -> int:
    ...
    ``_worktree``・``_feedback``）・``tmp/`` 外・symlink・階層違いは削除せず
    EXIT_ERROR（fail-close）。既定は dry-run。
