...
新フォーマット（`doc-system-v2/FORMAT.md`）は「1ノード=1YAML、本文は型別 body policy」
（required/shared/none）・`id`＝ファイル stem・`stage/type/status`＝path 導出・無名依存辺という
構造を採る。本パッケージはその v2 コーパスに対する索引生成とグラフ照会・機械改変を提供する:
  ...
  * ``clean-tmp``   … 書込後の tmp 著作ミラー（``tmp/<sprint>/<parent-id>``）をガード付きで削除
                      （保護名（``_handoff``・``_karte``・``_worktree``・``_feedback``）・
                      ``tmp/`` 外・階層違いは fail-close で拒否）。
