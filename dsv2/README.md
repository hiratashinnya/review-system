...
| `cleantmp.py` | 書込後の tmp 著作ミラー削除（`clean-tmp`）。削除してよいのは `<repo>/tmp/<sprint>/<parent-id>` の**ちょうど2階層**のみで、`tmp/` の外・保護名（`_handoff`・`_karte`・`_worktree`）を含むパス・symlink・階層違いは削除せず fail-close（`CleanTmpError`）。既定 dry-run・`--apply` で `shutil.rmtree`。 | 補助: CLAUDE.md「戻り値のハンドオフ規約」（`tmp/_handoff/` は掃除対象外）・Issue #307「診断カルテ CLI」（`tmp/_karte/` も同様に掃除対象外＝保護名として明示）・Issue #309「worktree 所有台帳」（`tmp/_worktree/` も同様）・`.claude/agents/reconciliation.md` Step 3-3・著作エージェント共通契約の tmp ミラーレイアウト。 |
