...
- `deferred_to` は形式（`#123` / `123` / URL）だけを検査し、**指す Issue が実在するかは
  検証しない**。
- **`scope` の覆しがオーナーの判断であることも機械側は検証していない**（Issue #531）。
  台帳は取り込まれた `scope` の値を受け取るだけで、それが誰の判断かも、レビューアの元の申告と
  違うかも見ていない。とりわけ **round 1 の覆しは原理的に検出できない**——`pr-reviewer` の
  finding は主文脈を経て取り込みレポートになるため、最初の `ingest-review` より前に
  書き換えられると元の申告値がどこにも残らない。
  - **round 2 以降**は、前ラウンドで台帳に記録済みの `scope` との差分として検出しうる
    （Issue #523 のスコープ。覆しに `override_ref`＝`feedback_ledger` の台帳エントリ参照を
    要求する変更）。
  - **round 1** を塞ぐには、レビューアの申告が**主文脈の手を経ずに改変不能な一次記録として
